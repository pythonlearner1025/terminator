...
export function createSoldierFigure(E, variant = 'olive') {
  ...
  for (const sign of [-1, 1]) {
    ball(head, [.017, .038, .025], [sign * .100, -.018, -.006], skinShade)
    ball(head, [.038, .030, .025], [sign * .051, -.031, .074], skin)
    ball(head, [.024, .014, .011], [sign * .042, .020, .085], skinShade)
    ball(head, [.017, .007, .005], [sign * .042, .018, .094], patch)
    ball(head, [.006, .006, .004], [sign * .042, .018, .099], rubber)
    box(head, [.039, .008, .008], [sign * .042, .034, .093], leather, [0, 0, sign * -.12])
  }
  ball(head, [.017, .039, .023], [0, -.007, .098], skin)
  ball(head, [.023, .014, .022], [0, -.030, .114], skinShade)
  ...
  for (const [side, sign] of [['Left', -1], ['Right', 1]]) {
    ...
    ball(upper, [.096, .107, .097], [0, -.039, 0], cloth)
    ball(upper, [.078, .142, .076], [0, -.146, 0], cloth)
    box(upper, [.014, .079, .07], [sign * .078, -.124, 0], seam)
    box(upper, [.017, .055, .058], [sign * .087, -.116, 0], patch)
    for (let i = 0; i < 3; i++) ball(upper, [.081, .014, .079], [0, -.224 - i * .016, .001], i % 2 ? seam : cloth)
