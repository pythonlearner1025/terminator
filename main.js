...
export async function main({viewer}) {
  ...
  registerGameValidation(async () => {
    ...
    const lab = manager?.constructor.ComponentType === 'WeaponsLab'
    ...
    if (!lab && manager?.world && !manager.viewsStarted) manager.startViews()
    await manager?.mapView?.ready
    await manager?.visualWarmup
    await manager?.ready
    const requiredNodes = lab
      ? ['Lab_Manager', 'Range', 'Firing_Line', 'Turntable', 'Targets_10m', 'Targets_20m', 'Targets_40m', 'Plates',
        'Lights_Night', 'Lights_Overcast', 'Lights_Noon', 'Lab_Camera', 'Unit T-600 Scout', 'Unit T-800 Endo', 'Unit T-800 Heavy']
      : ['Game Manager', 'Map', 'Unit T-600 Scout', 'Unit T-800 Endo', 'Unit T-800 Heavy', 'Player Start']
    ...
    const systemsReady = Boolean(manager?.world && manager?.hud && manager?.mapView && manager?.unitView && manager?.playerView)
      && (lab || authoredPieces > 0 && runtimeBatches > 0)
    const v2Modules = manager?.mapView?.v2Handles?.length || 0
    const v2Error = manager?.mapView?.v2Error?.message || null
    const v2TransparencyReady = lab || !viewer.renderManager?.rgbm || manager?.mapView?.v2Transparency?.attached === true
    const v2Ready = lab || !v2Error && v2Modules === 5 && v2TransparencyReady
    return {
      ...
      checks: {lab, nodesPresent, systemsReady, authoredPieces, runtimeBatches, v2Modules, v2Error, v2TransparencyReady, tickRate: 60},
    }
  })
}
