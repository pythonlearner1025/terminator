...
// Run on 4674 with --no-open. All browsers remain headless.
...
if(new URL(dev.url).port!=='4674')throw Error('Expected local weapons server on 4674')
...
try {
  ...
  for(const [id,distance] of [['m4',20],['m4',40],['sniper',40]]) {
    const record=await page.evaluate(({id,distance,variant})=>{
      ...
      if((variant==='before')!==(p.style.trail===(id==='m4'?2.4:5.5)))throw Error('Wrong tracer version')
  ...
  await page.evaluate(()=>{
    ...
    w.projectiles=Array.from({length:9},(_,i)=>({id:'tracer-volley-'+i,type:i%3===0?'round':'bolt',owner:'unit',
      pos:{x:-10+i*1.2,y:1.6+(i%3)*.35,z:-4},vel:{x:(5-i)*.3,y:0,z:18}}))
    window.pose(1)
    // Preserve head positions at twenty metres while warming their bounded trails.
    for(let t=0;t<4;t++)window.pose(6)
  })
  await page.waitForTimeout(300)
  ...
  if(errors.length||!cleanup||measurements.some(m=>m.active!==1))throw Error('Capture validation failed')
