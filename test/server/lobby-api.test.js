...
test('all lobby HTTP endpoints complete a match relay lifecycle', async (t) => {
  ...
  const game = (route, options = {}) => json(fixture.url, `${base}${route}`, {...options, headers: gameHeaders(created)})

  assert.equal((await json(fixture.url, `${base}/game/phase`, {method: 'POST', body: {phase: 'wave'}})).body.error, 'GAME_AUTH_FAILED')
  ...
  assert.deepEqual(rules.body.map_summary.gates.map(g=>g.id), map.spawnGates.map(g=>g.id))
  ...
  const intermission = await game('/game/wave_summary', {method: 'POST', body: {wave: 1, summary, budget: 540, deadline_ms: Date.now() + 30_000}})
  ...
  await game('/game/phase', {method: 'POST', body: {phase: 'wave', wave: 2, budget: 540, applied_config: plan.body.config}})
  ...
  assert.equal((await game('/game/events', {method: 'POST', body: {event: {type: 'purchase', wave: 2, t: 1, item: 'armor', price: 30}}})).body.delivered, 1)
  assert.equal((await game('/game/match_end', {method: 'POST', body: {stats: {wave: 2, won: true}}})).body.ok, true)
...
test('late config is not applied and increments fallback exactly once', async (t) => {
  ...
  await json(fixture.url, `${base}/game/plan/2?force=1`, {headers})
