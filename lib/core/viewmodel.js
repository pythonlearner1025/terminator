...
import {recentEventsFor} from './recent-events.js'

export function projectViewModel(world, playerId = world.hostPlayerId, {includeEnemyNameplates = true} = {}) {
  ...
  const recentEvents = recentEventsFor(world, now)
  ...
  // The game HUD omits enemy overlays; avoid their allocation and LOS queries.
  // Default keeps the full projection contract for tools and other consumers.
  const nameplates = includeEnemyNameplates ? world.aliveUnits.filter((unit) => {
  ...
  return {
    ...
    wave: {
      ...
      traderOpen: Boolean(world.traderOpen),
    },
    director: {
      state: world.director?.state || 'build_up',
      intensity: round(world.director?.intensity || 0, 3),
      reservoir: world.director?.reservoir || 0,
      reservoirMax: world.director?.reservoirMax || 0,
      traderOpen: Boolean(world.traderOpen),
    },
    extraction: world.extraction
      ? {pos: {...world.extraction.pos}, phase: world.extraction.phase, timer: round(world.extraction.timer, 2)}
      : null,
