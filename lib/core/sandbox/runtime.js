...
(() => {
  ...
  return (selfJson, senseJson, memJson) => {
    ...
    if (!initialized) {
      const restoredMem = parse(memJson)
      for (const key of objectKeys(restoredMem)) mem[key] = restoredMem[key]
...
export class UnitScriptRuntime {
  ...
  tick(self, sense, act, mem) {
    ...
    const inputMemJson = JSON.stringify(mem)
    const selfHandle = this.context.newString(selfJson)
    const senseHandle = this.context.newString(senseJson)
    const memHandle = this.context.newString(inputMemJson)
    this.currentSense = sense
    this.resetFuel()
    let result
    try {
      result = this.context.callFunction(this.runHandle, this.context.undefined, selfHandle, senseHandle, memHandle)
    } finally {
      this.finishFuel()
      this.currentSense = null
      memHandle.dispose()
