import {distance, pointInsideExpandedBox} from './math.js'
...
export class NavGrid {
  constructor(map, dynamicColliders = []) {
    ...
    this.colliderById = new Map(map.colliders.map((collider) => [collider.id, collider]))
  ...
  rebuild(dynamicColliders = []) {
    ...
    const blockers = [...this.map.colliders.filter((item) => item.navBlock), ...dynamicColliders]
  ...
  walkableSurfacesAt(pos, radius = 0) {
    ...
    for (const surface of this.surfaceSpecs) {
      ...
      if (Math.abs(pos.x - collider.center.x) > collider.size.x / 2 + radius) continue
      if (Math.abs(pos.z - collider.center.z) > collider.size.z / 2 + radius) continue
      matches.push({
        id: surface.collider,
  ...
  colliderBlocksNode(collider, node, point) {
    ...
    if (!pointInsideExpandedBox(point, collider, this.spec.agentRadius)) return false
    const bottom = collider.center.y - collider.size.y / 2
    const top = collider.center.y + collider.size.y / 2
    return top > node.y + EPSILON && bottom < node.y + this.agentHeight - EPSILON
  }
