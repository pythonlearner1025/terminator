...
import {colliderBounds, colliderSurfacesAt, pointInsideColliderFootprint, staticColliders} from './collision.js'
...
export class NavGrid {
  ...
  rebuild(dynamicColliders = []) {
    ...
    // Keep source order, but only test solids whose bounds can touch this cell.
    // The margin exceeds the narrow phase's footprint epsilon, including zero-radius queries.
    const candidates = Array.from({length: this.cellNodes.length}, () => [])
    const padding = (this.spec.agentRadius ?? 0) + .001, size = this.spec.cellSize, origin = this.spec.origin
    for (const collider of blockers) {
      const bounds = colliderBounds(collider)
      const x0 = Math.max(0, Math.ceil((bounds.min.x - padding - origin.x) / size - .5))
      const x1 = Math.min(this.spec.width - 1, Math.floor((bounds.max.x + padding - origin.x) / size - .5))
      const z0 = Math.max(0, Math.ceil((bounds.min.z - padding - origin.z) / size - .5))
      const z1 = Math.min(this.spec.height - 1, Math.floor((bounds.max.z + padding - origin.z) / size - .5))
      for (let z = z0; z <= z1; z++) for (let x = x0; x <= x1; x++) candidates[this.index(x,z)].push(collider)
    }
    for (const node of this.nodes) {
      const point = this.nodePoint(node)
      if (candidates[this.index(node.x,node.z)].some((collider) => this.colliderBlocksNode(collider, node, point))) {
    ...
    this.neighborCache = new Array(this.nodes.length)
    this.neighborStep = this.maxStep
  }
  ...
  colliderBlocksNode(collider, node, point) {
    ...
    const surfaces = colliderSurfacesAt(point, collider, this.spec.agentRadius)
    if (!surfaces.length || this.isMovementHole(collider.id, point, this.spec.agentRadius)) return false
    return surfaces
      .some(({bottom, top}) => top > node.y + EPSILON && bottom < node.y + this.agentHeight - EPSILON)
  }

  neighbors(node) {
    if (this.neighborCache && this.neighborStep === this.maxStep && this.nodes[node.index] === node) {
      return this.neighborCache[node.index] ??= Object.freeze(this.collectNeighbors(node))
    }
    return this.collectNeighbors(node)
  }
