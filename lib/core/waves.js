...
export function difficultyScaling(playerCount = 1) {
  const count = Math.max(1, Math.min(3, Math.floor(Number(playerCount) || 1)))
  return structuredClone(COOP_SCALING[count])
}

export function scaledWaveBudget(wave, telemetry = null, playerCount = 1) {
  const budget = waveBudget(wave, telemetry)
  const scaling = difficultyScaling(playerCount)
...
export class WaveDirector {
  ...
  getState() {
    ...
    const budget = this.phase === 'ended' ? null : scaledWaveBudget(nextWave, this.lastTelemetry, this.world.players.size)
    return {
      phase: this.phase,
      wave: this.wave,
  ...
  submitConfig(submission, {atMs = this.now()} = {}) {
    ...
    const budget = scaledWaveBudget(targetWave, this.lastTelemetry, this.world.players.size).applied
  ...
  beginWave(directConfig = null) {
    ...
    const budgetInfo = scaledWaveBudget(wave, this.lastTelemetry, this.world.players.size)
  ...
  selectConfig(wave, budget, directConfig) {
    ...
    if (!validateWaveConfig(config, {wave, budget, map: this.world.map, catalog: this.world.unitCatalog}).ok) {
      ...
      config = this.builtin?.plan({
        ...
        scaling: difficultyScaling(this.world.players.size),
      })
    }
    if (!validateWaveConfig(config, {wave, budget, map: this.world.map, catalog: this.world.unitCatalog}).ok) {
      source = 'emergency'
...
export function buildRulesPayload({
  ...
  return {
    ...
    scaling: difficultyScaling(playerCount),
