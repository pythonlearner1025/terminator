...
import {Pacer} from './director.js'
import {DEFAULT_LOCK_DOOR, Deck, DELAYED_CARDS, FOG_CARD_LEVEL, KNOB_PRICES, POPULATION, Population} from './population.js'
...
export const COOP_SCALING = Object.freeze({
  1: Object.freeze({players: 1, budgetMultiplier: 1, unitHealthMultiplier: 1, maxAlive: 32}),
  2: Object.freeze({players: 2, budgetMultiplier: 1.6, unitHealthMultiplier: 1.35, maxAlive: 38}),
  3: Object.freeze({players: 3, budgetMultiplier: 2.1, unitHealthMultiplier: 1.7, maxAlive: 44}),
})

export const PERFORMANCE_FORMULA = Object.freeze({
  base: '300 + 120 * wave',
  multiplier: 'clamp(1 + health_score + speed_score + damage_score, 0.9, 1.1)',
...
export function performanceMultiplier(telemetry) {
  ...
  return round(clamp(1 + healthScore + speedScore + damageScore, PERFORMANCE_FORMULA.min, PERFORMANCE_FORMULA.max), 3)
}
...
// Commons are the pacer's bread and butter, so the 40 percent per-type cap
// guards specials only. A type with no role declared counts as a special.
export function isCappedType(catalog, id) {
  const role = catalog?.types?.[id]?.role
  return role == null || role === 'special'
}

export function validateWaveConfig(config, {wave = 1, budget, map = mapData, catalog = unitsData, requireUnits = true} = {}) {
  ...
  for (const [id, spend] of Object.entries(spendByType)) {
    if (!isCappedType(catalog, id)) continue
  ...
  // An external agent still lists units. The built-in plan may leave the wave to
  // the pacer, so requireUnits is off for it.
  if (requireUnits) {
...
export class WaveDirector {
  constructor(world, {
    ...
    // The in-wave pacer. Tests that drive the legacy schedule alone turn it off.
    pacer = true,
  } = {}) {
    this.world = world
    ...
    this.pacer = pacer ? new Pacer(world) : null
    this.population = this.pacer ? new Population(world, this.pacer) : null
    this.deck = this.pacer ? new Deck(world.rng) : null
    this.card = 'nothing'
    this.finaleMobSeconds = 0
    this.phase = 'lobby'
    this.wave = 0
    this.config = null
    this.spawnSchedule = []
  ...
  returnToLobby() {
    ...
    // director pass: a new match deals a fresh deck and a cold intensity meter.
    this.card = 'nothing'
    this.world.extraction = null
    if (this.pacer) {
      this.pacer.reset()
      this.deck = new Deck(this.world.rng)
    }
  ...
  beginWave(directConfig = null) {
    ...
    this.startPacedWave(wave, budgetInfo.applied, selection.population)
    return {
      ok: true,
      errors: [],
  ...
  // ---- director pass: the in-wave pacer, the deck, and the finale ----

  // The legacy schedule is paid for first. Whatever is left is the reservoir the
  // pacer spends on mobs, specials, wanderers, and the end-of-wave rush.
  startPacedWave(wave, budget, population = null) {
    if (!this.pacer) return
    const legacyCost = configCost(this.config, this.world.unitCatalog)
    this.population.numbers = {...POPULATION, ...(population || {})}
    this.pacer.beginWave({
      wave,
      reservoir: budget - legacyCost,
      rushReserveFraction: this.population.numbers.rushReserveFraction,
    })
    const {card} = this.deck.deal(wave)
    this.card = card
    // Every card is dealt at wave start. A card with no delayed effect also
    // fires there; lights_out, heavy_pair, and t1000_hunt fire later.
    this._recordWorldEvent('deck_card', {card, wave, phase: 'dealt'})
    this.applyCardKnobs(card)
    if (!DELAYED_CARDS.includes(card)) this._recordWorldEvent('deck_card', {card, wave, phase: 'fired'})
    this.population.beginWave({wave, card, finale: this.isFinaleWave(wave)})
    this.world.pickups?.plan(wave, this.world.rng)
    this.startExtraction(wave)
  }

  // Wave-start cards are map knobs. They pay the normal knob price.
  applyCardKnobs(card) {
    if (card === 'fog') {
      if (!this.pacer.spend(KNOB_PRICES.fogLevel * FOG_CARD_LEVEL)) return
      this.world.configureMap({fog: FOG_CARD_LEVEL})
      return
    }
    if (card !== 'door_lock') return
    if (!this.pacer.spend(KNOB_PRICES.door)) return
    this.world.configureMap({doors: {[this.mostUsedDoor()]: 'locked'}})
  }

  mostUsedDoor() {
    const uses = this.lastTelemetry?.doorUses || this.lastTelemetry?.door_uses || {}
    const known = new Set(this.world.map.doors.map(({id}) => id))
    const ranked = Object.entries(uses)
      .filter(([id]) => known.has(id))
      .sort((a, b) => Number(b[1]) - Number(a[1]) || a[0].localeCompare(b[0]))
    return ranked[0]?.[0] || DEFAULT_LOCK_DOOR
  }

  isFinaleWave(wave) {
    return wave === this.maxWaves && Boolean(this.world.map.extraction)
  }

  startExtraction(wave) {
    if (!this.isFinaleWave(wave)) {
      this.world.extraction = null
      return
    }
    this.world.extraction = {pos: copyVec(this.world.map.extraction.pos), phase: 'announced', timer: 0}
    this.finaleMobSeconds = 0
    this._recordWorldEvent('extraction', {phase: 'announced', pos: copyVec(this.world.extraction.pos), t: 0})
  }
  ...
  // The chopper lands at holdSeconds. After that every unit hunts and free mobs
  // keep coming until the team stands on the pad.
  tickExtraction() {
    const extraction = this.world.extraction
    if (!extraction || extraction.phase === 'complete') return
    const dt = 1 / 60
    extraction.timer = round(extraction.timer + dt, 4)
    const hold = Number(this.world.map.extraction?.holdSeconds) || POPULATION.finale.holdSeconds
    if (extraction.phase === 'announced' && extraction.timer >= hold) {
      extraction.phase = 'arrived'
      this.world.configureMap({break_flank_wall: true})
      for (const unit of this.world.aliveUnits) this.world.enrageUnit?.(unit.id)
      this.finaleMobSeconds = 0
      this._recordWorldEvent('extraction', {phase: 'arrived', pos: copyVec(extraction.pos), t: extraction.timer})
    }
    if (extraction.phase !== 'arrived') return
    this.finaleMobSeconds -= dt
    if (this.finaleMobSeconds > 0) return
    this.population.spawnMob({size: POPULATION.finale.mobSize})
    this.finaleMobSeconds = POPULATION.finale.mobEverySeconds
  }

  // Only a chopper on the pad extracts anybody. Standing on an empty pad at wave
  // start would otherwise win the finale in one tick.
  extracted() {
    const extraction = this.world.extraction
    if (extraction?.phase !== 'arrived') return false
    const players = this.world.livingPlayers
    if (players.length === 0) return false
    return players.every((player) => planarDistance(player.pos, extraction.pos) <= POPULATION.finale.exitRadius)
  }

  waveCleared() {
    if (this.nextSpawn < this.spawnSchedule.length || this.world.aliveUnits.length > 0) return false
    // The finale ends at the landing pad, never by clearing the map.
    if (this.world.extraction) return false
    if (!this.pacer) return true
    return this.pacer.reservoir <= 0 && this.population.idle
  }

  // A different trader spot every intermission. world.setTraderSpot keeps the
  // current spot id and emits trader_moved.
  moveTrader() {
    const spots = this.world.map.traderSpots || []
    if (spots.length < 2 || typeof this.world.setTraderSpot !== 'function') return
    const options = spots.filter(({id}) => id !== this.world.traderSpotId)
    const spot = options[Math.floor(this.world.rng.next() * options.length)]
    if (spot) this.world.setTraderSpot(spot.id)
  }
  ...
  selectConfig(wave, budget, directConfig) {
    ...
    // director pass: the built-in plan leaves population to the pacer, so it
    // lists units only on boss waves and skips the at-least-one-unit rule.
    let requireUnits = true
    ...
    if (!validateWaveConfig(config, {wave, budget, map: this.world.map, catalog: this.world.unitCatalog, requireUnits}).ok) {
      source = 'emergency'
      requireUnits = false
      config = emergencyWaveConfig(wave, budget, this.world.map, this.world.unitCatalog)
    }
    const validation = validateWaveConfig(config, {wave, budget, map: this.world.map, catalog: this.world.unitCatalog, requireUnits})
    if (!validation.ok) return validation
    return {ok: true, config: normalizeConfig(config), fallback, reason, source, population: config.population}
  }
  ...
  step(inputs = {}) {
    ...
    if (this.phase === 'wave') {
      ...
      else if (this.extracted()) this.finishExtraction()
      else if (this.waveCleared()) this.finishWave()
      return
    }
    if (this.phase === 'intermission') {
      this.world.step(inputs)
      // The meter keeps cooling between waves. Only the pacer runs: nothing
      // spawns in an intermission, so the population stays still.
      this.pacer?.step()
      this._flushWorldEvents()
      this.emitPlayerPosition()
  ...
  // The team reached the pad. Everything still alive stays alive; the match is won.
  finishExtraction() {
    const extraction = this.world.extraction
    extraction.phase = 'complete'
    this._recordWorldEvent('extraction', {phase: 'complete', pos: copyVec(extraction.pos), t: extraction.timer})
    return this.finishWave({endReason: 'extracted'})
  }

  forceRush() {
    if (!this.rushActive) {
      this.rushActive = true
      // director pass: the cap also empties the reservoir so the wave can end.
      this.population?.forceRush()
  ...
  setPhase(phase) {
    ...
    // director pass: the trader opens in intermission and in every relax window.
    this.pacer?.sync()
