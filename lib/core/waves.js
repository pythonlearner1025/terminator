...
export class WaveDirector {
  ...
  finishWave({playerDied = false, endReason = 'cleared'} = {}) {
    const timeToClear = this.world.waveTime()
  ...
  _forwardWorldEvent(event) {
    ...
    if (event.type === 'purchase') return void this._emitProtocol('purchase', {item: event.item, price: event.price}, event)
...
export function buildRulesPayload({map = mapData, catalog = unitsData, defaultBrains = DEFAULT_BRAINS} = {}) {
  return {
    ...
    default_scripts: Object.fromEntries(Object.entries(defaultBrains).map(([type, brain]) => [type, defaultScriptSource(brain)])),
    map_summary: {
      id: map.id,
      name: map.name,
