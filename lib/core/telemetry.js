...
export function buildWaveSummary(telemetry, {
  ...
  return {
    ...
    kills: (source.kills || []).map((kill) => ({
      ...
      weapon: kill.weapon,
      part: kill.part || null,
      pos: cloneNullable(kill.pos),
      direction: cloneNullable(kill.direction),
    ...
    // What the in-wave pacer did with this wave.
    director: {
      peaks: finiteOr(source.director?.peaks, 0),
      mobs: finiteOr(source.director?.mobs, 0),
      stragglers: finiteOr(source.director?.stragglers, 0),
      relax_seconds: round(finiteOr(source.director?.relaxSeconds ?? source.director?.relax_seconds, 0), 4),
    },
    players: Object.values(source.players || {}).map((player) => buildPlayerSummary(player, source)),
    units,
    unit_types: structuredClone(source.unitTypes || source.unit_types || {}),
