import defaultMap from '../data/map.json' with {type: 'json'}
...
const DEFAULT_BRAINS = Object.freeze({scout: scoutBrain, endo: endoBrain, heavy: heavyBrain})
...
export function simulate({
  ...
  for (let tick = 0; tick < maxTicks; tick += 1) {
    while (nextSpawn < schedule.length && world.aliveUnits.length < world.unitCatalog.maxAlive) {
    ...
    world.step({...recorded, fire: false})
    if (recorded.fire) fireGhostAtNearestVisible(world, ghost)
  ...
  for (const type of Object.keys(world.unitCatalog.types)) {
    const spawned = world.units.filter((unit) => unit.type === type)
    if (spawned.length === 0) continue
    const stats = spawned.map((unit) => world.telemetry.units[unit.id])
    units[type] = {
      spawned: spawned.length,
      killed: spawned.filter((unit) => !unit.alive).length,
      damage_dealt: round(stats.reduce((sum, item) => sum + item.damageDealt, 0), 4),
      avg_lifetime: round(stats.reduce((sum, item) => sum + item.lifetime, 0) / spawned.length, 4),
    }
  }
  const unitStats = Object.values(world.telemetry.units)
  return {
    time_to_clear: cleared ? elapsed : null,
    player_died: !world.player.alive,
...
function fireGhostAtNearestVisible(world, ghost) {
  ...
  world.recordShot(weaponId, hit)
  world.addSound('gunshot', player.pos)
  world.emit('shot', {by: 'player', weapon: weaponId, hit, headshot: false, killed: Boolean(target && !target.alive), unitId: target?.id || null, origin})
  return true
}
...
function prepareGhostWeapons(world, ghost) {
  ...
  for (const id of world.weaponCatalog.slots) {
    ...
    world.player.ammo[id].reserve = world.weaponCatalog.weapons[id].reserveMax
  }
}
