import {normalizeInputs} from '../world.js'
...
export function ghostFromWave(telemetry, replay) {
  ...
  return {
    ...
    inputs: Array.isArray(replay) ? replay.map((input) => normalizeInputs(input)) : [],
    accuracy,
...
export function best(telemetry, replays) {
  const entries = waveEntries(telemetry).filter(([, summary]) => {
    ...
    return Number.isFinite(Number(clear)) && !summary?.player_died && !summary?.playerDied
  })
  if (entries.length === 0) return null
