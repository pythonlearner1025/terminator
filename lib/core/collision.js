...
export function sweepSphereCollider(origin, movement, radius, collider) {
  if (axisAlignedBox(collider)) {
    // Most projectile segments miss a wall entirely. Reject their swept bounds
    // before allocating local-origin/expanded-size records for the slab test.
    const x=origin.x-collider.center.x,y=origin.y-collider.center.y,z=origin.z-collider.center.z
    const hx=collider.size.x/2+radius,hy=collider.size.y/2+radius,hz=collider.size.z/2+radius
    if(Math.min(x,x+movement.x)>hx||Math.max(x,x+movement.x)<-hx||
      Math.min(y,y+movement.y)>hy||Math.max(y,y+movement.y)<-hy||
      Math.min(z,z+movement.z)>hz||Math.max(z,z+movement.z)<-hz)return null
