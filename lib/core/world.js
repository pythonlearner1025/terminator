...
export class World {
  ...
  unitHitCollider(unit, spec = this.unitCatalog.types[unit.type]) {
    ...
    shapes ||= [
      {id: 'body', part: 'body', shape: 'box', offset: {x: 0, y: spec.height * 0.42, z: 0}, size: {x: spec.radius * 2, y: spec.height * 0.66, z: spec.radius * 2}},
      {id: 'head', part: 'head', shape: 'sphere', offset: {x: 0, y: spec.height * 0.84, z: 0}, radius: spec.radius * 0.5},
    ]
  ...
  hitscan({origin, direction, weaponId = 'pistol', damage, source = 'player', playerId = null, maxDistance = 100}) {
    ...
    for (const collider of this.activeColliders()) {
      ...
      if (hit && hit.distance < closest.distance) closest = {kind: 'collider', colliderId: collider.id, distance: hit.distance}
    }
    for (const unit of this.units) {
      if (!unit.alive) continue
      ...
      closest = {kind: 'unit', unit, unitId: unit.id, distance: hit.distance, headshot: hit.part.part === 'head', hitPart: hit.part.id}
    }
