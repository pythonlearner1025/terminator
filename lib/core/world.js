 ...
 */
export class World {
  constructor({map = mapData, units = unitsData, weapons = weaponsData, brains = {}, seed = 1} = {}) {
    this.map = map
    ...
    this.skynet = {name: 'BUILT-IN', connected: true, fallbackCount: 0, revs: {scout: 1, endo: 1, heavy: 1}}
  ...
  configureMap(knobs = {}) {
    ...
    for (const unit of this.units) unit.pathCache.path = null
  }
  ...
  dynamicColliders() {
    ...
    for (const door of this.map.doors) {
      ...
      if (door.pos.y - door.size.y / 2 > 1.8) continue
  ...
  spawnUnit(typeId, pos, {yaw = 0, rev = 1, id} = {}) {
    ...
    this.emit('unit_spawn', {unitId: unit.id, unitType: unit.type, rev})
    return unit
  }
  ...
  step(rawInputs = EMPTY_INPUTS) {
    ...
    if (this.tick % 6 === 0) {
      for (const unit of this.units) if (unit.alive) this.tickBrain(unit)
    }
  ...
  movePlayer(inputs) {
    ...
    const canSprint = inputs.sprint && moving && !inputs.crouch && player.sprintStamina > 0
    ...
    const speed = inputs.crouch ? 2.6 : canSprint ? 7.5 : 5
    ...
    if (!this.positionBlocked({x: nextX, z: player.pos.z}, PLAYER_RADIUS)) player.pos.x = nextX
    else player.vel.x = 0
    if (!this.positionBlocked({x: player.pos.x, z: nextZ}, PLAYER_RADIUS)) player.pos.z = nextZ
    else player.vel.z = 0
    if (moving) {
  ...
  positionBlocked(pos, radius = 0) {
    return this.activeColliders().some((collider) => collider.navBlock && pointInsideExpandedBox(pos, collider, radius))
  }
  ...
  buildAct(unit) {
    return {
      ...
      stop: () => { unit.intent.moveTo = null; unit.vel.x = 0; unit.vel.z = 0 },
  ...
  pathForUnit(unit, target) {
    ...
    const targetKey = `${targetCell.x}:${targetCell.z}`
    const cache = unit.pathCache
    const mayRepath = this.tick - cache.computedAtTick >= 30
    if (!cache.path || (cache.targetKey !== targetKey && mayRepath)) {
    ...
    for (let ring = 1; ring <= cells; ring += 1) {
      for (let dz = -ring; dz <= ring; dz += 1) {
        for (let dx = -ring; dx <= ring; dx += 1) {
          ...
          if (this.nav.isBlocked(cell)) continue
          const candidate = this.nav.cellCenter(cell)
          candidate.y = 1
          if (this.lineOfSight(fromPos, candidate)) continue
  ...
  randomPoint(center, radius) {
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      const point = {x: center.x + Math.sin(angle) * length, y: 0, z: center.z + Math.cos(angle) * length}
  ...
  moveUnit(unit, spec) {
    ...
    const cache = unit.pathCache
    while (cache.index < path.length - 1 && planarDistance(unit.pos, path[cache.index]) < 0.45) cache.index += 1
    ...
    this.telemetry.units[unit.id].distanceTraveled += step
  }
