...
export class World {
  ...
  traderViewModel() {
    ...
    return {
      fillAmmo,
      fullArmor: Math.ceil(Math.max(0, 100 - this.player.armor) * 2),
      medkit: 100,
      error: null,
    }
  }
  ...
  purchase(item) {
    ...
    const quote = this.traderViewModel()
    let price = 0
    let name = item
    let apply = null
    ...
    player.scrap -= price
    apply()
    const purchase = {t: this.waveTime(), item: String(item), price}
  ...
  get playerSpread() {
    const base = this.weaponCatalog.weapons[this.player.activeWeapon]?.spreadDeg || 0
    return base * (this.player.aiming && this.player.reloadTimer === 0
  ...
  playerFire() {
    ...
    for (let pellet = 0; pellet < (weapon.pellets || 1); pellet += 1) {
      const spread = this.playerSpread * Math.PI / 180
