...
export class World {
  ...
  purchase(item, playerId = this.hostPlayerId) {
    if (this.phase !== 'intermission') return {ok: false, error: 'Trader closed'}
    const player = this.players.get(playerId)
    ...
    if (player.scrap < price) return {ok: false, error: `Need ${price} Scrap`}
    player.scrap -= price
    apply()
  ...
  playerSupportAt(pos, currentY, {radius = 0, maxAbove = this.nav.maxStep, maxBelow = this.nav.maxStep} = {}) {
    ...
    for (const collider of this.activeColliders()) {
      if (this.nav.isSurfaceCollider(collider.id) || this.nav.isMovementHole(collider.id, pos, radius)
        || !pointInsideExpandedBox(pos, collider, radius)) continue
      const y = collider.center.y + collider.size.y / 2
  ...
  throwGrenade(playerId = this.hostPlayerId) {
    ...
    const projectile = {
      id: `grenade-${player.id}-${this.tick}`,
      type: 'grenade',
      playerId: player.id,
      pos,
  ...
  updateProjectiles() {
    ...
    for (const projectile of this.projectiles) {
      ...
      if (this.tick >= projectile.detonateTick) {
  ...
  updateGrenadeProjectile(projectile, colliders) {
    ...
    const support = projectile.grounded
      ? colliders.find((collider) => {
        const top = collider.center.y + collider.size.y / 2
        return Math.abs(projectile.pos.y - radius - top) <= 0.012
          && Math.abs(projectile.pos.x - collider.center.x) <= collider.size.x / 2 + radius
          && Math.abs(projectile.pos.z - collider.center.z) <= collider.size.z / 2 + radius
      })
      : null
    if (!support) {
      ...
      projectile.pos.y = support.center.y + support.size.y / 2 + radius
      projectile.vel.y = 0
  ...
  sweepGrenade(origin, movement, radius, colliders) {
    ...
    for (const collider of colliders) {
      ...
      for (const axis of ['x', 'y', 'z']) {
        const min = collider.center[axis] - collider.size[axis] / 2 - radius
        const max = collider.center[axis] + collider.size[axis] / 2 + radius
        const delta = movement[axis]
        if (Math.abs(delta) < 1e-10) {
          if (origin[axis] < min || origin[axis] > max) far = -1
          continue
        }
        let enter = (min - origin[axis]) / delta
        let exit = (max - origin[axis]) / delta
        let sign = -1
        if (enter > exit) {
      ...
      if (!hitAxis || far < near || near < 0 || near > 1 || (closest && near >= closest.fraction)) continue
      const normal = {x: 0, y: 0, z: 0}
      normal[hitAxis] = hitSign
      closest = {fraction: near, normal, collider}
    }
    return closest
  }
  ...
  damageUnit(unitId, amount, context = {}) {
    ...
    if (shooter) {
      shooter.scrap += this.unitCatalog.types[unit.type].scrap
      const kill = {
        t: this.waveTime(),
        unitType: unit.type,
        unitId: unit.id,
        ...
        weapon: context.weapon || shooter.activeWeapon,
        distance: round(context.distance ?? planarDistance(shooter.pos, unit.pos), 3),
        headshot: Boolean(context.headshot),
  ...
  fireUnitWeapon(unit, spec) {
    ...
    const blockerDistance = this.closestColliderDistance(origin, direction, spec.range)
    let hitPlayer = null
    let hitDistance = spec.range
    for (const player of this.livingPlayers) {
      const center = {...player.pos, y: player.pos.y + (player.crouch ? CROUCH_EYE_HEIGHT : PLAYER_EYE_HEIGHT)}
      const candidate = raySphere(origin, direction, center, PLAYER_RADIUS * 1.5, spec.range)
      if (candidate === null || candidate >= hitDistance || (blockerDistance !== null && candidate >= blockerDistance)) continue
      hitPlayer = player
      hitDistance = candidate
    }
    const hit = Boolean(hitPlayer)
    let dealt = 0
    if (hitPlayer) dealt = this.damagePlayer(spec.damage, unit, hitPlayer.id)
    const stats = this.telemetry.units[unit.id]
    stats.shotsFired += 1
    stats.damageDealt += dealt
    this.recordShot(`unit:${unit.type}`, hit)
    this.emit('shot', {by: unit.id, unitType: unit.type, playerId: hitPlayer?.id || intended.id, weapon: spec.attack, hit, origin, target})
  }
  ...
  updateHazards() {
    ...
    for (const player of this.livingPlayers) {
      for (const active of this.mapState.hazards) {
        ...
        if (!slot || !pointInsideExpandedBox(player.pos, {center: slot.pos, size: slot.size})) continue
