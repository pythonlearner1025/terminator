...
import * as dummyBrain from './brains/dummy.js'
...
import * as enragedBrain from './brains/enraged.js'
import {NavGrid} from './nav.js'
import {Pickups} from './pickups.js'
import {SweepBroadphase} from './sweep-broadphase.js'
...
// The view accepts new corpses for two seconds, then owns their death effects.
export const DEAD_UNIT_RETENTION_TICKS = 2 * TICK_RATE
...
// Health regenerates back to a floor after a calm spell. See docs/director-design.md.
const REGEN_DELAY_TICKS = 5 * TICK_RATE
const REGEN_HP_PER_SECOND = 4
const REGEN_FLOOR_HP = 40
// Commons flinch on any hit. Specials keep the heavy-hit rule.
const COMMON_STAGGER_SECONDS = 0.25
const SPECIAL_STAGGER_DAMAGE = 65
const SPECIAL_STAGGER_SECONDS = 0.4
// Shove: the knife and a dense shotgun pattern push a common away from the player.
const SHOVE_SECONDS = 0.3
const KNIFE_SHOVE_STAGGER_SECONDS = 0.6
const KNIFE_SHOVE_METERS = 2.5
const SHOTGUN_SHOVE_PELLETS = 4
const SHOTGUN_SHOVE_METERS = 1.5
// Look-ahead path following aims this far along the path instead of at the next node.
const UNIT_LOOK_AHEAD_METERS = 2
// Spacing of the navigation-cell samples that keep look-ahead out of solids.
const NAV_SAMPLE_METERS = 0.4
// Commons push each other apart so a mob reads as a crowd, not a queue.
const COMMON_SEPARATION_METERS = 1.2
const COMMON_SEPARATION_SPEED = 1.6
// world.unitSeenByAnyPlayer uses this cone around the player's yaw.
const SEEN_BY_PLAYER_CONE_DEG = 100
// Wanderer sight bookkeeping runs at 2 Hz.
const WANDERER_SIGHT_INTERVAL_TICKS = TICK_RATE / 2
...
export class World {
  constructor({map = mapData, units = unitsData, weapons = weaponsData, brains = {}, scriptSources = {}, seed = 1} = {}) {
    ...
    this.pickups = new Pickups(map)
    this.traderSpotId = null
    this.phase = 'wave'
    this.wave = 1
    ...
    this.scaling = {players: 1, budgetMultiplier: 1, unitHealthMultiplier: 1, maxAlive: this.unitCatalog.maxAlive}
    this.skynet = {
      name: 'BUILT-IN',
      connected: true,
      fallbackCount: 0,
    ...
    // director pass: the pacer writes these every tick; they ride the snapshot.
    this.traderOpen = false
    this.director = {state: 'build_up', intensity: 0, reservoir: 0, reservoirMax: 0}
  ...
  sandboxEnabled() {
    return Boolean(this.sandbox.invulnerable || this.sandbox.infiniteScrap || this.sandbox.infiniteAmmo)
  }

  setSandbox({invulnerable = false, infiniteScrap = false, infiniteAmmo = false, noReload = false} = {}) {
    this.sandbox = {invulnerable: Boolean(invulnerable), infiniteScrap: Boolean(infiniteScrap),
      ...(infiniteAmmo ? {infiniteAmmo: true, noReload: Boolean(noReload)} : {})}
  ...
  purchase(item, playerId = this.hostPlayerId) {
    // The trader is a between-waves window. A live wave keeps it shut.
    if (this.phase !== 'intermission' && !this.traderOpen && !this.sandboxEnabled()) return {ok: false, error: 'Trader closed'}
    const player = this.players.get(playerId)
  ...
  // The director moves the trader between intermissions. The crate is a static
  // collider, so this world gets its own trader record and a fresh grid: the
  // shared authored map keeps the position it was loaded with.
  setTraderSpot(spotId) {
    if (!this.moveTraderTo(spotId)) return false
    this.emit('trader_moved', {id: this.traderSpotId, pos: copyVec(this.map.trader.pos)})
    return true
  }

  moveTraderTo(spotId) {
    const spot = (this.map.traderSpots || []).find((item) => item.id === spotId)
    if (!spot || spotId === this.traderSpotId) return false
    this.map = {...this.map, trader: {...this.map.trader, pos: copyVec(spot.pos), center: copyVec(spot.pos)}}
    this.traderSpotId = spot.id
  ...
  spawnUnit(typeId, pos, {yaw = 0, rev, id, brain, enraged = false, alerted = false, wanderer = false} = {}) {
    ...
    const unit = {
      ...
      enraged: Boolean(enraged),
      wanderer: Boolean(wanderer),
      knockback: null,
      lastSeenByPlayerTick: wanderer ? this.tick : null,
      brain: null,
      ...(brain === 'dummy' ? {brainType: 'dummy'} : {}),
      brainPhase: this.units.length % 6,
    }
    if (alerted) {
      const nearest = this.nearestLivingPlayer(spawnPos)
      if (nearest) unit.lastKnownPlayer = {id: nearest.id, pos: copyVec(nearest.pos), t: this.waveTime()}
    }
    ...
    try {
      unit.brain = this.resolveUnitBrain(unit)
    } catch (error) {
  ...
  retireDeadUnits() {
    // Keep projectile owners until impact/expiry so damage attribution survives
    // their death. Historical summaries live in telemetry, not simulation units.
    const projectileOwners = new Set(this.projectiles
      .filter((projectile) => projectile.owner === 'unit').map((projectile) => projectile.ownerId))
    let retained = 0
    for (const unit of this.units) {
      if (!unit.alive && unit.diedAtTick != null
        && this.tick - unit.diedAtTick > DEAD_UNIT_RETENTION_TICKS
        && !projectileOwners.has(unit.id)) {
        this.unitById.delete(unit.id)
        continue
      }
      this.units[retained++] = unit
    }
    this.units.length = retained
  }
  ...
  // The brain a unit runs given its current flags. Dummy and enraged are native
  // modules; every other unit runs its scripted or overridden default brain.
  resolveUnitBrain(unit) {
    if (unit.brainType === 'dummy') return dummyBrain
    if (unit.enraged) return enragedBrain
    return this.createUnitBrain(unit.type, unit.rev)
  }

  // Switch a living unit to the enraged brain. Returns whether it changed.
  enrageUnit(unitId) {
    const unit = this.unitById.get(unitId)
    if (!unit?.alive || unit.enraged) return false
    unit.enraged = true
    if (unit.brain !== enragedBrain) unit.brain?.destroy?.()
    unit.brain = enragedBrain
    unit.mem = {}
    const target = this.nearestLivingPlayer(unit.pos)
    if (target) {
  ...
  // Silent removal. No unit_death event, no scrap, no kill record. The unit
  // leaves this.units and aliveUnits; telemetry records causeOfDeath 'despawned'.
  despawnUnit(unitId) {
    const unit = this.unitById.get(unitId)
  ...
  // True when any living player has eye-to-eye line of sight to the unit and the
  // unit sits inside a SEEN_BY_PLAYER_CONE_DEG cone around that player's yaw.
  unitSeenByAnyPlayer(unit) {
    if (!unit) return false
    const halfCone = SEEN_BY_PLAYER_CONE_DEG * Math.PI / 360
    const unitEye = this.unitEye(unit)
    for (const player of this.players.values()) {
      if (!player.alive) continue
      if (Math.abs(normalizeAngle(yawTo(player.pos, unit.pos) - player.yaw)) > halfCone) continue
      const eye = {...player.pos, y: player.pos.y + (player.crouch ? CROUCH_EYE_HEIGHT : PLAYER_EYE_HEIGHT)}
      if (this.lineOfSight(eye, unitEye)) return true
    }
    return false
  }
  ...
  startReload(playerId = this.hostPlayerId) {
    ...
    const ammo = player.ammo[id]
    const weapon = this.weaponCatalog.weapons[id]
    if (this.sandbox.noReload && player.id === this.hostPlayerId) return false
  ...
  finishReload(playerId = this.hostPlayerId) {
    ...
    ammo.mag += rounds
    if (!(this.sandbox.infiniteAmmo && player.id === this.hostPlayerId)) ammo.reserve -= rounds
  ...
  playerFire(playerId = this.hostPlayerId) {
    ...
    if (!(this.sandbox.infiniteAmmo && this.sandbox.noReload && player.id === this.hostPlayerId)) ammo.mag -= 1
    ...
    let killed = false
    let hitUnitId = null
    const paths = this.sandbox.infiniteAmmo ? [] : null
    const pelletsPerUnit = (weapon.pellets || 1) >= SHOTGUN_SHOVE_PELLETS ? new Map() : null
    for (let pellet = 0; pellet < (weapon.pellets || 1); pellet += 1) {
      ...
      if (paths) {
        const ray = directionFromAngles(yaw, pitch)
        const last = result.hits?.at(-1) || result
        paths.push({direction: ray, point: last.point || addScaled(origin, ray, last.distance), hit: result.kind !== 'miss'})
      }
      if (result.kind === 'unit') {
        anyHit = true
        ...
        if (pelletsPerUnit) for (const hit of results) pelletsPerUnit.set(hit.unitId, (pelletsPerUnit.get(hit.unitId) || 0) + 1)
      }
    }
    // A dense pattern on one common shoves it back. Order is by unit id, so the
    // push does not depend on which pellet landed first.
    if (pelletsPerUnit) {
      for (const unitId of [...pelletsPerUnit.keys()].sort()) {
        if (pelletsPerUnit.get(unitId) < SHOTGUN_SHOVE_PELLETS) continue
        const unit = this.unitById.get(unitId)
        if (unit?.alive && this.isCommonUnit(unit)) this.shoveUnit(unit, player.pos, SHOTGUN_SHOVE_METERS)
      }
    }
    this.recordShot(id, anyHit, player.id)
    this.addSound('gunshot', player.pos, undefined, player.id)
    this.emit('shot', {by: player.id, playerId: player.id, weapon: id, hit: anyHit, headshot, killed, unitId: hitUnitId, origin, ...(paths ? {paths} : {})})
    return true
  }
  ...
  playerMelee(playerId = this.hostPlayerId) {
    ...
    if (best?.unit.alive && this.isCommonUnit(best.unit)) {
      best.unit.staggerTicks = Math.max(best.unit.staggerTicks || 0, Math.round(KNIFE_SHOVE_STAGGER_SECONDS * TICK_RATE))
      this.shoveUnit(best.unit, player.pos, KNIFE_SHOVE_METERS)
    }
  ...
  throwGrenade(playerId = this.hostPlayerId) {
    ...
    if (!(this.sandbox.infiniteAmmo && player.id === this.hostPlayerId)) player.grenades -= 1
  ...
  updateProjectiles() {
    ...
    // Map geometry is stable within this synchronous batch. Validate live
    // geometry every batch, including edits and changes to doors/flank walls.
    const broadphase = (this.projectileBroadphase ??= new SweepBroadphase()).prepare(colliders)
    for (const projectile of this.projectiles) {
      if (projectile.type === 'grenade') {
        if (this.tick >= projectile.detonateTick) this.detonateGrenade(projectile)
        else {
          this.updateGrenadeProjectile(projectile, colliders, broadphase)
      ...
      const collision = this.projectileCollision(projectile, direction, segmentLength, colliders, broadphase)
  ...
  projectileCollision(projectile, direction, maxDistance, colliders, broadphase = null) {
    let closest = null
    const radius = projectile.radius || 0.04
    const movement = scaleVec(direction, maxDistance)
    const mapHit = this.sweepGrenade(projectile.pos, movement, radius, colliders, broadphase)
    if (mapHit) closest = {
  ...
  updateGrenadeProjectile(projectile, colliders, broadphase = null) {
    ...
    for (let collisionCount = 0; remaining > 1e-7 && collisionCount < 4; collisionCount += 1) {
      ...
      const collision = this.sweepGrenade(projectile.pos, movement, radius, colliders, broadphase)
      if (!collision) {
  ...
  sweepGrenade(origin, movement, radius, colliders, broadphase = null) {
    if (broadphase) return broadphase.sweep(origin, movement, radius)
    let closest = null
  ...
  isCommonUnit(unit) {
    return this.unitCatalog.types[unit?.type]?.role === 'common'
  }

  // Push a living unit `meters` straight away from `fromPos` over SHOVE_SECONDS.
  // Movement integrates the per-tick step and stops the push at colliders.
  shoveUnit(unit, fromPos, meters) {
    if (!unit?.alive || meters <= 0) return false
    const dx = unit.pos.x - fromPos.x
    const dz = unit.pos.z - fromPos.z
    const length = Math.hypot(dx, dz)
    const ticksLeft = Math.max(1, Math.round(SHOVE_SECONDS * TICK_RATE))
    const direction = length > 1e-6
      ? {x: dx / length, z: dz / length}
      : {x: Math.sin(unit.yaw), z: Math.cos(unit.yaw)}
    unit.knockback = {
      x: direction.x * meters / ticksLeft,
      z: direction.z * meters / ticksLeft,
      ticksLeft,
    }
    return true
  }
  ...
  damageUnit(unitId, amount, context = {}) {
    ...
    const staggerSeconds = spec.role === 'common'
      ? (damage > 0 ? COMMON_STAGGER_SECONDS : 0)
      : (damage >= SPECIAL_STAGGER_DAMAGE ? SPECIAL_STAGGER_SECONDS : 0)
    if ((spec.staggerResist || 0) < 1 && staggerSeconds > 0) {
      unit.staggerTicks = Math.max(unit.staggerTicks || 0,
        Math.round(staggerSeconds * (1 - (spec.staggerResist || 0)) * TICK_RATE))
    }
  ...
  damagePlayer(amount, attacker = {}, playerId = this.hostPlayerId) {
    ...
    player.hp -= hpDamage
    player.lastDamagedAtTick = this.tick
    if (player.hp <= 0) {
      player.alive = false
      player.downed = true
    }
  ...
  // Regeneration heals only up to the floor, and only after a calm spell.
  // Medkits and the wave-clear reset heal past the floor; this never does.
  regeneratePlayer(player) {
    if (!player.alive || player.hp >= REGEN_FLOOR_HP) return
    if (this.tick - (player.lastDamagedAtTick ?? -Infinity) < REGEN_DELAY_TICKS) return
    player.hp = Math.min(REGEN_FLOOR_HP, player.hp + REGEN_HP_PER_SECOND * TICK_SECONDS)
  }
  ...
  tickBrain(unit) {
    if (unit.brainType === 'dummy') { dummyBrain.tick(null, null, this.buildAct(unit)); return }
    const type = this.unitCatalog.types[unit.type]
    ...
    if (target) {
      unit.targetPlayerId = target.id
      unit.lastKnownPlayer = {id: target.id, pos: copyVec(target.pos), t: this.waveTime()}
    } else if (unit.enraged) {
      // An enraged unit always knows where the nearest living player is.
      const nearest = this.nearestLivingPlayer(unit.pos)
      if (nearest) {
        unit.targetPlayerId = nearest.id
        unit.lastKnownPlayer = {id: nearest.id, pos: copyVec(nearest.pos), t: this.waveTime()}
      }
    ...
    try {
      ...
      this.telemetry.units[unit.id].scriptErrors += 1
      unit.brain?.destroy?.()
      unit.brain = unit.enraged ? enragedBrain : DEFAULT_BRAINS[unit.type]
      unit.mem = {}
  ...
  snapshot({includeLastInputsBundle = true, includeScriptRevisions = true} = {}) {
    ...
    const snapshot = {
      ...
      pickups: this.pickups?.snapshot() || null,
      traderSpotId: this.traderSpotId || null,
      phase: this.phase,
      wave: this.wave,
      ...
      ...(includeLastInputsBundle ? {lastInputsBundle: structuredClone(this.lastInputsBundle)} : {}),
      ...
      // director pass: guests mirror the pacer, the trader window, and the finale.
      traderOpen: Boolean(this.traderOpen),
      director: structuredClone(this.director),
      extraction: this.extraction ? structuredClone(this.extraction) : null,
      ...(includeScriptRevisions ? {scriptRevisions: Object.fromEntries([...this.scriptRegistry.revisions].map(([type, revisions]) => [type, structuredClone(revisions)]))} : {}),
      eventStart,
      eventCursor,
  ...
  applySnapshot(snapshot, {predictOnly = false} = {}) {
    ...
    // A snapshot from a build without the regeneration clock reads as never damaged.
    for (const player of this.players.values()) player.lastDamagedAtTick ??= null
    ...
    // Party guests predict players only. Reuse navigation while its immutable
    // map and effective blockers are unchanged; visual knobs do not affect it.
    const reuseNavigation = predictOnly && this.nav?.map === this.map
      && this.nav.spec === this.map.navGrid
      && Boolean(this.mapState?.flankWallBroken) === Boolean(snapshot.mapState?.flankWallBroken)
      && this.map.doors.every(door => (this.mapState?.doors?.[door.id] === 'locked')
        === (snapshot.mapState?.doors?.[door.id] === 'locked'))
    this.mapState = structuredClone(snapshot.mapState)
    if (!reuseNavigation) this.nav = new NavGrid(this.map, this.dynamicColliders())
    // A moved trader owns its own grid rebuild, whatever navigation reuse chose.
    this.moveTraderTo(snapshot.traderSpotId)
    this.pickups?.applySnapshot(snapshot.pickups)
    ...
    this.sandbox = snapshot.sandbox
      ? {invulnerable: Boolean(snapshot.sandbox.invulnerable), infiniteScrap: Boolean(snapshot.sandbox.infiniteScrap),
        ...(snapshot.sandbox.infiniteAmmo ? {infiniteAmmo: true, noReload: Boolean(snapshot.sandbox.noReload)} : {})}
    ...
    // director pass: an older snapshot has no pacer state and reads as closed.
    this.traderOpen = Boolean(snapshot.traderOpen)
    this.director = structuredClone(snapshot.director || {state: 'build_up', intensity: 0, reservoir: 0, reservoirMax: 0})
    this.extraction = snapshot.extraction ? structuredClone(snapshot.extraction) : null
    ...
    this.units = (snapshot.units || []).map((state) => {
      ...
      if (!unit.alive || predictOnly) return {...unit, brain: null}
      try {
        return {...unit, brain: this.resolveUnitBrain(unit)}
      } catch {
        return {...unit, brain: unit.enraged ? enragedBrain : DEFAULT_BRAINS[unit.type]}
      }
    })
  ...
  updateUnit(unit) {
    if (unit.brainType === 'dummy') return
    if (unit.wanderer && this.tick % WANDERER_SIGHT_INTERVAL_TICKS === 0 && this.unitSeenByAnyPlayer(unit)) {
      unit.lastSeenByPlayerTick = this.tick
    }
    const spec = this.unitCatalog.types[unit.type]
  ...
  moveUnit(unit, spec) {
    ...
    if (this.applyKnockback(unit)) return
    this.followPath(unit, spec)
    if (spec.role === 'common') this.separateFromCommons(unit)
  }

  // Advance an active push by one tick. It overrides the brain's move intent
  // while it runs. Returns whether a push consumed this tick.
  applyKnockback(unit) {
    const knockback = unit.knockback
    if (!knockback || knockback.ticksLeft <= 0) {
      unit.knockback = null
      return false
    }
    knockback.ticksLeft -= 1
    const previous = copyVec(unit.pos)
    this.slideUnit(unit, knockback.x, knockback.z)
    if (knockback.ticksLeft <= 0) unit.knockback = null
    unit.vel.x = (unit.pos.x - previous.x) / TICK_SECONDS
    unit.vel.y = (unit.pos.y - previous.y) / TICK_SECONDS
    unit.vel.z = (unit.pos.z - previous.z) / TICK_SECONDS
    this.telemetry.units[unit.id].distanceTraveled += distance(previous, unit.pos)
    return true
  }

  // Move a ground unit by dx and dz, one axis at a time. A blocked navigation
  // cell or missing footing rejects that axis, so a push stops at colliders.
  slideUnit(unit, dx, dz) {
    for (const [axis, delta] of [['x', dx], ['z', dz]]) {
      if (delta === 0) continue
      const candidate = copyVec(unit.pos)
      candidate[axis] += delta
      if (this.nav.isBlocked(this.nav.worldToCell(candidate))) continue
      const support = this.nav.supportAt(candidate, unit.pos.y)
      if (!support) continue
      unit.pos[axis] = candidate[axis]
      unit.pos.y = support.y
    }
  }

  // Commons push apart so a mob spreads into a crowd instead of a queue.
  separateFromCommons(unit) {
    let x = 0
    let z = 0
    for (const other of this.units) {
      if (other === unit || !other.alive || !this.isCommonUnit(other)) continue
      if (Math.abs(other.pos.y - unit.pos.y) > 1) continue
      const dx = unit.pos.x - other.pos.x
      const dz = unit.pos.z - other.pos.z
      const dist = Math.hypot(dx, dz)
      if (dist >= COMMON_SEPARATION_METERS) continue
      const strength = (COMMON_SEPARATION_METERS - dist) / COMMON_SEPARATION_METERS
      if (dist <= 1e-6) {
        // Exactly stacked. Split along x by id order so the result is stable.
        x += unit.id < other.id ? -strength : strength
        continue
      }
      x += dx / dist * strength
      z += dz / dist * strength
    ...
    const step = Math.min(length, 1) * COMMON_SEPARATION_SPEED * TICK_SECONDS
    this.slideUnit(unit, x / length * step, z / length * step)
  }

  // Look-ahead steers off the cell centre line, so only bodies that fit inside
  // the navigation grid's agent radius may use it. Wider bodies, such as the
  // Heavy, keep node-by-node following and their authored clearance.
  fitsNavClearance(spec) {
    return spec.radius <= (this.nav.spec.agentRadius ?? 0)
  }

  // Steer toward the path point UNIT_LOOK_AHEAD_METERS ahead instead of the next
  // node. A height change or a solid between the unit and the candidate ends the
  // look-ahead, so stairs, ramps, and turns keep their node-by-node footing.
  lookAheadPoint(unit, path, index) {
    let point = copyVec(path[index])
    let remaining = UNIT_LOOK_AHEAD_METERS - planarDistance(unit.pos, point)
    for (let next = index + 1; next < path.length && remaining > 0; next += 1) {
      if (Math.abs(path[next].y - point.y) > 1e-6) break
      const segment = planarDistance(point, path[next])
      if (segment <= 1e-9) continue
      const ratio = Math.min(1, remaining / segment)
      const candidate = {
        x: point.x + (path[next].x - point.x) * ratio,
        y: point.y,
        z: point.z + (path[next].z - point.z) * ratio,
      }
      if (!this.navSegmentOpen(unit.pos, candidate)) break
      if (ratio < 1) return candidate
      remaining -= segment
      point = candidate
    }
    return point
  ...
  // Every navigation cell on the straight line from `from` to `to` is open. The
  // grid already carries body clearance, so a cut corner never crosses a solid.
  // A diagonal cell change clips a corner, so both corner cells must be open too.
  navSegmentOpen(from, to) {
    const {origin, cellSize} = this.nav.spec
    const length = planarDistance(from, to)
    const steps = Math.max(1, Math.ceil(length / NAV_SAMPLE_METERS))
    const probe = {x: 0, z: 0, y: to.y}
    let previousX = Math.floor((from.x - origin.x) / cellSize)
    let previousZ = Math.floor((from.z - origin.z) / cellSize)
    for (let step = 1; step <= steps; step += 1) {
      const ratio = step / steps
      const cellX = Math.floor((from.x + (to.x - from.x) * ratio - origin.x) / cellSize)
      const cellZ = Math.floor((from.z + (to.z - from.z) * ratio - origin.z) / cellSize)
      if (cellX === previousX && cellZ === previousZ) continue
      probe.x = cellX
      probe.z = cellZ
      if (this.nav.isBlocked(probe)) return false
      if (cellX !== previousX && cellZ !== previousZ) {
        probe.x = previousX
        if (this.nav.isBlocked(probe)) return false
        probe.x = cellX
        probe.z = previousZ
  ...
  followPath(unit, spec) {
    ...
    const cache = unit.pathCache
    const lookAhead = this.fitsNavClearance(spec)
    // Accept a node when the unit is on it, or when look-ahead already carried
    // the unit past it and the next node is reachable in a straight line.
    // Without the second rule a cut corner never accepts and the unit walks back.
    while (cache.index < path.length - 1
      && (distance(unit.pos, path[cache.index]) < 0.45
        || (lookAhead && passedPathNode(unit.pos, path[cache.index], path[cache.index + 1])
          && this.navSegmentOpen(unit.pos, path[cache.index + 1])))) cache.index += 1
    const target = path[cache.index]
      ? (lookAhead ? this.lookAheadPoint(unit, path, cache.index) : path[cache.index])
      : unit.intent.moveTo
...
// True when pos already lies beyond node along the node-to-next segment.
function passedPathNode(pos, node, next) {
  if (!next) return false
  return (pos.x - node.x) * (next.x - node.x) + (pos.z - node.z) * (next.z - node.z) > 0
}
