...
export class World {
  ...
  movePlayer(player, inputs, {emitSound = true} = {}) {
    ...
    const canSprint = inputs.sprint && moving && !player.aiming && !player.crouch && player.sprintStamina > 0
    ...
    this.updatePlayerVertical(player, bodyHeight, grounded)
    if (moving) {
  ...
  updatePlayerVertical(player, bodyHeight, grounded) {
    if (grounded) {
      player.vel.y = 0
      return
    }
  ...
  predictPlayer(playerId, rawInputs = EMPTY_INPUTS) {
    ...
    if (player.alive) {
      player.aiming = inputs.aim && player.reloadTimer === 0
        && Boolean(this.weaponCatalog.weapons[player.activeWeapon]?.mag)
        && !inputs.melee && !inputs.grenade && player.meleeCooldown === 0 && player.grenadeCooldown === 0
      this.movePlayer(player, inputs, {emitSound: false})
    } else {
      player.moving = false
      player.sprinting = false
    }
  ...
  throwGrenade(playerId = this.hostPlayerId) {
    ...
    for (const unit of this.units) {
      ...
      hit = true
      this.damageUnit(unit.id, weapon.damage * (1 - dist / (weapon.radius * 1.5)), {source: 'player', playerId: player.id, weapon: 'grenade', point: center})
    }
    this.recordShot('grenade', hit, player.id)
    this.addSound('explosion', center, undefined, player.id)
    this.emit('explosion', {by: player.id, playerId: player.id, pos: center, hit})
    return true
  }
 ...
 * {move:{x,z}, yaw, pitch, fire, aim, reload, switchTo, sprint, crouch,
...
function looksLikeInputs(value) {
  return [
    ...
    'sprint', 'crouch', 'grenade', 'melee', 'ready',
