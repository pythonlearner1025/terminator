...
export class World {
  ...
  spawnUnit(typeId, pos, {yaw = 0, rev, id, brain} = {}) {
    ...
    try {
      unit.brain = unit.brainType === 'dummy' ? dummyBrain : this.createUnitBrain(typeId, unit.rev)
    } catch (error) {
  ...
  applySnapshot(snapshot, {predictOnly = false} = {}) {
    ...
    this.units = (snapshot.units || []).map((state) => {
      ...
      try {
        return {...unit, brain: unit.brainType === 'dummy' ? dummyBrain : this.createUnitBrain(unit.type, unit.rev)}
      } catch {
