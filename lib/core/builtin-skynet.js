...
export class BuiltinSkynet {
  ...
  plan({wave, budget, telemetry}) {
    ...
    const preferred = counterOrder(mostUsedWeapon(telemetry))
    const knobs = {
      gates,
      doors: {},
      lights: {},
    ...
    const counts = spendOnUnits({
      ...
      maxUnits: this.catalog.maxAlive,
    })
    const spawns = []
...
function spendOnUnits({budget, capBudget, preferred, catalog, minimumTypes, maxUnits}) {
  ...
  let spent = 0
  let total = 0
  const affordable = preferred.filter((id) => catalog.types[id].cost <= budget && catalog.types[id].cost <= perTypeLimit)
  ...
  while (progress && total < maxUnits) {
    ...
    for (const type of preferred) {
      const cost = catalog.types[type].cost
      if (spent + cost > budget || (counts[type] + 1) * cost > perTypeLimit + 1e-9) continue
      counts[type] += 1
      spent += cost
      total += 1
      progress = true
...
function counterOrder(weapon) {
  if (weapon === 'shotgun') return ['endo', 'heavy', 'scout']
