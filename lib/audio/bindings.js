...
export class AudioBindings {
  constructor({world = null, viewer = null, camera = null, engine = null, ownsEngine = engine == null, phaseProvider = null} = {}) {
    this.world = world
    this.viewer = viewer
  ...
  start(world = this.world) {
    ...
    const bus = world?.events || world?.eventBus || world?.bus
    this.unsubscribe = typeof bus?.subscribe === 'function'
      ? bus.subscribe((event) => this._receiveEvent(event))
      : null
    this.eventIndex = this.unsubscribe ? (world?.eventLog?.length || 0) : 0
    this.lastTransmission = null
    this.lastDryFireTick = -1000
    this.started = true
  ...
  sync(world = this.world) {
    ...
    if (!this.unsubscribe) {
      for (; this.eventIndex < world.eventLog.length; this.eventIndex += 1) this._processEvent(world.eventLog[this.eventIndex])
    }
