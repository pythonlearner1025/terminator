...
export class WebAudioEngine {
  constructor({camera = null, catalog = SOUND_CATALOG, contextFactory = null, logger = defaultLogger, volumes = null} = {}) {
    ...
    this.voices = new Map(Object.keys(this.catalog).map(name=>[name,[]]))
    this.loops = new Map()
  ...
  async _createAndResume() {
    ...
    for(const [name,definition] of Object.entries(this.catalog))for(const [index,variant] of definition.variants.entries()){
      const key=variant.file||`${name}:${index}`
      if(this.buffers.has(key))continue
      const recipe=variant.fallback||(!variant.file&&variant)
      if(recipe)this.buffers.set(key,renderSynthBuffer(context,recipe,hashString(key)))
    }
  ...
  _play(name, options = {}) {
    ...
    let voiceCount=0,oldest=null
    for(const voices of this.voices.values())for(const candidate of voices){
      voiceCount++
      if(!candidate.tag&&candidate.definition.bus==='effects'&&(!oldest||candidate.startedAt<oldest.startedAt))oldest=candidate
    }
    ...
    this.stats.peakVoices = Math.max(this.stats.peakVoices, voiceCount + 1)
