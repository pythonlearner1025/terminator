...
import {BOSS_MARKUP,BOSS_STYLE,renderBoss,waveBannerTitle} from './boss.js'
...
export class Hud {
  constructor(viewer) {
    ...
    this.style.textContent = STYLE+TERSE_STYLE+PRESENTATION_STYLE+HUD_SCALE_STYLE+BOSS_STYLE
  ...
  renderNameplates(items,heights = {}) {
    ...
    for(const item of items){
      ...
      plate.children[0].textContent=teammate?(item.label || item.name):String(item.label || item.name).replace(/^T-\d+\s+(?=\S)/,'')
