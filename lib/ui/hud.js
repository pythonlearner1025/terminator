...
import {traderOpenInView,DIRECTOR_STATE_WORDS,MOB_BANNER_SECONDS} from './presentation.js'
...
<div class="tm-panel tm-skynet" data-role="skynetPanel" data-testid="skynet-panel"><div class="tm-skynet-title"><span class="tm-dot" data-role="dot"></span>SKYNET · <span data-role="skynet"></span> · BUDGET <span data-role="budget"></span></div><div class="tm-skynet-row tm-reservoir" data-role="reservoirRow" data-testid="skynet-reservoir" hidden><span data-role="reservoir"></span><b class="tm-director-state" data-role="directorState" hidden></b></div><div class="tm-skynet-row tm-revs"><span data-role="revs"></span></div><div class="tm-transmission"><span class="tm-transmission-text" data-role="transmission"></span></div></div>
...
<div class="tm-caches" data-role="caches" data-testid="cache-markers"></div>
<div class="tm-extraction" data-role="extraction" data-testid="extraction-marker" hidden>${icon('core')}<small data-role="extractionTimer"></small></div>
...
export class Hud {
  ...
  sync() {
    if (!this.root) return
    const rect = canvasRect(this.viewer.canvas)
  ...
  render(view, extra = {}) {
    ...
    const traderOpen = traderOpenInView(view)
    ...
    // While the pacer still holds budget, the count is only what is alive now.
    const countLine = extra.remainingLabel ?? `${extra.remaining ?? view.wave.remaining} LEFT`
    this.text('waveSub', `${difficulty} · ${intermission ? `${Math.ceil(view.wave.timer)}s` : countLine}`)
    el.wavePanel.classList.toggle('trader',traderOpen)
    el.ready.hidden = !traderOpen
    ...
    // The reservoir drains through the wave. Show what the pacer still holds.
    // It sits on its own row: the title line has no room for it.
    const director = extra.director ?? view.director ?? null
    el.reservoirRow.hidden = !director
    this.text('reservoir',director ? `LEFT ${Math.max(0,Math.round(director.reservoir ?? 0))}` : '')
    const stateWord = director ? director.word ?? DIRECTOR_STATE_WORDS[director.state] ?? '' : ''
    el.directorState.hidden = !stateWord
    this.text('directorState',stateWord)
    ...
    const mob = extra.mobIncoming
    if (mob && mob.id !== this.lastMobId) {
      this.lastMobId = mob.id
      this.banner(`MOB INCOMING${mob.behind?' · BEHIND':''}`,'',MOB_BANNER_SECONDS*1000)
    }
    ...
    this.renderTrader(extra.trader,traderOpen)
    this.renderCaches(extra.caches)
    this.renderExtraction(view.extraction)
  }
  ...
  banner(title, subtitle, durationMs = 3000) {
    ...
    this.bannerUntil=(this.presentationTimeMs ?? performance.now())+durationMs
  }
  renderFeed(items) {
  ...
  renderCenterFeedback(view) {
    ...
    if(latest && latest.id!==this.lastHitId && !this.bullets?.handlesMarker(latest.id)){
  ...
  renderCaches(items=[]) {
    const active=new Set(items.map(item=>item.id))
    for(const [id,marker] of this.cacheMarkers) if(!active.has(id)){marker.remove();this.cacheMarkers.delete(id)}
    for(const item of items){
      let marker=this.cacheMarkers.get(item.id)
      if(!marker){marker=document.createElement('div');marker.className='tm-cache-marker';marker.dataset.cacheId=item.id;marker.innerHTML=`${icon('crate')}<small></small>`;this.cacheMarkers.set(item.id,marker);this.elements.caches.append(marker)}
      const point=this.project(item.pos,1.1)
      marker.hidden=!point.visible
      if(marker.hidden)continue
      marker.style.left=`${point.x}%`;marker.style.top=`${point.y}%`
      const label=`${Math.round(item.distance)}m`
      if(marker._label!==label){marker._label=label;marker.lastElementChild.textContent=label}
    }
  }
  renderExtraction(extraction) {
    const el=this.elements.extraction
    el.hidden=!extraction?.pos
    if(el.hidden)return
    const point=this.project(extraction.pos,1.7)
    el.style.left=`${Math.max(10,Math.min(90,point.z>=1?50:point.x))}%`
    el.style.top=`${Math.max(29,Math.min(72,point.z>=1?68:point.y))}%`
    const timer=Number(extraction.timer)
    this.text('extractionTimer',Number.isFinite(timer)?`${Math.max(0,Math.ceil(timer))}s`:String(extraction.phase || '').toUpperCase())
  }
  dispose() {
    sfx.stopSfx?.()
    this.root?.remove();this.style?.remove();this.root=null;this.style=null;this.elements={}
    this.plates.clear();this.feed.clear();this.arcs.clear();this.teammates.clear();this.cacheMarkers.clear()
  }
}
