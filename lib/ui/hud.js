...
import {STYLE,TERSE_STYLE} from './styles.js'
...
<div class="tm-panel tm-skynet" data-role="skynetPanel" data-testid="skynet-panel"><div class="tm-skynet-title"><span class="tm-dot" data-role="dot"></span>SKYNET · <span data-role="skynet"></span> · <span data-role="budget"></span></div><div class="tm-skynet-row tm-revs" data-role="revs"></div><div class="tm-transmission"><span class="tm-transmission-text" data-role="transmission"></span></div></div>
...
<div class="tm-ready" data-role="ready" data-testid="intermission" hidden><button data-action="ready" aria-label="Ready">${icon('check')}</button><button data-action="trader" aria-label="Trader">${icon('crate')}</button></div>
<div class="tm-banner" data-role="banner" hidden><b data-role="bannerTitle"></b><small data-role="bannerSub"></small></div>
<div class="tm-controls"><button data-action="pause" aria-label="Pause">${icon('pause')}</button></div>
...
export class Hud {
  constructor(viewer) {
    ...
    this.style.textContent = STYLE+TERSE_STYLE
    ...
    spectate.innerHTML=`<strong data-spectate="name"></strong><div class="tm-spectate-controls"><button data-action="spectate-previous" aria-label="Previous player">${icon('back')}</button><small data-spectate="count"></small><button data-action="spectate-next" aria-label="Next player">${icon('next')}</button></div>`
  ...
  render(view, extra = {}) {
    ...
    this.text('waveSub', intermission ? `${Math.ceil(view.wave.timer)}s` : extra.remaining ?? view.wave.remaining)
    ...
    this.text('revs',`E${view.skynet.revs.endo} S${view.skynet.revs.scout} H${view.skynet.revs.heavy}`)
    ...
    if (view.wave.phase !== this.lastPhase || view.wave.current !== this.lastWave) {
      if (view.wave.phase === 'wave' && extra.matchStarted) {
        animate(el.wavePanel,'slam');this.banner(`WAVE ${view.wave.current}`, '');sfx.waveKlaxon(view.wave.current)
  ...
  renderTeammates(items) {
    ...
    for(const item of items){
      ...
      row.lastElementChild.firstElementChild.innerHTML=icon('health')+(down?'0':Math.ceil(health))
      const armorNode=row.lastElementChild.lastElementChild
      armorNode.innerHTML=icon('armor')+`<span class="tm-armor-meter"><i style="width:${Math.max(0,Math.min(100,armor))}%"></i></span>`
  ...
  renderFeed(items) {
    ...
    for (const item of [...items].reverse()) {
      ...
      if (!row) {row=document.createElement('div');row.className='tm-feed-row';row.setAttribute('aria-label',item.text);const weapon=/PLASMA/i.test(item.text)?'plasma':/SHOTGUN/i.test(item.text)?'shotgun':/M4/i.test(item.text)?'m4':'pistol';const type=/SCOUT/i.test(item.text)?'scout':/HEAVY/i.test(item.text)?'heavy':'endo';row.innerHTML=icon(weapon)+(item.text.includes('HEADSHOT')?icon('aim'):icon('skull'))+icon(type);this.feed.set(item.id,row);this.elements.feed.prepend(row)}
  ...
  renderNameplates(items,heights = {}) {
    ...
    for(const item of items){
      ...
      const teammate=item.kind==='teammate' || item.teammate===true || item.type==='teammate'
      const down=teammate && (item.downed || item.alive===false)
      plate.className=`tm-nameplate${teammate?' teammate':''}${down?' downed':''}`
      const point=this.project(item.pos,heights[item.id] ?? (teammate?(down?.65:2.05):2.5))
      plate.hidden=!point.visible || (!teammate && item.distance>25) || (this.elements.spectate.hidden===false && item.id===this.spectatingId)
      plate.style.left=`${point.x}%`;plate.style.top=`${point.y}%`
      plate.children[0].textContent=teammate?(item.label || item.name):String(item.label || item.name).replace(/^T-\d+\s*/,'')
      plate.children[1].textContent=teammate?(down?'✚':''):`r${item.rev}`
