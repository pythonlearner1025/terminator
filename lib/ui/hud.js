...
<section class="tm-teammates" data-role="teammates" data-testid="teammates" aria-label="Teammates" hidden><div class="tm-eyebrow">RESISTANCE SQUAD</div><div data-role="teammateRows"></div></section>
...
<div data-role="centerFeedback" style="position:absolute;left:50%;top:50%;width:0;height:0"><div class="tm-hit" data-role="hit"></div><svg class="tm-reload" data-role="reload" data-testid="reload-ring" viewBox="0 0 70 70" hidden><circle class="track" cx="35" cy="35" r="30"/><circle data-role="reloadProgress" cx="35" cy="35" r="30"/></svg></div><div class="tm-reload-label" data-role="reloadLabel" hidden>RELOADING</div>
...
export class Hud {
  constructor(viewer) {
    ...
    scaling.className='tm-wave-scaling';scaling.dataset.testid='wave-scaling';scaling.hidden=true
    this.elements.wavePanel.append(scaling);this.elements.scaling=scaling
    ...
    spectate.innerHTML='<div class="tm-eyebrow">FIGHTER DOWN / SQUAD CAMERA</div><strong data-spectate="name"></strong><p>You respawn at the start of the next wave.</p><div class="tm-spectate-controls"><button data-action="spectate-previous">← PREVIOUS / RMB</button><small data-spectate="count"></small><button data-action="spectate-next">NEXT / LMB →</button></div>'
  ...
  render(view, extra = {}) {
    ...
    el.scaling.hidden=!view.scaling || view.scaling.players<2
    this.root.classList.toggle('is-coop',(view.scaling?.players || 1)>1)
    if(view.scaling)this.text('scaling',`${view.scaling.players} PLAYERS x${Number(view.scaling.budgetMultiplier).toFixed(1)}`)
  ...
  renderTeammates(items) {
    ...
    for(const item of items){
      ...
      row.querySelector('small').textContent=`${Math.round(item.distance)} m`
      ...
      row.lastElementChild.firstElementChild.textContent=down?'✚ DOWN':`HP ${Math.ceil(health)}`
      row.lastElementChild.lastElementChild.textContent=`ARMOR ${Math.ceil(armor)}`
    }
  }
  renderSpectate(target) {
    ...
    if(!target)return
    this.elements.spectate.querySelector('[data-spectate="name"]').textContent=`SPECTATING ${target.name}`
  ...
  renderNameplates(items,heights = {}) {
    ...
    for(const item of items){
      ...
      plate.children[1].textContent=teammate?`${down?'✚ DOWN':'RESISTANCE'} / ${Math.round(item.distance)} m`:`rev ${item.rev} / ${Math.round(item.distance)} m`
