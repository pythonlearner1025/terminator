...
export function playLanding(fallHeight = 0) {
  const height = Math.max(0, Number(fallHeight) || 0)
  return play('footstep_concrete', {gain: Math.min(1, 0.34 + height * 0.12), pitch: Math.max(0.82, 1.02 - height * 0.025)})
}

export function grenadeThrown(position) {
  if (autoBindings) return null
  return play('grenade_throw', position ? {position, gain: 0.9} : {gain: 0.9})
}

export function grenadeBounce(position, speed = 1) {
  if (autoBindings) return null
  return play('sparks_metal', {
    position,
    gain: Math.max(0.12, Math.min(0.5, Number(speed) / 18)),
    pitch: 0.58,
  })
}

export function grenadeExplosion(position) {
  if (autoBindings) return null
  return play('grenade_explosion', position ? {position} : undefined)
}
...
export async function prepareAudio() {
  ensureAudio()
  for(let attempts=0;attempts<120&&!audioEngine;attempts++)await new Promise(resolve=>requestAnimationFrame(resolve))
  if(!audioEngine)return {ready:false,decoded:0,failed:0}
  await audioEngine.unlock()
  for(let attempts=0;attempts<1200&&audioEngine?.loading;attempts++)await new Promise(resolve=>requestAnimationFrame(resolve))
  return {ready:audioEngine.context?.state==='running'&&!audioEngine.loading,
    decoded:audioEngine.stats.loaded,failed:audioEngine.stats.failed.length}
}
