...
export function parsePartyInvite(value, relay = '') {
  ...
  if (code.includes('?') || /^https?:/i.test(code)) {
    ...
    signal = invite.searchParams.get('signal') || undefined
  }
  ...
  if (signal) {
    let url
    try { url = new URL(signal) } catch { throw partyError('bad_signal') }
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password) throw partyError('bad_signal')
    signal = url.href.replace(/\/$/, '')
  }
  return {code, relay: relay || undefined, signal}
}
...
const ERRORS = {
  ...
  bad_signal: 'Invalid signaling address. Ask the host for a new invite link.',
  ...
  sandbox: 'Sandbox is single-player only.',
}
...
export function partyErrorMessage(error) {
  if(Object.values(ERRORS).includes(error))return error
  if (String(error?.message || error) === 'Could not connect to the host. Both players may be behind strict NAT.') return String(error.message || error)
  const code = String(error?.code || error?.error?.code || error?.message || error?.notice || error || '').toLowerCase().replaceAll('-', '_')
  if (ERRORS[code]) return ERRORS[code]
  if (/full|capacity/.test(code)) return ERRORS.party_full
  if (/host.*(left|disconnect|closed|lost)/.test(code)) return ERRORS.host_left
...
export class PartySession {
  constructor(manager, changed) {
    ...
    this.receive = state => {
      ...
      if(this.state.error)this.state.error=partyErrorMessage(this.state.error)
  ...
  async connect(role, options) {
    ...
    try {
      ...
      const pending = this.manager[method](options)
      // The browser's fetch requires Window as its receiver. The host stores
      // that function on its transport instance; bind this instance before its
      // asynchronous invite lookup starts. Injected fetch implementations stay intact.
      const transport = this.manager.party
      if (role === 'host' && transport?.fetch === globalThis.fetch) transport.fetch = globalThis.fetch.bind(globalThis)
  ...
  async ready(value) {
    ...
    const result=await this.manager.setPartyReady(value)
    if(result?.ok===false)throw result.error || result
    return result
  }
