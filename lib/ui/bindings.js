...
export const ACTIONS = Object.freeze([
  ...
  ['slot4', 'Plasma', 'Digit4', 'combat'], ['lookLeft', 'Look left', 'ArrowLeft', 'combat'],
...
export class BindingInput {
  ...
  sample() {
    ...
    const slot = [1,2,3,4].find(index => this.pulses.has(`slot${index}`))
    const result = {...base, yaw:input.yaw, pitch:input.pitch,
      ...
      reload:this.pulses.has('reload'), grenade:this.pulses.has('grenade'), melee:this.pulses.has('melee'),
      ready:false, switchTo:slot ?? null,
    }
    this.pulses.clear()
    return result
  }
