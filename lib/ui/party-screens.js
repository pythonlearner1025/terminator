...
export class PartyScreens {
  ...
  refresh(state = this.state) {
    ...
    this.live('inviteNote',state?.inviteUrls?.tunnel?'PUBLIC':invite?.value?'LOCAL NETWORK':'')
    this.live('rosterNote',(players?.length || 0)<2?'WAITING':'')
  }
  async action(action) {
