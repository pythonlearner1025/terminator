...
export class PartyScreens {
  ...
  show(route) {
    ...
    this.screens.root.innerHTML = `<header class="tm-screen-header">${iconButton('leave-party','Main menu','back')}</header>
      ...
      ${host ? `<div class="tm-eyebrow">PARTY CODE</div><div class="tm-code" data-party="code"></div><div class="tm-eyebrow">INVITE LINK</div><div class="tm-invite-row"><div><input readonly aria-label="Invite link" data-party="invite"><button type="button" class="tm-btn ghost" data-action="copy-party-invite" data-testid="copy-party-invite">COPY</button></div></div><p class="tm-muted" data-party="inviteNote"></p>` : `<form data-party-form="join-party-submit">${field('name','YOUR NAME',this.name,'Name','autocomplete="off" spellcheck="false" required')}${field('invite','PARTY CODE OR INVITE LINK',this.invite,'ABC123 or https://...','autocomplete="off" spellcheck="false" required aria-describedby="party-error"')}${field('relay','RELAY ADDRESS (only for a code without a link)',this.relay,'wss://...','autocomplete="off" spellcheck="false" aria-describedby="party-error"')}${button('join-party-submit','JOIN','primary')}</form>`}
      ...
      ${host ? `<div class="tm-screen-footer"><label class="tm-party-override"><input type="checkbox" data-party-field="override" aria-label="Start even if someone is not ready"> START EVEN IF NOT EVERYONE IS READY</label>${button('party-start','START','primary')}</div>` : ''}`
    this.refresh()
  ...
  refresh(state = this.state) {
    ...
    this.live('inviteNote',state?.inviteUrls?.tunnel?'Send this link to your friends.':invite?.value?'This link works on your local network. For friends elsewhere run: npm run tunnel':'')
