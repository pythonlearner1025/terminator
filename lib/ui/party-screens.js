...
export class PartyScreens {
  ...
  show(route) {
    ...
    this.screens.root.innerHTML = `<header class="tm-screen-header">${iconButton('leave-party','Main menu','back')}</header>
      ...
      ${host ? `<form data-party-form="create-party">${field('name','YOUR NAME',this.name,'Name','autocomplete="off" spellcheck="false" required')}${field('relay','RELAY ADDRESS (optional on this Mac)',this.relay,'wss://...trycloudflare.com','autocomplete="off" spellcheck="false" aria-describedby="party-error"')}${button('create-party','CREATE','primary')}</form><div class="tm-eyebrow">PARTY CODE</div><div class="tm-code" data-party="code"></div><div class="tm-eyebrow">INVITE LINK</div><div class="tm-invite-row"><div><input readonly aria-label="Invite link" data-party="invite"><button type="button" class="tm-btn ghost" data-action="copy-party-invite" data-testid="copy-party-invite">COPY</button></div></div><p class="tm-muted" data-party="inviteNote"></p>${this.screens.difficultyControl()}` : `<form data-party-form="join-party-submit">${field('name','YOUR NAME',this.name,'Name','autocomplete="off" spellcheck="false" required')}${field('invite','PARTY CODE OR INVITE LINK',this.invite,'ABC123 or https://...','autocomplete="off" spellcheck="false" required aria-describedby="party-error"')}${field('relay','RELAY',this.relay,'wss://...','autocomplete="off" spellcheck="false" aria-describedby="party-error"')}${button('join-party-submit','JOIN','primary')}</form>`}
  ...
  refresh(state = this.state) {
    ...
    this.live('inviteNote',state?.inviteUrls?.tunnel?'Send this link to your friends.':invite?.value?'This link works on your local network only. For friends elsewhere run npm run tunnel on the host Mac and open the link it prints.':'')
    ...
    try {
      ...
      if (action.startsWith('copy-party-')) {
        ...
        if (action === 'create-party') {
          ...
          if (!this.relay && !localPage) throw new Error('This page cannot reach a relay on your Mac by itself. On the host Mac run npm run server and npm run tunnel, then open the link the tunnel prints, or paste its wss address above.')
          await this.screens.actions.partyHost({name, relay: this.relay || undefined})
        }
        else {
