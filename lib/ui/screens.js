...
export class Screens {
  ...
  main(){this.root.innerHTML=`<div class="tm-brand"><h1>TERMINATOR</h1><div class="tm-brand-subtitle">HUMAN VS SKYNET</div><nav class="tm-menu-nav" aria-label="Main menu">${btn('play','PLAY','primary')}${btn('host-party','HOST')}${btn('join-party','JOIN')}${btn('connect-agent','SKYNET')}${btn('settings','SETTINGS')}${btn('quit','QUIT')}</nav></div>`}
  ...
  lobby(){this.root.innerHTML=`${header('SKYNET','main-menu')}<div class="tm-lobby-grid"><div><div class="tm-eyebrow">LOBBY CODE</div><h2 class="tm-code" data-live="code"></h2><p class="tm-status" role="status"><span data-live="agentIcon"></span> <span data-live="agent"></span></p><div class="tm-eyebrow">INSTALL</div><div class="tm-install"><code class="tm-mono" data-live="install"></code><button class="tm-btn ghost" data-action="copy-mcp" data-testid="copy-mcp">COPY</button></div>${this.difficultyControl()}</div></div><div class="tm-screen-footer">${btn('start-match','START','primary')}</div>`;this.refreshLobby()}
  refreshLobby(){
  ...
  controlsCard(){this.root.innerHTML=`<div class="tm-controls-card">${RESISTANCE_EMBLEM}<h1>CONTROLS</h1><div class="tm-controls-grid">${[['forward','Move'],['fire','Fire'],['aim','Aim'],['reload','Reload'],['sprint','Sprint'],['crouch','Crouch'],['grenade','Grenade'],['melee','Knife'],['pause','Pause']].map(([id,label])=>`<div><kbd>${id==='forward'?['forward','left','backward','right'].map(key=>esc(keyLabel(this.settings.bindings[key]))).join(' '):esc(keyLabel(this.settings.bindings[id]))}</kbd><span>${label}</span></div>`).join('')}</div>${btn('dismiss-controls','CONTINUE')}<small class="tm-eyebrow">ANY KEY</small></div>`}
  ...
  quit(){this.root.innerHTML=`<div class="tm-pause-card" style="margin:auto"><h1>QUIT</h1>${btn('main-menu','QUIT','danger')}${btn('back','BACK')}</div>`}
