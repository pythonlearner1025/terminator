...
export class Screens {
  ...
  main(){this.root.innerHTML=`<div class="tm-brand"><h1>TERMINATOR</h1><div class="tm-brand-subtitle">HUMAN VS SKYNET</div><nav class="tm-menu-nav" aria-label="Main menu">${btn('play','PLAY','primary')}${btn('host-party','HOST')}${btn('join-party','JOIN')}${btn('connect-agent','SKYNET')}${btn('settings','SETTINGS')}${btn('quit','QUIT')}</nav></div><div class="tm-menu-art">${SKULL}</div>`}
  lobby(){this.root.innerHTML=`${header('','main-menu')}<div class="tm-lobby-grid"><div><h2 class="tm-code" data-live="code"></h2><p class="tm-status" data-live="agent" role="status"></p><div class="tm-install"><code class="tm-mono" data-live="install"></code>${iconButton('copy-mcp','Copy install command','copy')}</div></div></div><div class="tm-screen-footer">${btn('start-match','START','primary')}</div>`;this.refreshLobby()}
  refreshLobby(){
    const state=this.lobbyState || {}
    this.live('code',state.code || '------')
    const status=this.root.querySelector('[data-live="agent"]')
    status.innerHTML=icon(state.error?'pause':state.agentName?'check':state.code?'skull':'pause')
    status.setAttribute('aria-label',state.agentName || (state.error?'Offline':state.code?'Built-in':'Connecting'))
  ...
  pause(){this.root.innerHTML=`<div class="tm-pause-card" aria-label="Paused">${icon('pause')}${btn('resume','RESUME','primary')}<div class="tm-pause-utilities">${iconButton('settings','Settings','settings')}${iconButton('main-menu','Main menu','home','danger')}</div></div>`}
  settingsScreen(){this.root.innerHTML=`${header()}<div class="tm-settings-grid"><div>${[['sensitivity','Sensitivity',.25,3,.05],['fov','FOV',60,110,1],['master','Master',0,100,1],['music','Music',0,100,1],['effects','Effects',0,100,1]].map(([key,label,min,max,step])=>`<label class="tm-setting"><span>${label}</span><input aria-label="${label}" data-setting="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${this.settings[key]}"><output data-output="${key}">${this.formatSetting(key,this.settings[key])}</output></label>`).join('')}<label class="tm-setting"><span>Quality</span><select data-setting="quality" aria-label="Quality">${['low','medium','high'].map(q=>`<option value="${q}" ${q===this.settings.quality?'selected':''}>${q.toUpperCase()}</option>`).join('')}</select></label></div></div>`}
  formatSetting(key,value){return key==='sensitivity'?`${Number(value).toFixed(2)}x`:key==='fov'?`${value}°`:`${value}%`}
  trader(){this.root.innerHTML=`${header()}<div class="tm-trader-grid"><nav class="tm-categories" aria-label="Trader categories">${[['weapons','m4'],['ammo','ammo'],['armor','armor'],['items','crate']].map(([category,i])=>`<button data-action="category:${category}" class="${this.category===category?'selected':''}">${icon(i)}${category.toUpperCase()}</button>`).join('')}</nav><div class="tm-items" data-live="items"></div><aside class="tm-loadout"><div class="tm-balance" aria-label="Scrap">${icon('scrap')}<span data-live="balance"></span></div><button class="tm-btn" data-action="purchase:fill-ammo" aria-label="Fill all ammo">${icon('ammo')}<span data-live="fillPrice"></span></button><button class="tm-btn" data-action="purchase:full-armor" aria-label="Buy full armor">${icon('armor')}<span data-live="armorPrice"></span></button><b class="tm-time" data-live="time" aria-label="Time remaining"></b></aside></div>`;this.traderKey='';this.refreshTrader()}
  refreshTrader(){
  ...
  postmatch(){
    ...
    this.root.innerHTML=`${header(s.survived?'SURVIVED':'<span class="tm-red">TERMINATED</span>','main-menu')}<div class="tm-result-wave" aria-label="Wave reached">${s.wave}/10</div><div class="tm-match-stats">${[['skull','Kills',Object.values(s.kills).reduce((a,b)=>a+b,0)],['aim','Accuracy',`${s.accuracy}%`],['health','Damage taken',s.damage],['scrap','Scrap',s.scrap.toLocaleString()]].map(([glyph,label,value])=>`<div class="tm-match-stat" aria-label="${label}">${icon(glyph)}<b>${value}</b></div>`).join('')}</div><div class="tm-screen-footer">${btn('play-again','PLAY AGAIN','primary')}</div>`
  }
  scoreboard(){
    const stats=this.extra.stats
    const names=playerLabels(this.extra.scoreboard)
    this.root.innerHTML=`${header(stats.survived?'SURVIVED':'<span class="tm-red">TERMINATED</span>','main-menu')}<div class="tm-result-wave" aria-label="Wave reached">${stats.wave}/10</div><div class="tm-scoreboard-wrap"><table class="tm-scoreboard" data-testid="coop-scoreboard" aria-label="${this.extra.scoreboardPartial?'Partial match results':'Match results'}"><thead><tr><th scope="col" aria-label="Player"></th>${[['skull','Kills'],['aim','Accuracy'],['health','Damage taken'],['scrap','Scrap']].map(([glyph,label])=>`<th scope="col" aria-label="${label}">${icon(glyph)}</th>`).join('')}</tr></thead><tbody>${this.extra.scoreboard.map((player,index)=>`<tr class="${player.id===this.extra.localPlayerId?'local':''}" aria-label="${esc(player.name)}${player.connected?'':' disconnected'}"><td class="tm-score-name" aria-label="${esc(player.name)}">${esc(names[index])}</td><td>${player.kills}</td><td>${player.accuracy}%</td><td>${player.damageTaken==null?'?':player.damageTaken.toLocaleString()}</td><td>${player.scrap==null?'?':player.scrap.toLocaleString()}</td></tr>`).join('')}</tbody></table></div><div class="tm-screen-footer">${btn('play-again','PLAY AGAIN','primary')}</div>`
  }
  quit(){this.root.innerHTML=`<div class="tm-pause-card" style="margin:auto"><h1>QUIT</h1>${btn('main-menu','MENU','primary')}</div>`}
