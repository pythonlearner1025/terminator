...
export class Screens {
  ...
  main(){this.root.innerHTML=`<div class="tm-brand"><div class="tm-eyebrow">LOS ANGELES / 2029</div><h1>TERMINATOR</h1><div class="tm-brand-subtitle">HUMAN VS SKYNET</div><p class="tm-menu-description">The machines learned to hunt.<br>Give them something to fear.</p><nav class="tm-menu-nav" aria-label="Main menu">${btn('play','PLAY <small>01</small>','primary')}${btn('host-party','HOST PARTY <small>02</small>')}${btn('join-party','JOIN PARTY <small>03</small>')}${btn('connect-agent','CONNECT AGENT <small>04</small>')}${btn('dossier','DOSSIER <small>05</small>')}${btn('settings','SETTINGS <small>06</small>')}${btn('quit','QUIT <small>07</small>')}</nav></div><div class="tm-menu-art">${SKULL}</div><div class="tm-art-caption tm-mono">CYBERDYNE SYSTEMS<br>MODEL 101 // T-800<br><span class="tm-red">NEURAL NET PROCESSOR: ACTIVE</span></div><footer class="tm-menu-bottom"><span>RESISTANCE COMMAND // BUNKER 7</span><span>BUILD 0.15.0 / HUMANITY IS NOT OBSOLETE</span></footer>`}
  ...
  refreshLobby(){const state=this.lobbyState || {};this.live('code',state.code || '------');this.live('agent',state.agentName?`${state.agentName} connected`:'Waiting for Skynet');this.live('connection',state.status || 'Opening uplink...');if(state.agentName && state.agentName!==this.lastLobbyAgent){this.lastLobbyAgent=state.agentName;animate(this.root.querySelector('[data-live="agent"]'),'tm-power');sfx.skynetPowerOn()}this.live('install',state.installLine || 'Waiting for lobby code');this.live('fallback',state.agentName?'External Skynet agent will play':'Built-in Skynet will play');this.live('lobbyNotice',state.error || 'Start when ready. An external agent can join this lobby.');const copy=this.root.querySelector('[data-action="copy-mcp"]');if(copy)copy.disabled=!state.code}
  ...
  refreshTrader(){
    ...
    if(this.category==='ammo')items=extra.loadout.map(w=>({...w,id:'ammo',owned:false,action:`ammo:${w.id}`,name:`${w.name} ammo`,price:w.ammoPrice,description:`${extra.catalog[w.id].mag} ROUNDS / MAGAZINE`,slot:null}))
  ...
  postmatch(){if(this.extra.scoreboard?.length>1){this.scoreboard();return}const s=this.extra.stats;this.root.innerHTML=`${header('RESISTANCE / AFTER ACTION REPORT',s.survived?'SURVIVED':'<span class="tm-red">TERMINATED</span>','main-menu','Main menu')}<div class="tm-post-grid"><div class="tm-match-stats">${[['WAVE REACHED',`${s.wave} / 10`],['ACCURACY',`${s.accuracy}%`],['DAMAGE TAKEN',s.damage],['SCRAP EARNED',s.scrap.toLocaleString()],['TIME IN COMBAT',clock(s.seconds)],['TOTAL KILLS',Object.values(s.kills).reduce((a,b)=>a+b,0)]].map(([label,value])=>`<div class="tm-match-stat"><small>${label}</small><b>${value}</b></div>`).join('')}<div class="tm-kills">${Object.entries(s.kills).map(([type,n])=>`<div><span class="tm-eyebrow">${type}</span><b>${String(n).padStart(2,'0')}</b></div>`).join('')}</div></div>${this.dossierMarkup()}</div><footer class="tm-screen-footer"><span class="tm-mono">THE MACHINES ARE LEARNING.<br>SO ARE YOU.</span>${btn('play-again','PLAY AGAIN','primary')}</footer>`;this.beginDossier()}
  scoreboard(){
    const stats=this.extra.stats
    this.root.innerHTML=`${header('RESISTANCE / SQUAD AFTER ACTION REPORT',stats.survived?'SURVIVED':'<span class="tm-red">TERMINATED</span>','main-menu','Main menu')}<div class="tm-scoreboard-wrap"><div class="tm-scoreboard-intro"><span>WAVE ${stats.wave} / 10</span><span>${clock(stats.seconds)} IN COMBAT</span><span>${this.extra.scoreboard.length} FIGHTERS</span></div><table class="tm-scoreboard" data-testid="coop-scoreboard"><thead><tr><th scope="col">FIGHTER</th><th scope="col">KILLS</th><th scope="col">DAMAGE</th><th scope="col">ACCURACY</th><th scope="col">SCRAP</th></tr></thead><tbody>${this.extra.scoreboard.map(player=>`<tr class="${player.id===this.extra.localPlayerId?'local':''}"><td class="tm-score-name">${esc(player.name)}<small>${player.id===this.extra.localPlayerId?'YOU / ':''}${player.connected?'RESISTANCE':'DISCONNECTED'}</small></td><td>${player.kills}</td><td>${player.damage.toLocaleString()}</td><td>${player.accuracy}%</td><td>${player.scrap==null?'N/A':player.scrap.toLocaleString()}</td></tr>`).join('')}</tbody></table><p class="tm-scoreboard-note">${this.extra.scoreboardPartial?'Partial history: totals cover events received by this client.':'Match totals across all waves.'} Damage dealt to machines. Scrap shows final balance.</p></div><footer class="tm-screen-footer"><span class="tm-mono">THE MACHINES ARE LEARNING.<br>READ WHAT THEY LEARNED ABOUT YOU.</span>${btn('postmatch-dossier','CONTINUE TO DOSSIER','primary')}</footer>`
  }
  ...
  dossierScreen(){this.root.innerHTML=`${header('INTERCEPTED INTELLIGENCE','THE DOSSIER')}<div class="tm-dossier-only">${this.dossierMarkup()}</div><footer class="tm-screen-footer"><span class="tm-mono">CLASSIFIED / RESISTANCE EYES ONLY</span>${btn(this.dossierReturn?'play-again':'back',this.dossierReturn?'PLAY AGAIN':'BACK','primary')}</footer>`;this.beginDossier()}
  ...
  beginDossier(){if(!this.root.querySelector('[data-live="traits"]'))return;this.dossierStart=performance.now();this.dossierChars=-1;this.root.querySelector('[data-live="traits"]').replaceChildren();this.typeDossier()}
  typeDossier(){
  ...
  async action(action,button){
    ...
    if(action==='back'){if(['party-host','party-join'].includes(this.route)){this.actions.partyLeave();this.party.state=null;this.show('main')}else if(this.route==='dossier' && this.dossierReturn){this.show(this.dossierReturn);this.dossierReturn=null}else if(this.route==='settings')this.show(this.settingsReturn || 'main');else if(['pause','trader'].includes(this.route))this.show(null);else this.show('main');return}
    ...
    if(action==='postmatch-dossier'){this.dossierReturn='postmatch';this.show('dossier');return}
    if(action==='dossier'){this.dossierReturn=null;this.show('dossier');this.actions.dossier();return}
