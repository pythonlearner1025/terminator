...
import {loadSettings,saveSettings,QUALITY_IDS} from './settings.js'
import {traderOpenInView} from './presentation.js'
...
export class Screens {
  constructor(hud,actions){
    ...
    this.onInput=e=>{const key=e.target.dataset.setting;if(!key)return;this.settings[key]=e.target.type==='checkbox'?e.target.checked:['quality','difficulty'].includes(key)?e.target.value:Number(e.target.value);const output=this.root.querySelector(`[data-output="${key}"]`);if(output)output.textContent=this.formatSetting(key,this.settings[key]);this.persistSettings()}
    this.onMouse=e=>{
  ...
  render(view,extra,lobby){
    ...
    if(view.wave.phase==='lobby')this.ended=false
    ...
    if(this.route==='trader' && !traderOpenInView(view) && !this.actions.sandbox?.()){this.show(null)}
  ...
  settingsScreen(){this.root.innerHTML=`${header('SETTINGS')}<div class="tm-settings-grid"><div>${[['sensitivity','Sensitivity',.25,3,.05],['fov','FOV',60,110,1],['hud','HUD size',50,100,2],['master','Master',0,100,1],['music','Music',0,100,1],['effects','Effects',0,100,1]].map(([key,label,min,max,step])=>`<label class="tm-setting"><span>${label}</span><input aria-label="${label}" data-setting="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${this.settings[key]}"><output data-output="${key}">${this.formatSetting(key,this.settings[key])}</output></label>`).join('')}<label class="tm-setting"><span>Quality</span><select data-setting="quality" aria-label="Quality">${QUALITY_IDS.map(q=>`<option value="${q}" ${q===this.settings.quality?'selected':''}>${q.toUpperCase()}</option>`).join('')}</select></label><label class="tm-setting tm-setting-toggle"><span>Crosshair</span><input aria-label="Crosshair" data-setting="crosshair" data-testid="crosshair-setting" type="checkbox" ${this.settings.crosshair?'checked':''}><output data-output="crosshair">${this.formatSetting('crosshair',this.settings.crosshair)}</output></label><label class="tm-setting tm-setting-toggle"><span>Hit stop</span><input aria-label="Hit stop" data-setting="hitStop" data-testid="hitstop-setting" type="checkbox" ${this.settings.hitStop?'checked':''}><output data-output="hitStop">${this.formatSetting('hitStop',this.settings.hitStop)}</output></label></div></div><div class="tm-screen-footer">${btn('bindings','KEY BINDINGS')}</div>`}
  ...
  handleShortcut(code,e){
    ...
    else if((traderOpenInView(this.view) || this.actions.sandbox?.()) && (!this.route || this.route==='trader')){
      if(this.view?.wave.phase==='intermission' && code===bindings.ready)action='ready'
  ...
  formatSetting(key,value){return key==='crosshair'||key==='hitStop'?(value?'ON':'OFF'):key==='sensitivity'?`${Number(value).toFixed(2)}x`:key==='fov'?`${value}°`:`${value}%`}
  ...
  refreshTrader(){
    ...
    // Only intermission publishes a close time. Relax has none, so show no clock.
    this.live('balance',this.view.scrap.toLocaleString(),true);this.live('time',this.actions.sandbox?.()?'∞':this.view.wave.phase==='intermission'?`${Math.ceil(this.view.wave.timer)}s`:'—')
  ...
  async action(action,button){
    ...
    if(action==='host-party' || action==='join-party'){
      if(this.actions.sandbox?.()){this.toast('Sandbox is single-player only.');return}
      this.show(action==='host-party'?'party-host':'party-join');return
    }
    ...
    if(action==='play' && this.actions.sandbox?.()){await this.enterMatch(()=>this.actions.start());return}
    ...
    if(action==='return-party'){await this.actions.returnParty();return}
    if(action==='trader'){if(traderOpenInView(this.view) || this.actions.sandbox?.())this.show('trader');return}
