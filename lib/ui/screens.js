...
export class Screens {
  ...
  render(view,extra,lobby){
    ...
    if(this.route==='trader' && view.wave.phase!=='intermission' && !this.actions.sandbox?.()){this.show(null)}
  ...
  handleShortcut(code,e){
    ...
    else if((this.view?.wave.phase==='intermission' || this.actions.sandbox?.()) && (!this.route || this.route==='trader')){
  ...
  refreshTrader(){
    ...
    this.live('balance',this.view.scrap.toLocaleString(),true);this.live('time',this.actions.sandbox?.()?'∞':`${Math.ceil(this.view.wave.timer)}s`)
  ...
  async action(action,button){
    ...
    if(action==='trader'){if(this.view.wave.phase==='intermission' || this.actions.sandbox?.())this.show('trader');return}
