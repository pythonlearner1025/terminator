...
export const iconButton=(action,name,glyph,kind='ghost')=>`<button class="tm-btn tm-icon-button ${kind}" data-action="${action}" data-testid="${action}" aria-label="${esc(name)}">${icon(glyph)}<span>${esc(name)}</span></button>`
...
export const playerLabels=players=>{
  const initials=players.map(player=>{
    const words=String(player.name || '?').trim().split(/\s+/)
    return [words[0],...(words.length>1?[words.at(-1)]:[])].map(word=>Array.from(word)[0] || '?').join('').toUpperCase()
  })
  return initials.map((name,index)=>initials.filter(other=>other===name).length>1?`${name}${index+1}`:name)
}
