...
import {PartySession, partyErrorMessage} from './party.js'
...
export class UiSession {
  ...
  partyChanged(state){
    ...
    if(state.status==='host_left' || state.error?.code==='host_left'){
      this.setInput(false)
      this.screens.show('main')
      this.screens.toast(partyErrorMessage({code:'host_left'}))
  ...
  sync(view){
    ...
    extra.spectate=this.spectate.sync(world,this.localPlayerId,playerView,!this.screens.route,this.manager.playersView)
