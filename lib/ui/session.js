...
export class UiSession {
  constructor(manager){
    ...
    this.sandboxEnabled=parseSandboxFlag(globalThis.location?.search)
    ...
    this.sandbox=this.sandboxEnabled?new SandboxPanel(manager):null
  ...
  async startMatch(){
    ...
    await Promise.all([this.manager.visualWarmup,prepareAudio()])
    if(!this.active)return
