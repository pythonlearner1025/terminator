...
import {AUTHORED_CLIPS} from '../view/weapon-clips.js'
...
// The main scene reaches the range through `?range=1` and nothing else. The
// weapons lab is a range already, so it builds its panel directly.
...
export class RangePanel {
  // `clips` adds the authored animation clips of the held rig. Only the weapons
  // lab asks for them: the rig it holds is the one an animator is reviewing.
  constructor(manager, {clips = false} = {}) {
    this.manager = manager; this.range = manager.ui?.range || manager.range
    this.clips = clips; this.clip = null; this.clipFraction = 0
    ...
    this.root.innerHTML = `<header><h2>WEAPONS RANGE</h2><small>F1 PANEL</small></header>
      ...
      ${clips ? group('AUTHORED CLIP / PLAYS ON THE HELD RIG', AUTHORED_CLIPS.map(name => button('clip',name,name.toUpperCase())).join('') + button('clip','live','LIVE')) : ''}
  ...
  action(action, value) {
    ...
    else if (action === 'clip') this.setClip(value === 'live' ? null : value)
    else if (action === 'targets') range.respawn()
    else if (action === 'shots') view.clearShots()
    this.sync(); m.syncViews()
  }
  // A selected clip replaces the live pose with the authored one, sampled at
  // `clipFraction`. `null` gives the rig back to the gameplay animation.
  setClip(name) {
    this.clip = name; this.clipFraction = 0
    this.manager.playerView.weapons.animation.setClipTime(name, 0, this.manager.world)
  }
  // The tick delta already carries the range clock's scale, so Pause holds a
  // clip still and Step walks it one 60 Hz frame at a time.
  advanceClip(ticks) {
    const m = this.manager, animation = m.playerView.weapons.animation
    // Only a rig with authored clips can hold a pose. Any other weapon gives the clip back.
    if (this.clip && !m.playerView.weapons.rigs[m.world.player.activeWeapon]?.clipPlayer) return this.setClip(null)
    const duration = this.clip ? animation.clipPreview?.duration : 0
    if (!duration) return
    this.clipFraction = (this.clipFraction + Math.max(0, ticks) / 60 / duration) % 1
    animation.setClipTime(this.clip, this.clipFraction, m.world)
  }
  sync() {
    const m = this.manager, range = m.range, view = m.rangeView
    if (!view) return
    if (this.clips) this.advanceClip(m.world.tick - (this.lastTick ?? m.world.tick))
    ...
    for (const b of this.root.querySelectorAll('[data-range-action]')) {
      ...
      const selected = a === 'weapon' ? m.world.player.activeWeapon === v : a === 'time' ? range.clock.scale === Number(v)
        : a === 'camera' ? (v === 'inspect') === view.inspecting : a === 'loop' ? view.loop === v : a === 'light' ? view.lighting === v
        : a === 'clip' ? (v === 'live' ? this.clip === null : this.clip === v) : false
      b.setAttribute('aria-pressed', String(selected))
      b.disabled = (a === 'step' && range.clock.scale !== 0) || (a === 'loop' && (!view.inspecting || (v === 'reload' && m.world.sandbox.noReload)))
        || (a === 'clip' && !m.playerView.weapons.rigs[m.world.player.activeWeapon]?.clipPlayer)
    }
  ...
  dispose() {
    if (this.clip) this.manager.playerView?.weapons?.animation?.setClipTime(null)
