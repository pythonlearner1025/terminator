...
export class RangePanel {
  constructor(manager) {
    ...
    this.root.innerHTML = `<header><h2>WEAPONS RANGE</h2><small>F1 PANEL</small></header>
      ...
      ${group('OVERLAYS', [['trajectories','Shot paths'],['impacts','Impact marks'],['freeze','Freeze flash'],['hitboxes','Hitboxes'],['ruler','Aim distance']].map(([id,label]) => `<label><input type="checkbox" data-range-option="${id}" data-testid="range-${id}" ${['trajectories','impacts','ruler'].includes(id)?'checked':''}>${label}</label>`).join(''), 'two')}
    ...
    this.ruler = document.createElement('output'); this.ruler.className = 'tm-range-ruler'; this.ruler.dataset.testid = 'range-distance'
    manager.hud.elements.combat.append(this.root, this.ruler); manager.hud.root.classList.add('is-range')
  ...
  sync() {
    ...
    this.ruler.hidden = !view.options.ruler || view.inspecting
    this.ruler.textContent = view.distanceLabel || 'Aim at a target'
  ...
  dispose() {
    ...
    this.manager.hud.root.classList.remove('is-range'); this.root.remove(); this.ruler.remove(); this.style.remove()
  }
}
