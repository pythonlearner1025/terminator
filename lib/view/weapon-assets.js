...
function cloneRig(source) {
  ...
  // world (Environment.plugin hides each node carrying `weaponAsset`, both the
  // scene template and the asset root inside it). A clone is the runtime copy,
  // not a source: it must draw. Without this the copy inherits the hidden asset
  // root, and three stops at it, so no mesh under it ever reaches the renderer.
  clone.traverse(n=>{if(n.userData?.weaponAsset)n.visible=true})
...
export function instantiateWeaponRigs(parent,modelRoot,materialFor=source=>source,{hands=true,ids=WEAPON_IDS}={}) {
  ...
  for(const id of ids){
    ...
    root.position.set(0,0,0);root.quaternion.identity();root.scale.setScalar(root.userData.viewModel?.scale??1);root.name=id+' viewmodel';parent.add(root)
