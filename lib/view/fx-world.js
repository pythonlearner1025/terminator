...
export function solidMaterial() {
  const maps={}
  for(const [key,name] of [['map','albedo'],['normalMap','normal'],['roughnessMap','roughness'],['metalnessMap','metalness'],['aoMap','ao']]){
    maps[key]=loader.load(weaponAsset(`weapon-${name}.png`));if(key==='map')maps[key].colorSpace=SRGBColorSpace
  }
  return new PhysicalMaterial({name:'Textured brass and impact fragments',...maps,metalness:1,roughness:1})
}

export class WeaponWorldFx {
  constructor(parent) {
    this.root=new Group();this.root.name='Weapon world impacts, smoke and brass';parent.add(this.root)
    this.eventIndex=null;this.lastTick=null;this.v=new Vector3();this.to=new Vector3();this.normal=new Vector3();this.velocity=new Vector3();this.size=new Vector3()
  ...
  shot(world,event) {
    ...
    if(unit) {
      ...
      hit=rayColliders(this.v,this.velocity,world.activeColliders())
    ...
    this.emit('bolts',this.v,this.velocity.clone().multiplyScalar(speed),this.size.set(plasma?.035:.008,plasma?.035:.008,plasma?.45:.75),Math.max(.016,distance/speed),plasma?0x8cecff:0xffd997)
  ...
  floorAt(pos) {
    let top=0
    for(const c of this.colliders){const y=c.center.y+c.size.y/2;if(y<pos.y&&y>top&&Math.abs(pos.x-c.center.x)<=c.size.x/2&&Math.abs(pos.z-c.center.z)<=c.size.z/2)top=y}
    return top
  }
...
// Slab intersection against read-only gameplay boxes also gives a stable normal.
export function rayColliders(origin,direction,colliders) {
  let best=null
  for(const collider of colliders) {
    let near=0,far=100,axis=null,sign=1
    for(const key of ['x','y','z']) {
      const min=collider.center[key]-collider.size[key]/2,max=collider.center[key]+collider.size[key]/2,d=direction[key],o=origin[key]
      if(Math.abs(d)<1e-8){if(o<min||o>max){far=-1;break}continue}
      let a=(min-o)/d,b=(max-o)/d
      if(a>b)[a,b]=[b,a]
      if(a>near){near=a;axis=key;sign=d>0?-1:1}
      far=Math.min(far,b)
    }
    if(axis&&far>=near&&near>.01&&(!best||near<best.distance)){
      const normal=new Vector3();normal[axis]=sign;best={distance:near,normal,collider}
    }
  }
  return best
}
