...
function sprite(name,additive=false,ownedTextures) {
...
export class WeaponWorldFx {
  constructor(parent) {
    ...
    this.ownedTextures.add(concrete.map);this.ownedTextures.add(concrete.normalMap);this.ownedTextures.add(concrete.roughnessMap)
    this.pools={
      ...
      smoke:new FxPool(this.root,'48 pooled smoke wisps',new PlaneGeometry(1,1),sprite('smoke',false,this.ownedTextures),48,{billboard:true,growth:1.1,fade:true}),
      ...
      holes:new FxPool(this.root,'64 persistent bullet impacts',new PlaneGeometry(1,1),sprite('hole',false,this.ownedTextures),64),
      scorch:new FxPool(this.root,'16 persistent plasma and blast scorches',new PlaneGeometry(1,1),sprite('scorch',false,this.ownedTextures),16),
      blast:new FxPool(this.root,'4 pooled explosion flashes',new PlaneGeometry(1,1),sprite('blast',true,this.ownedTextures),4,{billboard:true,fade:true,growth:12}),
      rings:new FxPool(this.root,'4 pooled expanding shockwaves',new PlaneGeometry(1,1),sprite('shockwave',true,this.ownedTextures),4,{growth:20,fade:true}),
    }
  ...
  dispose() {
    this.onSound=null;this.camera=null;this.light.intensity=0
    ...
    releaseSubtree(this.root);this.root.removeFromParent();for(const resource of resources)resource.dispose();for(const texture of this.ownedTextures)texture.dispose()
    this.ownedTextures.clear();this.pools={};this.poolList=[]
  }
}
