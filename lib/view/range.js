...
import {createRangeProps} from '../../generators/range-props.js'
...
export class RangeView {
  constructor(manager) {
    ...
    const source=this.viewer.scene.modelRoot.getObjectByName('Weapons Range') || this.viewer.scene.modelRoot.getObjectByName('Weapons_Range')
    ...
    this.props=createRangeProps(E);this.root.add(this.props)
    ...
    this.key=manager.mapView.root.getObjectByName('Map moon shadow key')
    ...
    for(;this.cursor<world.eventLog.length;this.cursor++) {
      ...
      if(e.type==='shot'&&e.by===world.player.id&&e.paths) {
        const paths=e.paths.map(path=>{
          ...
          const plate=this.plateHit(e.origin,path.direction,distance,true)
          return {points:[muzzle.clone(),new E.Vector3().copy(plate?.point||path.point)],hit:!!plate||path.hit}
        })
  ...
  draw() {
    ...
    for(const shot of this.shots)for(const path of shot.paths) {
      ...
      if(path.hit&&markers<180){this.matrix.position.copy(path.points.at(-1));this.matrix.updateMatrix();this.markers.setMatrixAt(markers++,this.matrix.matrix)}
    }
  ...
  clearShots() {
    ...
    this.manager.playerView.weapons.tracers.pool.reset();this.draw()
  }
  dispose() {
