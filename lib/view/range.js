...
export class RangeView {
  constructor(manager, options = {}) {
    ...
    this.root=new E.Group();this.viewer.scene.add(this.root);this.root.name='Weapons Range Runtime'
  ...
  dispose() {
    ...
    this.lines.dispose();this.lineMesh.material.dispose()
    this.markers.geometry.dispose();this.markers.material.dispose()
...
function disposeWire(wire){wire.traverse(node=>{node.geometry?.dispose();node.material?.dispose()});releaseSubtree(wire);wire.removeFromParent()}
