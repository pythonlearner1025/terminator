...
export function compileWarmupTargets(viewer,targets,{signal}={}) {
  ...
  for(const object of targets){
    ...
    // The shadow pass draws through customDepthMaterial, a separate program the
    // material loop above never sees. Instanced skinning patches it, so link it
    // here rather than on the first shadow render inside a match.
    if(object.customDepthMaterial)opaque.push({object,material:object.customDepthMaterial,
      compileObject:Object.create(object,{material:{value:object.customDepthMaterial}})})
  }
  const pending=[]
