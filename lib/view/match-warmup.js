...
export async function warmupMatch(manager) {
  ...
  let releaseWeapons=manager.playerView?.weapons?.primeWarmup?.(viewer.scene.mainCamera)
  const fixedPools=[
  ...
  try{
    ...
    for(let frame=0;frame<2;frame++){viewer.setDirty(manager);await nextFrame()}
    // Active projectile lights use one stable two-light shader variant. Compile
    // the cheaper inactive variant too before returning to an empty projectile pool.
