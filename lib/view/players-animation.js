...
export function animatePlayerWeaponMechanisms(gun, id, recoil, reloadTime = null) {
  gun.magazine.position.copy(gun.rest.magazine)
  gun.slide.position.copy(gun.rest.slide)
  gun.pump.position.copy(gun.rest.pump)
  if (reloadTime !== null) {
    const cycle = (reloadTime % 2) / 2
    gun.magazine.position.y -= Math.sin(clamp((cycle - .12) / .6) * Math.PI) * .18
    gun.magazine.rotation.z = Math.sin(Math.PI * cycle) * .12
  } else gun.magazine.rotation.set(0, 0, 0)
  gun.slide.position.z += recoil * .026
  if (id === 'shotgun') gun.pump.position.z += Math.sin(recoil * Math.PI) * .10
}
...
export function animatePlayer(rig, player, weapon, dt, time) {
  ...
  animatePlayerWeaponMechanisms(gun, weapon.id, rig.recoil, reloading ? rig.reload : null)
  if (reloading) {
