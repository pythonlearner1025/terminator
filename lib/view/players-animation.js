...
export function bindPlayerRig(object) {
  ...
  if (!joints.Pelvis || !joints['Hand Right']) throw new Error('Soldier Template is missing its articulated rig')
  ...
  return {object, joints, lamp, phase: 0, move: 0, run: 0, crouch: 0, aim: 0, reload: 0,
...
export function animatePlayer(rig, player, weapon, dt, time) {
  ...
  const running = moving && Boolean(player.sprinting)
  ...
  j.Pelvis.position.set(0, .96 - crouch * .33 + Math.abs(gait) * (.018 + rig.run * .022), 0)
  ...
  for (const [side, sign] of [['Left', -1], ['Right', 1]]) {
    ...
    pose(`Thigh ${side}`, -step * (.42 + rig.run * .4) - crouch * .88, 0, sign * .035)
    pose(`Shin ${side}`, Math.max(0, step) * (.72 + rig.run * .65) + crouch * 1.48)
    pose(`Foot ${side}`, step * (.25 + rig.run * .3) - Math.max(0, step) * .6 - crouch * .70)
  ...
  weapon.root.position.set(-.025, .055 + rig.aim * .075 - rig.run * .08, .30 - rig.recoil * .045)
  ...
  if (rig.death || rig.downed > .001) {
    ...
    pose('Thigh Left', -.5 * t, 0, -.1); pose('Shin Left', .9 * t)
    pose('Thigh Right', -.16 * t, 0, .12); pose('Shin Right', .28 * t)
    pose('Foot Left', -.2); pose('Foot Right', -.1)
    pose('Upper Arm Left', -.24, 0, dead ? -.15 : -.8); pose('Forearm Left', dead ? -.3 : -1.25)
    pose('Upper Arm Right', -.3, 0, -.4); pose('Forearm Right', -.7)
    if (!dead) j.Chest.rotation.x += Math.sin(time * 2) * .02
    weapon.root.visible = false
  }
  if (rig.lamp) { rig.lamp.intensity = rig.death ? Math.max(0, 1 - rig.death) : 1.1; rig.lamp.visible = rig.lamp.intensity > 0 }
