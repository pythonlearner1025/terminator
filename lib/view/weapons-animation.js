import {AnimationMixer,LoopOnce,LoopRepeat,Vector3} from 'threepipe'
import {AUTHORED_CLIPS} from './weapon-clips.js'

// A rig needs every authored clip but `Inspect` before it gets a clip player.
const REQUIRED_CLIPS=AUTHORED_CLIPS.filter(name=>name!=='Inspect')
// Iron sights. `sight.distance` says how far from the eye the aim pose holds the
// gun; the height and the pitch say where the sight line sits, and a rig that
// carries its own sight geometry states that itself. So read those two off the
// rig: lay the rear-to-front sight line on the view axis, and the sights land on
// the reticle whatever the authored numbers say. Rigs without sight nodes, the
// old pistol and the swing-out revolver, keep their authored offsets. The aim
// pose only rotates around X, so an off-centre sight line keeps them too.
//
// The centre of a sight's top edge, in the rig's own units. The shooter lines
// up that edge, not one corner vertex, so average the vertices that share the
// highest edge, the same reading `tools/blender/revolver-rebuild/ads.mjs` takes.
function sightTip(rig,name) {
  ...
  object.traverse(node=>{
    ...
    for(let i=0;i<attribute.count;i++){
      point.fromBufferAttribute(attribute,i);node.localToWorld(point);rig.root.worldToLocal(point)
...
export function sightLineAim(rig) {
  const rear=sightTip(rig,'RearSight'),front=sightTip(rig,'FrontSight')
  const scale=rig.root.scale.x
  if(!rear||!front||Math.abs(rear.x*scale)>.001||Math.abs(front.x*scale)>.001)return null
  const pitch=Math.atan2((rear.y-front.y)*scale,(rear.z-front.z)*scale)
  return {height:scale*(rear.y*Math.cos(pitch)-rear.z*Math.sin(pitch)),pitch}
}
// The authored aim offset is measured in the rig's own units, so a rig the view
// model scales keeps its sight picture only when the offset scales with it. The
// pitch is an angle and never scales.
export function applySightAim(rig) {
  const sight=rig.root.userData.viewModel?.sight
  if(!sight)return null
  const scale=rig.root.scale.x
  rig.sight={...sight,height:(sight.height||0)*scale,distance:(sight.distance||0)*scale,...(sightLineAim(rig)||{})}
  return rig.sight
}
...
export class WeaponAnimation {
  constructor(rigs,fx) {
    ...
    for(const rig of Object.values(rigs)){
      if(REQUIRED_CLIPS.every(name=>rig.clips?.some(clip=>clip.name===name)))rig.clipPlayer=new AuthoredWeaponClips(rig)
      applySightAim(rig)
    }
  }
  sync(world) {
  ...
  setClipTime(clip, fraction = 0, world = this.clipWorld) {
    ...
    if (!['idle', 'fire', 'reload', 'aim', 'switch', ...AUTHORED_CLIPS].includes(clip) || !world) return null
