...
import {releaseMesh, releaseSubtree} from '../mesh-release.js'
...
if (diffuseColor.a < 0.001) discard;`)
  ...
  const handle = {
    ...
    dispose() {
      if (disposed) return
      disposed = true
      releaseSubtree(group)
      group.removeFromParent()
      for (const batch of batches) { batch.mesh.removeFromParent(); batch.geometry.dispose(); releaseMesh(batch.mesh) }
