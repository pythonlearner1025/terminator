...
export function mountV2Lighting({viewer, root, map, refs = {}, preview = false, params = {}}) {
  ...
  const assetAbort=loadBakedPlumes?new AbortController():null
  const pendingAssets=[]
  ...
  const texture = (pixels, srgb = false) => {
    ...
    result.needsUpdate = true; ownedTextures.add(result); return result
  }
  const skyMap = texture(bakeSky(), true); skyMap.wrapS = RepeatWrapping
  ...
  const smokeMaterials = Array.from({length: 6}, (_, i) => {
    const map = texture(i < 4 ? bakePlume(loadBakedPlumes?32:384, i) : bakeSmoke(384, i)); map.name = `V2 independent smoke volume ${i + 1}`
    if(i<4 && loadBakedPlumes) {
      const url=`/files/assets/v2/lighting/plume-${i}.rgba`
  ...
  const horizonMap=texture(bakeHorizonHaze()); horizonMap.name='V2 continuous lower haze profile'
  ...
  const mistMap = texture(bakeMist()); mistMap.name = 'V2 illuminated exit air'
  ...
  viewer.setDirty?.()
  return {
    root: group, params: Object.freeze(p), ready: Promise.all(pendingAssets).then(()=>undefined), sync,
    dispose() {
      if (disposed) return
      disposed = true
      assetAbort?.abort()
