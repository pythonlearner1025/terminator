 ...
 */
export function createLightingNoiseLoader({moduleUrl=import.meta.url,fetchImpl=(...args)=>fetch(...args),decompress=async response=>{
  ...
  return async function load({signal}={}) {
    ...
    const response=await fetchImpl(new URL(manifest.url,moduleUrl).href,{signal})
