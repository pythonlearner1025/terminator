...
export class RosterFx {
  ...
  update(dt,camera){
    ...
    if(count){markRange(this.mesh.instanceMatrix,count*16);markRange(this.attributes,count*4)}
  }
  reset(){
  ...
  dispose(){
    ...
    // Instance buffers belong to the mesh, not its geometry.
    this.mesh.dispose();this.mesh.geometry.dispose();this.material.dispose();this.map.dispose();this.root.removeFromParent()
    // Clearing the link is what lets the geometry drop these meshes.
    releaseMeshGeometry(this.mesh)
    for(const v of this.prepared)for(const p of v.rig.roster?.pieces||[])releaseMeshGeometry(p.mesh)
    this.prepared.clear()
  }
}
