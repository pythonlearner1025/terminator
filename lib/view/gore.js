...
const SPARK_ONLY_TYPES = new Set(['t1000', 'hkaerial', 'hktank'])
...
export function goreDecision(event, hpMax, fallbackPart) {
  if(event.unitType==='t1000')return 0
...
export class GoreDeformer {
  ...
  reset() {
    // An undeformed rig already holds the source vertices, and its GPU copy is
    // current. Restoring it anyway copied and re-uploaded the whole mesh for
    // every unit a mob spawn recycled, whether or not it had ever been hit.
    if (!this.changed) { this.next = 1; this.lastVertexCount = 0; return }
...
export class GoreSystem {
  constructor(parent, ragdolls, fx, materials) {
    ...
    this.pieces=new GorePiecePool(index=>{
      ...
      const item={mesh,fade,record:null,born:0,trail:0,floor:0,pieceGeometry:null,batchId:null}
      mesh.userData.beginWreckFade=()=>{
        if(item.batchId!==null)this.pieceBatch?.setVisibleAt(item.batchId,false)
        mesh.visible=true
        for(const attribute of Object.values(mesh.geometry.attributes))attribute.needsUpdate=true
      }
      item.onRelease=()=>{
        item.record=null;mesh.visible=false;item.trail=0
        if(item.batchId!==null)this.pieceBatch?.setVisibleAt(item.batchId,false)
      }
    ...
    combineOrmMaterial(this.fluidMaterial)
  ...
  prepareType(visual) {
    ...
    if(SPARK_ONLY_TYPES.has(visual.unitType)){this.definitions.set(visual.unitType,definitions);return}
  ...
  primePieces() {
    ...
    if(!this.pieceBatch&&count){
      this.pieceBatch=new E.BatchedMesh(this.pieces.items.length,count*this.pieces.items.length,count*this.pieces.items.length*2,this.materials.metal)
      this.pieceBatch.name='Batched detached machine limbs';this.pieceBatch.frustumCulled=false;this.pieceBatch.raycast=NO_RAYCAST
      this.root.add(this.pieceBatch)
      for(const item of this.pieces.items){
        item.batchId=this.pieceBatch.addGeometry(item.pieceGeometry,count)
        this.pieceBatch.setVisibleAt(item.batchId,false)
      }
    }
  }
  prepareVisual(visual) {
    if(visual.gore||SPARK_ONLY_TYPES.has(visual.unitType))return
  ...
  detach(visual,name,direction,floor) {
    ...
    if(item.pieceGeometry) {
      ...
      if(this.pieceBatch&&item.batchId!==null){
        const attributes=Object.values(geometry.attributes),counts=attributes.map(attribute=>attribute.count)
        for(const attribute of attributes)attribute.count=source.attributes.position.count
        this.pieceBatch.setGeometryAt(item.batchId,geometry)
        for(let i=0;i<attributes.length;i++)attributes[i].count=counts[i]
      }
    ...
    mesh.name='Detached '+name
    if(this.pieceBatch&&item.batchId!==null){
      mesh.visible=false;mesh.updateMatrix();this.pieceBatch.setMatrixAt(item.batchId,mesh.matrix);this.pieceBatch.setVisibleAt(item.batchId,true)
  ...
  update(dt,nav) {
    ...
    for(const item of this.pieces.items)if(item.record&&this.pieceBatch&&item.batchId!==null&&this.pieceBatch.getVisibleAt(item.batchId)){
      item.mesh.updateMatrix();this.pieceBatch.setMatrixAt(item.batchId,item.mesh.matrix)
    }
    ...
    for(const pool of this.particles) {
      ...
      pool.mesh.count=live;pool.mesh.visible=live>0;if(live)markRange(pool.mesh.instanceMatrix,live*16)
    }
    let live=0
    ...
    this.stumpMesh.count=live;this.stumpMesh.visible=live>0;if(live)markRange(this.stumpMesh.instanceMatrix,live*16)
    live=0
    ...
    this.stainMesh.count=live;this.stainMesh.visible=live>0;if(live)markRange(this.stainMesh.instanceMatrix,live*16)
  ...
  dispose() {
    this.pieces.reset()
    this.pieceBatch?.dispose();this.pieceBatch?.removeFromParent();this.pieceBatch=null
    for(const item of this.pieces.items){item.pieceGeometry?.dispose();item.fade.dispose();releaseMeshGeometry(item.mesh)}
    ...
    for(const pool of this.particles){pool.mesh.geometry.dispose();pool.mesh.material.dispose();pool.mesh.dispose?.();releaseMeshGeometry(pool.mesh)}
    for(const key of ['map','normalMap','roughnessMap'])this.fluidMaterial[key]?.dispose()
    // Geometry disposal does not release the meshes' instanceMatrix buffers.
    this.stainMesh.dispose();this.stumpMesh.dispose()
    this.fluidMaterial.dispose();this.stainMesh.geometry.dispose();this.stumpMesh.geometry.dispose();this.placeholder.dispose()
    // The pooled meshes go out of scope here; clear the link that keeps each
    // one reachable from the geometry it last drew.
    releaseMeshGeometry(this.stainMesh);releaseMeshGeometry(this.stumpMesh)
