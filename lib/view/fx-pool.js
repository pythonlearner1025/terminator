...
export class FxPool {
  constructor(parent,name,geometry,material,capacity,{billboard=false,gravity=0,growth=0,fade=false,bounce=false,alignVelocity=false}={}) {
    ...
    parent.add(this.mesh);this.mesh.visible=false
    ...
    for(let i=0;i<capacity;i++)this.mesh.setMatrixAt(i,this.temp.matrix)
  }
  ...
  update(dt,camera,onBounce) {
    ...
    for(let i=0;i<this.items.length;i++) {
      ...
      if(p.life>0) {
        alive++;p.pos.addScaledVector(p.vel,dt);p.vel.y-=this.gravity*dt
        ...
        } else this.mesh.setColorAt(i,p.color)
      } else this.temp.scale.setScalar(0)
      this.temp.updateMatrix();this.mesh.setMatrixAt(i,this.temp.matrix)
    }
    this.active=alive;this.mesh.visible=alive>0;this.mesh.instanceMatrix.needsUpdate=true
