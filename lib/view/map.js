...
export class MapView {
  ...
  start() {
    ...
    // Keep the complete source and child relationships available for baked
    // geometry borrowers, without visiting the stopped copy in every pass.
  ...
  startEffects() {
    ...
    this.ready = environmentReady.then(async () => {
      ...
      for (const [name, mount] of mounts) {
        if (name === 'ground' && this.v2Handles[0]?.root?.userData?.aggregateFragments?.length) throw new Error('Ground would duplicate the architecture fine field')
