...
export class MapView {
  ...
  sync(world) {
    ...
    for (const gate of this.refs.gates) {
      ...
      gate.light.visible = active && (distance < 18 || angle < 1.15)
      gate.light.intensity = gate.light.visible ? 8 + Math.sin(t * 7) * 5 : 0
    ...
    for (const hazard of this.refs.hazards) {
      ...
      hazard.light.visible = kind === 'electric'
      if (kind === 'electric') {
    ...
    trader.light.visible = trader.amount > 0.01
