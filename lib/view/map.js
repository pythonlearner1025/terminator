...
import {sleepSubtrees,holdStill} from './hidden-subtrees.js'
import {mountV2Architecture} from './v2/architecture.js'
import {mountV2Ground} from './v2/ground.js'
import {mountV2Materials} from './v2/materials.js'
import {mountV2Lighting} from './v2/lighting.js'
import {mountV2Effects} from './v2/effects.js'
import {installV2Transparency} from './v2/transparency.js'
import {releaseSubtree} from './mesh-release.js'

export const MAP_FOG_DENSITIES = [0.0045, 0.018, 0.035, 0.058]

// The baked environment: 341 meshes and 121.3 MB of vertex data that only Play
// draws. It used to hang off the authored `V2 Environment Preview` node, so
// every editor session paid for it while stopped, and Play borrowed it from
// there. The authored node is now a stand-in (tools/v2/drop-preview-subtree.mjs)
// and Play imports the bake itself, then drops it on stop.
// Play still works without it: mountV2Architecture and mountV2Ground rebuild
// the same triangles from the recipes, which costs about 7 s of CPU instead of
// 40 ms. test/view/baked-environment.test.js pins that the two agree.
export const V2_BAKED_ENVIRONMENT = '/kite3d/@v2-stopped-preview-019/f.gltf'

// User-timing marks, so a frame trace can place this work on the same clock as
// its long tasks. Never throws and never fails a mount if the API is missing.
function mark(name) {
...
export class MapView {
  ...
  start() {
    ...
    const preview = (this.viewer.scene.modelRoot.getObjectByName('V2 Environment Preview') || this.viewer.scene.modelRoot.getObjectByName('V2_Environment_Preview'))
    this.previewState = preview ? {node: preview, visible: preview.visible} : null
    // The authored node is a stand-in with no geometry of its own. Hide it for
    // the same reason the map source is hidden: Play draws runtime copies.
    ...
    // Play renders runtime copies, so the scene root, the model root and the
    // runtime root hold still. That is what lets the authored map and the
    // environment preview under them sleep: hidden or not, three otherwise
    // recomputes a world matrix for all 1,700 of their nodes on every pass.
    this.releaseStatic = holdStill([this.viewer.scene, this.viewer.scene.modelRoot, this.root])
    this.wakeAuthored = sleepSubtrees([source, preview].filter(Boolean))
    this.root.updateMatrixWorld(true)
    // Ask for the bake here, not in startEffects. start() runs from the menu's
    // idle preparation, so the download and the meshopt decode overlap the menu
    // instead of the first frame of the match.
    const activeRoot = this.root
    this.bakedEnvironment = null
    // Named so a frame trace can say whether the download and the meshopt
    // decode landed before Start returned or inside a running wave.
    mark('terminator:bake-import:start')
    this.bakedEnvironmentReady = this.viewer.import(V2_BAKED_ENVIRONMENT, {cacheAsset: false})
      .catch(error => {
        console.warn('[Map] baked environment unavailable, rebuilding from recipes', error)
        return null
      })
      .then(asset => {
        // A stop between the request and the answer owns the asset, not Play.
        if (this.root !== activeRoot) {
          asset?.dispose?.(true)
          return null
        }
        this.bakedEnvironment = asset || null
        return this.bakedEnvironment
      })
      .finally(() => measure('terminator:bake-import'))
  }

  startEffects() {
    if (this.refs) return
    this.refs = createRuntimeRefs(engine, this.root, this.map, this.batching.dynamic)
    this.refs.v2BakedPreviewSource = this.previewState?.node || null
    ...
    this.refs.localLights = this.localLights
    this.refs.localLightEntries = this.localLightEntries
    ...
    const activeRoot = this.root
    const environmentReady = this.viewer.import(new URL('../../assets/hdri/qwantani_moon_noon_puresky_2k.hdr', import.meta.url).href).then(texture => {
      if (this.root !== activeRoot) {
        texture?.dispose()
        return
      }
      ...
      if (this.root === activeRoot) throw error
    })
    ...
    for (const name of ['Cool Blue Key', 'Orange Fill']) {
      const light = scene.modelRoot.getObjectByName(name) || scene.modelRoot.getObjectByName(name.replaceAll(' ', '_'))
      if (light) {
    ...
    this.post.start()
    mark('terminator:v2-mount:start')
    // HDR must settle before modules own background/environment settings.
    // Every asynchronous continuation checks the captured root before mutation.
    this.ready = environmentReady.then(async () => {
      if (this.root !== activeRoot) return
      this.v2Transparency = installV2Transparency({viewer: this.viewer})
      // Borrow the bake when it arrives; fall through to the recipes when it
      // does not. Either way the geometry is identical, so a missing file costs
      // time, never correctness.
      const baked = await this.bakedEnvironmentReady
      if (this.root !== activeRoot) return
      if (baked) this.refs.v2BakedPreviewSource = baked
      const context = {viewer: this.viewer, root: activeRoot, map: this.map, refs: this.refs, preview: false}
      const mounts = [['architecture', mountV2Architecture], ['ground', mountV2Ground], ['materials', mountV2Materials], ['lighting', mountV2Lighting], ['effects', mountV2Effects]]
      for (const [name, mount] of mounts) {
        if (name === 'ground' && this.v2Handles[0]?.root?.userData?.aggregateFragments?.count) throw new Error('Ground would duplicate the architecture fine field')
        const handle = mount(context)
        if (!handle || typeof handle.sync !== 'function' || typeof handle.dispose !== 'function') {
          handle?.dispose?.()
          throw new Error(`${mount.name} returned an invalid V2 lifecycle handle`)
        }
        this.v2Handles.push(handle)
        this.v2ModuleNames.push(name)
        // A later material pass must see geometry produced by an earlier
        // module's async asset load. Recheck ownership after every await.
        await handle.ready
        if (this.root !== activeRoot) return
      }
      this.v2WeatherBalance = mountV2WeatherBalance(this.refs)
      if (this.latestWorld) this.syncV2(this.latestWorld)
      this.viewer.renderManager.resetShadows()
      this.viewer.setDirty()
      measure('terminator:v2-mount')
    })
    // Attach a rejection observer immediately; ready itself remains rejected so
    // warmup, capture and validation cannot report missing assets as success.
    this.ready.catch(error => {
      if (this.root !== activeRoot) return
      this.v2Error = error
      console.error('[Map] V2 scene failed to initialize', error)
    })
    this.viewer.setDirty()
  }
  ...
  sync(world) {
    ...
    for (; this.eventIndex < world.eventLog.length; this.eventIndex += 1) {
      ...
      // The director moves the crate between waves. The clone follows the
      // collider the world already moved, so trading stays where it looks.
      if (event.type === 'trader_moved' && this.refs.trader) {
        this.refs.trader.object.position.set(event.pos.x, event.pos.y, event.pos.z)
        moving = true
      }
    ...
    if (trader) {
      // The director opens the trader during relax too, not only between waves.
      const traderOpen = world.phase === 'intermission' || world.traderOpen === true
      trader.amount += ((traderOpen ? 1 : 0) - trader.amount) * ease
  ...
  syncV2(world) {
    this.v2Transparency?.sync()
    for (const handle of this.v2Handles) handle.sync(world)
    this.v2WeatherBalance?.sync(world)
    // Original light values and all overrides feed the fixed pool in this frame.
  ...
  stop() {
    this.v2WeatherBalance?.dispose()
    this.v2WeatherBalance = null
    for (let index = this.v2Handles.length - 1; index >= 0; index--) this.v2Handles[index].dispose()
    this.v2Handles.length = 0
    this.v2ModuleNames.length = 0
    this.v2Transparency?.dispose()
    this.v2Transparency = null
    // After the handles, never before: mountBakedGround shares the source
    // geometry rather than copying it. An import still in flight sees the root
    // change and disposes itself.
    this.bakedEnvironmentReady = null
    this.bakedEnvironment?.dispose?.(true)
    this.bakedEnvironment = null
    this.latestWorld = null
    this.wakeAuthored?.()
    this.wakeAuthored = null
    this.releaseStatic?.()
    this.releaseStatic = null
    if (this.previewState) {
      this.previewState.node.visible = this.previewState.visible
    ...
    this.refs?.weather?.dispose()
    for (const object of [
      ...(this.refs?.hazards || []).flatMap(hazard => [hazard.arcs, hazard.steam]),
      this.refs?.trader?.inner,
      ...(this.refs?.fires || []).flatMap(fire => [fire.flame, fire.smoke]),
      this.refs?.atmosphere?.steam,
      this.refs?.atmosphere?.sparks,
    ].filter(Boolean)) {
      object.geometry?.dispose()
      object.material?.dispose()
    }
    const debugResources = new Set()
    this.debugColliders?.traverse(object => {
      if (object.geometry) debugResources.add(object.geometry)
      if (object.material) debugResources.add(object.material)
    })
    for (const resource of debugResources) resource.dispose()
    this.root?.traverse(object => {
      if (object.isLight) object.shadow?.dispose?.()
    })
    releaseSubtree(this.debugColliders)
...
function createRuntimeRefs(api, root, map, dynamic) {
  ...
  for (const object of byRole('trader')) {
    ...
    object.add(light)
    refs.trader = {object, lid, inner, light, amount: 0, depth: map.trader.size.z, baseZ: lid.position.z}
  }
...
// Presentation weights after correct transparent composition. Weather density,
// timing, settings, trajectories and gameplay light switches remain authoritative.
export function mountV2WeatherBalance(refs) {
  const weather = refs.weather
  const rain = weather?.rain?.material
  const dust = weather?.root?.getObjectByName('Dust caught in floodlight cones')?.material
  const rainOpacity = rain?.opacity, dustOpacity = dust?.opacity
  const shafts = (weather?.shafts || []).map(shaft => ({
    uniform: shaft.beam.material.uniforms?.opacity,
    opacity: shaft.beam.material.uniforms?.opacity?.value,
  }))
  let disposed = false
  return {
    sync() {
      if (disposed) return
      if (rain) rain.opacity = .27 * weather.settings.rain * .18
      if (dust) dust.opacity = dustOpacity * .24
      // Saved after lighting mounted: do not accumulate scaling on repeated sync.
      for (const {uniform, opacity} of shafts) if (uniform) uniform.value = opacity * .16
    },
    dispose() {
      if (disposed) return
      disposed = true
      if (rain) rain.opacity = rainOpacity
      if (dust) dust.opacity = dustOpacity
      for (const {uniform, opacity} of shafts) if (uniform) uniform.value = opacity
    },
  }
}
