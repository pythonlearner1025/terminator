...
import {detachAuthoredRoot} from './authored-detachment.js'
...
export class MapView {
  ...
  start() {
    ...
    this.previewState = preview ? {node: preview, visible: preview.visible,
      attachment: detachAuthoredRoot(preview,this.viewer.object3dManager)} : null
    if (preview) {
  ...
  stop() {
    // Register the retained source before borrowed V2 runtime geometry leaves
    // the scene, so Object3DManager still sees its authored resource users.
    this.previewState?.attachment.restore()
