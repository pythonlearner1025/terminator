...
export class MapView {
  ...
  syncLocalLights(player){
    for(const entry of this.localLightEntries){
      const dx=entry.position.x-player.x,dz=entry.position.z-player.z
      entry.score=entry.light.intensity>0?entry.light.intensity/(1+(dx*dx+dz*dz)*.05):-1
  ...
  stop() {
    ...
    this.owner?.cleanup(); this.owner = null; this.root = null; this.geometry = null; this.refs = null;this.localLights=[];this.localLightEntries=[]
