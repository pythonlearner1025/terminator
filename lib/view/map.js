...
export const MAP_FOG_DENSITIES = [0.002, 0.018, 0.035, 0.058]
...
export class MapView {
  ...
  start() {
    ...
    this.post = new MapPost(this.viewer); this.post.start()
  ...
  sync(world) {
    ...
    for (const gate of this.refs.gates) {
      ...
      gate.light.intensity = active ? 8 + Math.sin(t * 7) * 5 : 0.5
    ...
    for (const hazard of this.refs.hazards) {
      ...
      if (kind === 'steam') updatePoints(hazard.steam, (i) => {
        const age = (t * 0.48 + i * 0.137) % 1
        return [hazard.pos[0] + Math.sin(i * 13) * hazard.size.x * 0.26 + age * 0.45,
    ...
    for (const [index, fire] of this.refs.fires.entries()) {
      ...
      updatePoints(fire.flame, i => {
        const age = (t * (i % 4 === 0 ? 0.28 : 0.85) + i * 0.073) % 1
        const ember = i % 4 === 0
        return [fire.pos[0] + Math.sin(i * 17 + age * 2) * 0.26 * scale * (1 - age * 0.6) + (ember ? age * 0.6 : 0),
          fire.pos[1] + age * (ember ? 3.8 : 1.1) * scale,
          fire.pos[2] + Math.cos(i * 9) * 0.24 * scale]
      })
      updatePoints(fire.smoke, i => {
        const age = (t * 0.11 + i * 0.139) % 1
        return [fire.pos[0] + age * 2 * scale + Math.sin(i * 3) * 0.3, fire.pos[1] + (1 + age * 5) * scale, fire.pos[2] + age * scale]
      })
    }
    updatePoints(this.refs.dust, i => [Math.sin(i * 17) * 24 + Math.sin(t * 0.15 + i) * 0.6, 0.5 + (i * 0.179 + t * 0.04) % 5, Math.cos(i * 31) * 25])
    this.refs.wire.visible = Math.sin(t * 2.7) > 0.8
    updatePoints(this.refs.wire, i => {
      const age = (t * 2 + i * 0.19) % 1
      return [-23 + age * 0.8, 2.3 - age * age * 1.8, 3.65 - age * 0.3 + Math.sin(i * 7) * age * 0.3]
    })
  ...
  stop() {
    ...
    if (this.saved) {
      this.viewer.scene.fog = this.saved.fog; this.viewer.scene.background = this.saved.background; this.saved = null
    }
    ...
    this.owner?.cleanup(); this.owner = null; this.root = null; this.geometry = null; this.refs = null
...
function updatePoints(points, position) {
  ...
  for (let i = 0; i < attribute.count; i++) attribute.setXYZ(i, ...position(i))
