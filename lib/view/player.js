...
export class PlayerView {
  ...
  start(world, authoredSource) {
    this.stop()
    const source = authoredSource || findAuthored(this.viewer.scene.modelRoot, 'Player Start')
    ...
    if (new URLSearchParams(globalThis.location?.search || '').get('weapon') === 'revolver-rebuild') {
      this.weapons.selectVariant('revolver-rebuild')
    }
  ...
  sync(world) {
    ...
    const aimFov=world.weaponCatalog.weapons[player.activeWeapon]?.aimFov??AIM_VIEW.fov
    camera.fov=baseFov+(aimFov-baseFov)*aim
    camera.updateProjectionMatrix()
    camera.updateMatrixWorld()
