...
// Keeps every rig in the view and hides the ones that are not shown. Showing
// the current rig stays with WeaponAnimation, which fades it in.
...
export function keepRigsResident(rigs,feel,shown) {
  for(const rig of Object.values(rigs)){
    ...
    if(rig.id!==shown)rig.root.visible=false
  }
}
