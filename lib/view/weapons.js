...
function makeMaterial(projection) {
  ...
  material.registerMaterialExtensions([projection, {
    uuid:'terminator-weapon-surfaces',computeCacheKey:'terminator-weapon-surfaces-v2',
    ...
    shaderExtender(shader) {
      ...
      // One camera-space key plus the shared reflection probe. Evaluating every
      // courtyard and pooled combat light on a foreground gun wastes GPU time.
...
class Builder {
  ...
  finish() {
    ...
    for(const [parent,a] of this.parts) {
      ...
      const hand=parent.name.endsWith('gloved hand')
...
function roundedBox() {
  ...
  const face=points=>{
    ...
    for(let i=1;i<points.length-1;i++)for(const p of [points[0],points[i],points[i+1]]) {
      positions.push(...p);uvs.push(p[0]+p[2]+.5,p[1]+.5)
    }
  }
