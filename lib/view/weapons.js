...
import {instantiateWeaponRigs, instantiateGrenade, getWeaponLibrary, WEAPON_IDS} from './weapon-assets.js'
export {WEAPON_IDS} from './weapon-assets.js'
import {AuthoredWeaponClips,WeaponAnimation,applySightAim} from './weapons-animation.js'
...
export const DEFAULT_REVOLVER_VARIANT='revolver-rebuild'
export function revolverVariantFromSearch(search=globalThis.location?.search||'') {
  const requested=new URLSearchParams(search).get('weapon')
  return ['pistol','swingout','revolver-rebuild'].includes(requested)?requested:DEFAULT_REVOLVER_VARIANT
}
...
export class WeaponView {
  constructor(parent,viewer) {
    ...
    this.selectVariant(revolverVariantFromSearch())
  }
  selectVariant(id=DEFAULT_REVOLVER_VARIANT) {
    ...
    if(!this.variants.has(id)){
      // Only the pistol and the rebuilt revolver are authored in the play scene;
      // the swing-out revolver lives in the weapons lab scene. A request this
      // scene cannot serve falls back to the default rig instead of failing the
      // match inside the view constructor.
      if(!getWeaponLibrary(this.viewer.scene.modelRoot).sources[id]){
        const fallback=id===DEFAULT_REVOLVER_VARIANT?null:DEFAULT_REVOLVER_VARIANT
        console.warn(`No authored weapon asset "${id}" in this scene. Using the ${fallback||this.variant} rig.`)
        return fallback?this.selectVariant(fallback):undefined
      }
      const rig=createWeaponRigs(this.feel,this.material,this.viewer.scene.modelRoot,{ids:[id]})[id]
      rig.id='pistol';rig.clipPlayer=new AuthoredWeaponClips(rig);applySightAim(rig);this.variants.set(id,rig)
  ...
  beforeRender(camera) {
    ...
    // Match world zoom while retaining the active rig's authored hip projection.
    const zoom=Math.tan(camera.fov*Math.PI/360)/Math.tan(this.baseFov*Math.PI/360)
    const vm=this.rigs[this.animation.shown].root.userData.viewModel
    const hipFov=vm.fov||54,authoredFov=hipFov+((vm.aimFov??hipFov)-hipFov)*this.animation.aimAmount
    const fov=2*Math.atan(Math.tan(authoredFov*Math.PI/360)*zoom)*180/Math.PI
