...
function roundedBox() {
  ...
  for(let axis=0;axis<3;axis++)for(const sign of [-1,1]) {
    face([[-1,-1],[1,-1],[1,1],[-1,1]].map(([a,b])=>{const p=[0,0,0];p[axis]=sign*.5;p[(axis+1)%3]=a*.41;p[(axis+2)%3]=b*.41;return p}))
  }
  for(let axis=0;axis<3;axis++)for(const s of [-1,1])for(const t of [-1,1]) {
    face([[.5,.41,-.41],[.41,.5,-.41],[.41,.5,.41],[.5,.41,.41]].map(([a,b,c])=>{
  ...
  for(const x of [-1,1])for(const y of [-1,1])for(const z of [-1,1])face([
    [x*.5,y*.41,z*.41],[x*.41,y*.5,z*.41],[x*.41,y*.41,z*.5],
  ])
...
function makeGun(b,id,parent,includeHands=true) {
  ...
  let front=-.32
  if(id==='pistol') {
    b.box(body,[.063,.053,.235],[0,-.014,-.105],'dark');grip(b,body)
    b.box(rig.slide,[.064,.060,.27],[0,.044,-.115],'steel')
    b.box(rig.slide,[.057,.011,.24],[0,.08,-.12],'dark')
    b.box(rig.slide,[.014,.028,.076],[.0325,.043,-.087],'dark')
    b.rod(body,.014,.255,[0,.046,-.142],'edge');front=-.274
    b.rod(body,.010,.004,[0,.046,front],'rubber')
    for(let i=0;i<9;i++)for(const s of [-1,1])b.box(rig.slide,[.003,.043,.005],[s*.033,.045,.013-i*.009],'dark')
    rig.sight=sights(b,rig.slide,-.244,.016,.085)
    marking(b,rig.slide,[.033,.045,-.156],'pistolMark')
    b.box(rig.magazine,[.052,.155,.057],[0,-.125,.05],'dark',[-.22,0,0])
    b.box(rig.magazine,[.086,.021,.085],[0,-.205,.064],'steel')
    screws(b,body,[[.033,-.01,.02],[.038,-.11,.07]])
...
export class WeaponView {
  ...
  beforeRender(camera) {
    ...
    this.screenFx?.layout()
  }
  dispose() { this.fx.dispose();this.worldFx.dispose();this.screenFx?.dispose();this.world=null }
}
