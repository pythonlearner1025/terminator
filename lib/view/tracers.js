...
export const TRACER_STYLE = Object.freeze({
  pistol: {speed:150,width:.075,glow:.72,trail:4.8,pixels:72,color:[4.8,1.6,.18]},
  m4: {speed:150,width:.114,glow:1.02,trail:8,pixels:112,color:[5.2,1.25,.08]},
  shotgun: {speed:150,width:.081,glow:.75,trail:3.2,pixels:64,color:[4.8,1.5,.12],pellets:9},
  plasma: {speed:150,width:.36,glow:1.86,trail:8,pixels:120,color:[.25,1.9,5.8]},
  sniper: {speed:150,width:.09,glow:.90,trail:14,pixels:176,color:[5.4,3.1,.65]},
})
...
export class StreakBatch {
  constructor(parent,capacity,name='Travelling light ribbons') {
    ...
    this.material.registerMaterialExtensions([{
      ...
      uuid:'terminator-streak-ribbon',computeCacheKey:'terminator-streak-ribbon-v2',
      ...
      shaderExtender(shader) {
        shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>',`
          ...
          pixelA=pixelB-along*max(span,shape.w*streakViewport.y/1080.);
        ...
        shader.fragmentShader=shader.fragmentShader.replace('#include <opaque_fragment>',`
          ...
          float halo=exp(-ribbonX*ribbonX*5.)*.65*(1.-smoothstep(.75,1.,ribbonX));
          // Keep the forward two thirds hot. Only the rear third dissolves.
          float tail=smoothstep(0.,1./3.,vStreakUv.y);
          float head=1.-smoothstep(.93,1.,vStreakUv.y);
          outgoingLight=vec3(6.)*core+vTint*halo;
