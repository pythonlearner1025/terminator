...
export class PlayersView {
  constructor(viewer) {
    ...
    this.root = null; this.showcase = null; this.world = null
  }
  start(world, localPlayerId) {
    this.stop(); this.world = world; this.localPlayerId = localPlayerId
    this.template = this.viewer.scene.modelRoot.getObjectByName('Unit Resistance Soldier')
      || this.viewer.scene.modelRoot.getObjectByName('Unit_Resistance_Soldier')
    if (!this.template) throw new Error('Unit Resistance Soldier authored node not found; run npm run scene')
    this.root = new E.Group(); this.viewer.scene.add(this.root); this.root.name = 'Players Runtime'
  ...
  ensureResources() {
    ...
    this.weapons ||= new PlayerWeaponLibrary(this.root, this.materials.olive, this.viewer.scene.modelRoot)
  ...
  createVisual(id, variant, parent = this.root) {
    ...
    if (!this.runtimeTemplate) {
      this.runtimeTemplate = clonePlacedUnitFigure(this.template, 'soldier')
      this.runtimeTemplate.visible = false
      this.root.add(this.runtimeTemplate)
    }
    // The asset-backed figure stays hidden until a teammate exists.
    const object = cloneSkinnedFigure(this.runtimeTemplate)
    object.visible = true
  ...
  removeVisual(visual) { disposePlayerRig(visual.rig); releaseSubtree(visual.object); visual.object.removeFromParent() }
  stop() {
    ...
    this.runtimeTemplate?.traverse(child => { if (child.isSkinnedMesh) child.skeleton.dispose() })
    releaseSubtree(this.runtimeTemplate)
    releaseSubtree(this.root)
    this.root?.removeFromParent(); this.root = null; this.world = null; this.materials = null; this.runtimeTemplate = null; this.template = null
  }
}
