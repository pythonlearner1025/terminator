...
export class PlayersView {
  constructor(viewer) {
    this.viewer = viewer; this.visuals = new Map(); this.variants = new Map()
  ...
  start(world, localPlayerId) {
    ...
    this.preview = this.template.children.find(child => child.userData.soldierTemplate)
    if (!this.preview) throw new Error('Soldier Template generator preview is missing')
  ...
  createVisual(id, variant, parent = this.root) {
    ...
    // Match UnitView: clone the generated figure, never its Generator component.
    // trackEffect protects authored geometry/materials, while skeletons are private.
    const object = this.preview.clone(true)
  ...
  fire(visual, tick) {
    ...
    visual.cooldown = this.world?.weaponCatalog?.weapons?.[id]?.fireInterval || ({m4: .1, plasma: .24, shotgun: .8, pistol: .25}[id] || .5)
  ...
  stop() {
    ...
    this.visuals.clear(); this.variants.clear()
    this.weapons?.stop(); this.weapons = null
    this.fx?.dispose(); this.fx = null
    this.owner?.cleanup(); this.owner = null; this.root = null; this.world = null; this.preview = null; this.template = null
  }
}
