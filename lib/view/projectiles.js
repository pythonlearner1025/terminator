...
export class ProjectileView {
  constructor(parent,capacity=192,resources={}) {
    ...
    // A point light that comes and goes changes the scene's point-light count,
    // and three bakes that count into every program cache key. The whole scene
    // would then link a fresh program on its next draw. These stay visible for
    // the life of the view; intensity zero is off.
  ...
  sync(world,camera) {
    ...
    for(let i=0;i<this.lights.length;i++){this.lights[i].intensity=0;this.lightCandidates[i].active=false}
    ...
    for(const candidate of this.lightCandidates)if(candidate.active){
      ...
      light.position.copy(candidate.pos);light.color.setRGB(style.hot[0],style.hot[1],style.hot[2]);light.intensity=1.4
    }
  }
