...
export class ProjectileView {
  ...
  sync(world,camera) {
    ...
    for(let i=0;i<world.projectiles.length;i++) {
      ...
      if(base.renderer==='orb') {
        ...
        this.offerLight(this.to,style,camera)
    ...
    for(const candidate of this.lightCandidates)if(candidate.active){
      ...
      light.position.copy(candidate.pos);light.color.setRGB(style.color[0],style.color[1],style.color[2]);light.intensity=1.4
    }
    if(lightIndex)for(const light of this.lights)light.visible=true
    this.orbs.visible=this.orbs.count>0;this.shells.visible=this.shells.count>0
    if(this.orbs.count){this.markRange(this.orbs.instanceMatrix,this.orbs.count*16);this.markRange(this.orbs.instanceColor,this.orbs.count*3)}
    if(this.shells.count)this.markRange(this.shells.instanceMatrix,this.shells.count*16)
  }
  ...
  primeWarmup(camera){
    const projectiles=[
      ...
      {id:'warmup-bolt-2',type:'bolt',owner:'unit',pos:{x:-.6,y:1,z:7.5},vel:{x:0,y:0,z:48}},
      {id:'warmup-bolt-3',type:'bolt',owner:'unit',pos:{x:.8,y:1,z:8.5},vel:{x:0,y:0,z:50}},
      {id:'warmup-shell',type:'shell',owner:'unit',pos:{x:.6,y:1,z:8},vel:{x:0,y:0,z:32}},
    ]
    this.sync({tick:1,projectiles},camera)
    return()=>this.reset()
  }
  reset(){
    for(const slot of this.slots){slot.id=null;slot.seen=0;slot.missingAge=0}
    this.serial=0;this.lastTick=null;this.streaks.begin();this.streaks.finish();this.smoke.begin();this.smoke.finish()
    this.orbs.count=0;this.orbs.visible=false;this.shells.count=0;this.shells.visible=false
