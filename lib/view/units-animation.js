...
export function bindUnitRig(object) {
  ...
  return {object,mesh,joints,contacts,pickParts,actuators,targets,pose,scoutContacts,rise:0,groundPitch:0,phase:0,move:0,aim:0,spin:0,recoil:0,hit:0,headshot:0,death:0,
    stagger:0,heat:0,fallVelocity:0,fallAngle:0,fallSpin:0,feet:{Left:contactState(),Right:contactState()},flinches:{},severed:new Set(),states:new Set(),
...
export function animateUnit(rig,unit,dt,time,nav) {
  ...
  const melee=scout&&unit.alive&&Boolean(unit.intent?.melee)&&!moving
  ...
  for(const limb of LIMBS) {
    ...
    pose(limb.arm,armed?-.12+rig.aim*.12+rig.recoil*.07:-step*.25*rig.move-crouch*1.3,armed?-turn*.2:0,sign*(.05+crouch*.14))
    pose(limb.fore,armed?-.2-rig.aim*1.12-pitch*rig.aim:-.14-crouch*.25)
  ...
  // Actuator endpoints all read the settled anatomy pose. Submit their matrix
  // changes once instead of recursively updating each hydraulic subtree.
  rig.object.updateMatrixWorld(true)
...
function updateActuators(rig) {
  for(const a of rig.actuators){
    target.set(a.point[0],a.point[1],a.point[2]);a.targetBone.localToWorld(target);a.bone.parent.worldToLocal(target);target.sub(a.bone.position)
