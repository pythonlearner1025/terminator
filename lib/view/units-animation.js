...
import {animateUnitPlaceholder} from '../../generators/unit-placeholders.js'
...
export function animateUnit(rig,unit,dt,time,nav) {
  if(animateUnitPlaceholder(rig,unit,time))return
  ...
  const aiming=unit.alive&&!scout&&!liquid&&Boolean(unit.intent?.aimAt||unit.intent?.fire)
