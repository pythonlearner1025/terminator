...
export function animateUnit(rig, unit, dt, time) {
  ...
  if (rig.death > 0) {
    ...
    pose('Jaw', .24)
    pose('Upper Arm Right', -.2, 0, -.22)
    pose('Upper Arm Left', .1, 0, .22)
    pose('Forearm Right', -.3); pose('Forearm Left', -.5)
    pose('Thigh Left', -clamp((t-.2)/.4)*.15)
    pose('Thigh Right', -clamp((t-.25)/.4)*.35)
    pose('Shin Left', -clamp((t-.2)/.4)*.2)
    pose('Shin Right', clamp((t-.25)/.4)*.15)
    pose('Spine', clamp((t-.3)/.55)*.12)
    pose('Pelvis', -clamp((t-.45)/.55)*1.5, 0, clamp((t-.35)/.7)*.15)
    pelvisY = .22
  }
