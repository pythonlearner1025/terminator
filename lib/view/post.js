...
import {optimizeFrameRendering} from './frame-rendering.js'
...
export class MapPost {
  ...
  start() {
    ...
    // The final compositor reads the bloom texture directly. Adding it back
    // into the decoded scratch input produced an image nobody reads.
    bloom.blendToInput = false
    ...
    if (this.quality.ssaoEnabled && viewer.renderManager.gbufferTarget && viewer.renderManager.gbufferUnpackExtension) {
    ...
    this.frameRendering = optimizeFrameRendering(viewer)
  }
  setQuality(quality) {
  ...
  stop() {
    this.frameRendering?.dispose()
    this.frameRendering = null
    const manager = this.viewer.renderManager
