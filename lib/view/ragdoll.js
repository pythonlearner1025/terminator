...
import {cacheSettledPose} from './settled-pose-cache.js'
...
// A settled wreck leaves physics but keeps its whole skinned rig in the scene
// for WRECK_SECONDS. A mob of scouts dies faster than that, so the scene grew
// without limit and every frame paid for it. Past these counts the oldest
// wreck starts its normal fade instead of waiting out the full lifetime.
export const WRECK_BUDGET = 10
export const DEBRIS_BUDGET = 32
// How many wrecks stay in the solver, and how fast it runs. A quality preset
// owns both through setOptions; these are the stock values.
export const ACTIVE_BUDGET = 8
export const STEP_HZ = 1 / RAGDOLL_STEP
...
export class RagdollSystem {
  constructor(map, resourceManager, options) {
    this.resourceManager=resourceManager
    this.activeBudget=ACTIVE_BUDGET; this.stepSeconds=1/STEP_HZ
  ...
  // The two levers a quality preset owns. An option left out goes back to the
  // stock value, so switching presets never leaves the previous one's setting
  // behind. A lower budget freezes the oldest live wreck sooner; a lower rate
  // runs fewer fixed solver steps for the same wall time, and each step covers
  // more time, so a wreck still settles after the same number of seconds.
  setOptions({activeBudget = ACTIVE_BUDGET, stepHz = STEP_HZ} = {}) {
    this.activeBudget = Number.isFinite(activeBudget) ? Math.max(1, Math.floor(activeBudget)) : ACTIVE_BUDGET
    this.stepSeconds = Number.isFinite(stepHz) && stepHz > 0 ? 1 / stepHz : 1 / STEP_HZ
    return this
  }
  makeBody(mass=1){
  ...
  add(visual, unit, impact, release) {
    while(this.activeCount('unit')>=this.activeBudget)this.freeze(this.firstActive('unit'))
  ...
  addDetached(mesh, velocity, release, direction) {
    ...
    this.retire('limb', DEBRIS_BUDGET)
    return record
  }
  ...
  retainedCount(kind){let count=0;for(const record of this.records)if(record.kind===kind)count++;return count}
  // Start the oldest wrecks fading until the retained population fits the
  // budget. Each one takes the normal fade and release path, so nothing
  // disappears in a single frame and no pooled resource leaks.
  retire(kind, budget) {
    // A record already past its lifetime is on its way out, so it no longer
    // counts against the budget. Counting it would retire a second wreck for
    // the same overflow on every later death.
    const candidates = []
    for (const record of this.records) if (record.kind === kind && !this.expiring(record)) candidates.push(record)
    let over = candidates.length - budget
    if (over <= 0) return 0
    ...
    for (const record of candidates) {
      ...
      record.settledAt = this.clock - WRECK_SECONDS
      over--; retired++
    }
    return retired
  }
  expiring(record){return record.settledAt !== null && this.clock - record.settledAt >= WRECK_SECONDS}
  syncPose(record) {
    if (record.kind==='limb') {
  ...
  freeze(record) {
    ...
    // A settled wreck holds its pose until it fades. Its forty-odd bones keep
    // their world matrices, so the per-pass walk can skip the whole rig. The
    // fade below still moves it with an explicit updateMatrixWorld(true).
    record.object.matrixWorldAutoUpdate = false
    ...
    record.poseCache = cacheSettledPose(record.object)
    this.stats.settled++
  }
  update(dt) {
    ...
    while (this.accumulator + 1e-9 >= step) {
      this.accumulator -= step; this.clock += step; steps++
      if (this.active.length) this.world.step(step)
      for (let i=this.active.length-1; i>=0; i--) {
        ...
        record.quiet = quiet ? record.quiet+step : 0
    ...
    for (const record of this.records) {
      ...
      if(!record.fadeMaterial) {
        ...
        // Mesh2 swaps unregister the outgoing material before registering the
        // incoming one. Pools own both materials and their shared maps until
        // Stop; last-use disposal here would evict the warmed shader programs.
        retainResourcesDuring(this.resourceManager,()=>{mesh.material=material})
        record.fadeMesh=mesh;record.fadeOriginal=original;record.fadeMaterial=material;record.fadeOwned=!prepared
        record.object.userData.beginWreckFade?.()
      }
  ...
  release(record) {
    ...
    if (record.settledAt===null) this.freeze(record)
    record.poseCache?.dispose(); record.poseCache = null
    if(record.fadeMaterial){
      retainResourcesDuring(this.resourceManager,()=>{record.fadeMesh.material=record.fadeOriginal})
      record.fadeMaterial.opacity=1
      if(record.fadeOwned)record.fadeMaterial.dispose()
    }
    record.object.matrixWorldAutoUpdate = true
