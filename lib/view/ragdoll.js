...
export class RagdollSystem {
  constructor(map) {
    ...
    this.inverse = new Matrix4(); this.c = new Vec3(); this.impulse = new Vec3()
    this.stats = {frames: 0, steps: 0, totalMs: 0, lastMs: 0, maxMs: 0, active: 0, settled: 0, recycled: 0}
  }
  body(box, position, quaternion, mass) {
    const b = this.freeBodies.pop() || new Body({mass, collisionFilterGroup: DEBRIS,
      collisionFilterMask: TERRAIN, linearDamping: .15, angularDamping: .3,
      sleepSpeedLimit: .16, sleepTimeLimit: .6})
    b.mass = mass; b.type = Body.DYNAMIC
    if (!b.shapes.length) b.addShape(new Box(new Vec3()))
    const half = b.shapes[0].halfExtents
    half.set(Math.max(.025, box.x/2), Math.max(.025, box.y/2), Math.max(.025, box.z/2))
    b.shapes[0].updateConvexPolyhedronRepresentation(); b.shapes[0].updateBoundingSphereRadius()
  ...
  add(visual, unit, impact, release) {
    ...
    const record = {visual, object, release, parts: [], constraints: [], born: this.clock,
      settledAt: null, quiet: 0, sinkY: object.position.y, materials: [], kind: 'unit'}
    const drivers = new Map(JOINTS.map(([name]) => [rig.joints[name], name]))
    const boxes = new Map(JOINTS.map(([name]) => [name, new Box3()]))
    for (const contact of rig.contacts) {
      ...
      if (hidden) continue
      while (bone && !drivers.has(bone)) bone = bone.parent
      if (!bone) continue
      ...
      for (const corner of contact.corners) {
        ...
        boxes.get(drivers.get(bone)).expandByPoint(this.p)
      }
    }
    const byName = new Map()
    for (const [index, [name, parentName, angle, twistAngle]] of JOINTS.entries()) {
      const bone = rig.joints[name], box = boxes.get(name)
      if (!bone || box.isEmpty()) continue
      const center = box.getCenter(new Vector3())
      bone.matrixWorld.decompose(this.p, this.q, this.scale)
      const size = box.getSize(new Vector3()).multiply(this.scale).addScalar(.012)
      const offset = center.clone().multiply(this.scale)
      bone.localToWorld(this.p.copy(center))
      const mass = masses[index] * (unit.type==='heavy' ? 1.65 : unit.type==='scout' ? .7 : 1)
      const body = this.body(size, this.p, this.q, mass)
      ...
      const part = {bone, body, offset}
      record.parts.push(part); byName.set(name, part)
      const parent = byName.get(parentName)
      if (!parent) continue
      bone.getWorldPosition(this.p)
      const pivot = new Vec3(this.p.x, this.p.y, this.p.z)
      ...
      parent.body.pointToLocalFrame(pivot, joint.pivotA); body.pointToLocalFrame(pivot, joint.pivotB)
      // Limits are relative to the captured gait, so death never snaps to a T pose.
      body.vectorToWorldFrame(new Vec3(0,1,0), this.c)
    ...
    record.byBone = new Map(record.parts.map(part => [part.bone, part]))
    record.nodes = []; object.traverse(node => record.nodes.push(node))
    ...
    while (this.active.filter(r => r.kind==='unit').length > 8) this.freeze(this.active.find(r => r.kind==='unit'))
    return record
  }
  kick(record, impact) {
    if (!impact) return
    const {direction, point, weapon, headshot} = impact
    const strength = weapon==='shotgun' ? 4.8 : weapon==='grenade' ? 6 : weapon==='plasma' ? 3.2 : 1.25
    ...
    if (target && point) {
      ...
      if (headshot) body.angularVelocity.set(-direction.z*9, 1.5, direction.x*9)
      else if(strength>3) body.angularVelocity.set(-direction.z*4, .4, direction.x*4)
    }
  }
  addDetached(mesh, velocity, release) {
    mesh.geometry.computeBoundingBox()
    const box = mesh.geometry.boundingBox, center = box.getCenter(new Vector3())
    mesh.updateMatrixWorld(true); mesh.matrixWorld.decompose(this.p, this.q, this.scale)
    mesh.localToWorld(this.p.copy(center))
    const body = this.body(box.getSize(new Vector3()).multiply(this.scale).addScalar(.012), this.p, this.q, 3)
    body.velocity.copy(velocity); body.angularVelocity.set(3,1,2)
    const record = {object: mesh, release, parts: [{body, offset: center.multiply(this.scale)}], constraints: [],
      born: this.clock, settledAt: null, quiet: 0, sinkY: mesh.position.y, materials: [], kind: 'limb'}
    this.records.add(record); this.active.push(record)
    while (this.active.filter(r => r.kind==='limb').length>12) this.freeze(this.active.find(r => r.kind==='limb'))
    return record
  }
  syncPose(record) {
    if (record.kind==='limb') {
  ...
  freeze(record) {
    ...
    if(record.visual) {
      const {rig}=record.visual, bounds=new Box3()
      ...
      rig.mesh.boundingSphere=bounds.getBoundingSphere(new Sphere());rig.mesh.frustumCulled=true
    }
    for (const joint of record.constraints) { this.world.removeConstraint(joint); this.freeConstraints.push(joint) }
    for (const {body} of record.parts) { this.world.removeBody(body); body.sleep(); this.freeBodies.push(body) }
    record.constraints = []; record.parts = []; record.byBone?.clear(); record.nodes = null
  ...
  update(dt) {
    ...
    for (const record of this.records) {
      ...
      if (!record.materials.length) {
        ...
        mesh.material = material; record.materials.push({mesh, original, material})
      }
      record.object.position.y = record.sinkY - fade*.65
      for (const {material} of record.materials) material.opacity = 1-fade
      record.object.updateMatrixWorld(true)
    }
    const ms = performance.now()-start
    Object.assign(this.stats, {frames: this.stats.frames+1, steps: this.stats.steps+steps,
      lastMs: ms, totalMs: this.stats.totalMs+ms, maxMs: Math.max(this.stats.maxMs,ms), active: this.active.filter(r=>r.kind==='unit').length})
  }
  release(record) {
    if (record.settledAt===null) this.freeze(record)
    for (const {mesh, original, material} of record.materials) { mesh.material = original; material.dispose() }
  ...
  dispose() {
    ...
    this.terrain=[]; this.freeBodies=[]; this.freeConstraints=[]
  }
}
