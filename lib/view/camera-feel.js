...
export class CameraFeel {
  constructor(){this.eventIndex=0;this.hitStopEnabled=false;this.kick=0;this.velocity=0;this.roll=0;this.rollVelocity=0;this.dip=0;this.dipVelocity=0;this.shake=0;this.freezeUntil=0;this.lastTime=0;this.headshotKills=0;this.shots=0}
  consume(world) {
    const player=world.getPlayer?.(world.localPlayerId||world.hostPlayerId)||world.player
    for(const unit of world.units||[]){
      if(unit.type!=='hktank'||!unit.alive)continue
      const speed=Math.hypot(unit.vel?.x||0,unit.vel?.z||0)
      if(speed>.1)this.shake=Math.max(this.shake,.008*Math.min(1,speed)*Math.max(0,1-distance(unit.pos,player.pos)/16))
    }
    for(;this.eventIndex<world.eventLog.length;this.eventIndex++){
      ...
      if(event.type==='kill' && event.headshot && (event.playerId||player.id)===player.id){if(this.hitStopEnabled)this.freezeUntil=performance.now()+HIT_STOP_MS;this.headshotKills++}
    }
  }
  // View-only: the caller holds presentation for this window. Core steps and party clocks keep running.
  // Off by default: a 40 ms presentation hold reads as three dropped frames at 75 fps.
  get hitStopped(){return performance.now()<this.freezeUntil}
  apply(camera, simulationSeconds) {
    const now=simulationSeconds===undefined?performance.now():simulationSeconds*1000,dt=Math.min(.05,Math.max(0,now-(this.lastTime??now))/1000)
    this.lastTime=now
