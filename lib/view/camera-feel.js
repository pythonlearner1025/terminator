...
const IMPULSES={pistol:.68,m4:.38,shotgun:1.7,plasma:.92,knife:.12}
// Presentation springs never feed their offsets back into core pitch or yaw.
export class CameraFeel {
  constructor(){this.eventIndex=0;this.kick=0;this.velocity=0;this.roll=0;this.rollVelocity=0;this.shake=0;this.freezeUntil=0;this.lastTime=0;this.headshotKills=0;this.shots=0}
  consume(world) {
    for(;this.eventIndex<world.eventLog.length;this.eventIndex++){
      const event=world.eventLog[this.eventIndex]
      if(event.type==='shot' && event.by===world.player.id){
        const impulse=(IMPULSES[event.weapon]||.4)*(world.player.aiming?.72:1)
      ...
      if(event.type==='shot' && event.unitType==='heavy' && event.origin && distance(event.origin,world.player.pos)<18) this.shake=Math.max(this.shake,.009)
      if(event.type==='explosion' && event.pos) this.shake=Math.max(this.shake,.09*Math.max(0,1-distance(event.pos,world.player.pos)/22)**1.4)
      if(event.type==='kill' && event.headshot && (event.playerId||world.player.id)===world.player.id){this.freezeUntil=performance.now()+40;this.headshotKills++}
    }
  }
  ...
  apply(camera) {
    ...
    this.kick=Math.min(.13,Math.max(-.015,this.kick));this.shake*=Math.exp(-dt*9)
    if(!camera)return
  ...
  dispose(){this.kick=0;this.velocity=0;this.roll=0;this.rollVelocity=0;this.shake=0;this.freezeUntil=0;this.lastTime=0}
}
