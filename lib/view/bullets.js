...
export class BulletView {
  ...
  arrive(p) {
    ...
    if(p.kind==='unit') {
      if(m.unitView?.visuals.has(e.unitId)&&world.unitById?.has(e.unitId))m.unitView.damageEvent(world,e)
  ...
  dispose(){this.reset();this.pool.dispose();releaseSubtree(this.root);this.root.removeFromParent();this.manager=null;this.world=null}
}
