...
export class MenuScene {
  ...
  build() {
    ...
    this.figure.traverse(child => {
      ...
      if (child.isSkinnedMesh || child.name === 'Combined articulated steel') {
        ...
        if (!steel.envMap && child.material.envMap) {
          steel.envMap = child.material.envMap.clone()
          steel.envMap.needsUpdate = true
          steel.envMapIntensity = .75
        }
    ...
    this.root.add(floor)
    const fill = new E.HemisphereLight(0x9dcfff, 0x090e17, 1.2)
    const key = new E.DirectionalLight(0xd7e6ef, 3.2); key.position.set(-3,4,4)
    ...
    this.root.add(fill, key, rim, red)
  ...
  render() {
    ...
    camera.position.set(.08+this.cursor.x*.018*motion, STAGE_Y+1.5-this.cursor.y*.012*motion, 3.35)
    camera.lookAt(.08,STAGE_Y+1.28,0)
