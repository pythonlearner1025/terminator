...
export class MenuScene {
  ...
  build() {
    ...
    const stageEnvironment = new E.RGBELoader().load(new URL('../assets/textures/units/studio_small_09_1k.hdr', import.meta.url).href)
    stageEnvironment.mapping = E.EquirectangularReflectionMapping
    ...
    const steel = new E.PhysicalMaterial({name:'Menu worn steel', color:0xffffff, vertexColors:true,
      ...
      aoMap:maps.ao, metalness:1, roughness:.58, normalScale:new E.Vector2(.12,.12),
      envMap:stageEnvironment, envMapIntensity:.9})
    ...
    const floorMaterial = new E.PhysicalMaterial({name:'Wet steel stage', map:maps.albedo, normalMap:maps.normal,
      ...
      roughness:.95, metalness:.3, normalScale:new E.Vector2(.15,.15), envMap:steel.envMap, envMapIntensity:.08})
    ...
    this.figureBox = new E.Box3().setFromObject(this.figure)
  ...
  render() {
    ...
    const box = this.figureBox, size = box.getSize(new E.Vector3()), center = box.getCenter(new E.Vector3())
    const aspect = camera.aspect || (this.viewer.canvas.clientWidth/Math.max(1,this.viewer.canvas.clientHeight))
    const halfTan = Math.tan(camera.fov*Math.PI/360)
    const shift = Math.min(.75, Math.max(0,(aspect-1)*.6))
    const distance = Math.max((size.y*1.18/2)/halfTan, ((size.x+2*shift)*1.25/2)/(halfTan*aspect), 2.6)
    const lookX = center.x-shift, lookY = center.y+.05
    camera.position.set(lookX+this.cursor.x*.018*motion, lookY+.12-this.cursor.y*.012*motion, center.z+distance)
