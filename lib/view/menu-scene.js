...
export class MenuScene {
  ...
  async load(progress) {
    ...
    this.stageEnvironment?.dispose();this.stageEnvironment=null
  ...
  build() {
    const source = this.viewer.scene.modelRoot.getObjectByName('Unit T-800 Endo')
      || this.viewer.scene.modelRoot.getObjectByName('Unit_T-800_Endo')
    ...
    this.figure = clonePlacedUnitFigure(source, 'endo')
    ...
    this.pose={type:'endo',alive:true,pos:{x:0,y:0,z:0},yaw:0,vel:{x:0,y:0,z:0},intent:{aimAt:{x:0,y:.6,z:3}}}
    ...
    this.geometries.add(floor.geometry)
  ...
  render() {
    ...
    // Pulled back so the skull and feet both stay inside the frame at the game camera's field of view.
    const distance = aspect >= 1.2 ? 4.7 : 4.7 + (1.2-aspect)*2.4
    camera.position.set(shift+this.cursor.x*.018*motion, STAGE_Y+1.5-this.cursor.y*.012*motion, distance)
    camera.lookAt(shift,STAGE_Y+1.12,0)
