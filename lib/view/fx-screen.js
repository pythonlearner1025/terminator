...
export class WeaponScreenFx {
  ...
  layout() {
    ...
    let index=0
    for(const p of this.worldFx.pools.bolts.items){
      if(index===4)break
      if(p.life<=0||p.color.b<.8||p.color.r>.7)continue
      this.projected.copy(p.pos).project(camera)
      if(this.projected.z>1||this.projected.z<0)continue
      const radius=Math.min(.08,.5/Math.max(1,p.pos.distanceTo(camera.position)))
    ...
    const r=this.viewer.canvas.getBoundingClientRect(),key=`${r.x},${r.y},${r.width},${r.height}`
