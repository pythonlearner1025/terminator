...
export class RangeInspect {
  constructor(manager) {
    this.manager=manager;this.active=false;this.theta=1.3;this.phi=.18;this.zoom=2.6;this.fov=42
  ...
  pose() {
    ...
    camera.position.copy(this.target).add(this.offset);camera.fov=this.fov;camera.lookAt(this.target);camera.updateProjectionMatrix();camera.updateMatrixWorld(true)
