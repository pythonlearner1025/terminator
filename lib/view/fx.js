...
export class WeaponFx {
  constructor(parent,projection,metal,worldFx) {
    this.worldFx=worldFx;this.metal=metal;this.position=new Vector3();this.velocity=new Vector3();this.size=new Vector3();this.quaternion=new Quaternion();this.parentQuaternion=new Quaternion();this.rig=null
  ...
  fire(rig,id,interval) {
    ...
    this.flash.scale.setScalar(id==='shotgun'?1.8:id==='plasma'?1.25:id==='m4'?1.3:.8)
    this.flashScale=this.flash.scale.x;this.layoutFlash()
  ...
  layoutFlash() {
    ...
    this.flash.position.add(this.velocity.set(0,0,-.035).applyQuaternion(this.flash.quaternion));this.flash.scale.setScalar(this.flashScale||1)
  }
