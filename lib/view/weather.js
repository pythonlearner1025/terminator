...
export function createWeather(api, parent, map, {particle, runtime = false} = {}) {
  const settings = {...defaults}, root = new api.Group()
  ...
  for (const [zone, pos, target, radius, power] of [
    ...
    root.add(beam);shafts.push({zone,beam,light,power})
  }
  ...
  const dust = new api.Points(dustGeometry,new api.PointsMaterial({map:particle,color:0xc9d9db,size:.033,transparent:true,opacity:.32,depthWrite:false}))
  ...
  const weather = {
    ...
    sync(t, state) {
      ...
      geometry.setDrawRange(0,Math.round(count*Math.min(1,Math.max(0,settings.rain)))*2)
      for(let i=0;i<count;i++) {
      ...
      for(const shaft of shafts) {
        ...
        shaft.light.intensity=on?shaft.power:0
