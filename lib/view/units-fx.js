...
export class UnitFx {
  constructor(parent) {
    ...
    this.pools={};this.bodies=[];this.nextDecal=0;this.nextEmber=0;this.clock=0
    ...
    for(const [name,color,capacity]of [['sparks',0xffc572,192],['oil',0x15120f,96],['plasma',0x90dfff,48],['bullet',0xffa749,64]]){
      ...
      this.pools[name]={mesh,next:0,items:Array.from({length:capacity},()=>({life:0,max:1,pos:new E.Vector3(),vel:new E.Vector3(),scale:new E.Vector3(),floor:0}))}
    }
  ...
  update(dt,nav){
    ...
    for(const [name,pool]of Object.entries(this.pools)){
      ...
      pool.mesh.visible=active>0;pool.mesh.instanceMatrix.needsUpdate=true
    }
  ...
  dispose(){
    ...
    for(const p of Object.values(this.pools)){p.mesh.geometry.dispose();if(p.mesh.material!==this.materials.metal)p.mesh.material.dispose();p.mesh.dispose?.()}
    ...
    this.plane.dispose();this.root.removeFromParent();this.bodies=[];this.pools={}
  }
}
...
export class UnitOptics {
  constructor(parent){
    ...
    this.temp=new E.Object3D();this.color=new E.Color();this.pools={}
    for(const [name,capacity,geometry,source]of [['lens',128,new E.SphereGeometry(.012,10,6),shared.eye],['glow',448,new E.PlaneGeometry(1,1),shared.halo]]){
      ...
      this.root.add(mesh);this.pools[name]={mesh,power,next:0}
    }
  }
  update(visuals){
    for(const p of Object.values(this.pools))p.next=0
    for(const v of visuals){
      if(!v.object.visible)continue
      const emitters=v.rig.eyes?.visible?v.rig.eyes.children:[]
      const children=v.rig.muzzleHeat?.userData.batchVisible?[...emitters,v.rig.muzzleHeat]:emitters
    ...
    for(const p of Object.values(this.pools)){p.mesh.count=p.next;p.mesh.visible=p.next>0;p.mesh.instanceMatrix.needsUpdate=true;if(p.mesh.instanceColor)p.mesh.instanceColor.needsUpdate=true;p.power.needsUpdate=true}
  }
  dispose(){for(const p of Object.values(this.pools)){p.mesh.geometry.dispose();p.mesh.material.dispose();p.mesh.dispose?.()}this.root.removeFromParent()}
}
