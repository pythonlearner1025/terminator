...
export class UnitFx {
  constructor(parent,ragdolls) {
    ...
    this.ragdolls=ragdolls;this.limbPool=[]
  ...
  sever(visual,name,normal,floor){
    ...
    const mesh=this.limbPool.pop()||new E.Mesh2(new E.BufferGeometry(),this.materials.metal)
    const geometry=mesh.geometry
    for(const [key,data,size] of [['position',positions,3],['uv',uvs,2]]) {
      const old=geometry.attributes[key]
      if(old?.array.length===data.length){old.array.set(data);old.needsUpdate=true}
      else {geometry.dispose();geometry.setAttribute(key,new E.Float32BufferAttribute(data,size));if(key==='position')geometry.deleteAttribute('normal')}
    }
    geometry.computeVertexNormals();geometry.computeBoundingBox();geometry.computeBoundingSphere()
    mesh.name='Detached '+name;mesh.position.copy(origin);mesh.quaternion.identity();mesh.visible=true;this.root.add(mesh)
    const velocity=(visual.impact?.direction||normal).clone().multiplyScalar(2.2).add(new E.Vector3(0,2.1,0))
    const debris={mesh}
    this.bodies.push(debris)
    this.ragdolls.addDetached(mesh,velocity,()=>{
      mesh.removeFromParent();mesh.visible=false;this.bodies.splice(this.bodies.indexOf(debris),1)
      if(this.limbPool.length<12)this.limbPool.push(mesh);else geometry.dispose()
    })
  ...
  dispose(){
    ...
    for(const mesh of this.limbPool)mesh.geometry.dispose()
