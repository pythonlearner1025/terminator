...
const TEMPLATE_NAMES = {scout:'Unit Template Scout',endo:'Unit Template Endo',heavy:'Unit Template Heavy',t1000:'Unit Template Endo',hkaerial:'Unit Template Endo',hktank:'Unit Template Heavy'}
...
export class UnitView {
  ...
  cloneTemplateFigure(unit, parent=this.root, track=true) {
    ...
    if(recycled) {
      ...
      recycled.unitType=unit.type;recycled.wreckSpawned=false;recycled.ragdoll=null;recycled.lastAnimationTick=undefined
    ...
    const visual={object,rig,restPose,unitType:unit.type,wreckSpawned:false,ragdoll:null,
      impact:{direction:new Vector3(),point:new Vector3(),weapon:null,headshot:false,tick:-1}}
    if(parent===this.root)this.fx.gore.prepareVisual(visual)
    return visual
  }
  primeWarmup(){
    ...
    for(let i=0;i<3;i++)for(let n=1;n<12;n++)visuals.push(this.cloneTemplateFigure(states[i],this.root,false))
    return()=>{
