...
import {applyUnitPlaceholderMaterial, createUnitPlaceholder} from '../../generators/unit-placeholders.js'
...
export const TEMPLATE_NAMES = {scout:'Unit Template Scout',endo:'Unit Template Endo',heavy:'Unit Template Heavy',t1000:'Unit Template T-1000',hkaerial:'Unit Template HK-Aerial',hktank:'Unit Template HK-Tank'}
...
export class UnitView {
  ...
  cloneTemplateFigure(unit, parent=this.root, track=true) {
    ...
    if(recycled) {
      ...
      applyUnitPlaceholderMaterial(engine,object,unit.type)
      rig.ragdollBounds=oldRig.ragdollBounds;rig.ragdollBounds.center.set(0,.9,0);rig.ragdollBounds.radius=1.65
      if(!rig.roster)rig.mesh.boundingSphere=rig.ragdollBounds
      rig.mesh.frustumCulled=true;rig.eyes.visible=!rig.roster
  ...
  primeWarmup(){
    ...
    for(let i=0;i<states.length;i++)for(let n=1;n<(i<3?12:i===5?2:8);n++)visuals.push(this.cloneTemplateFigure(states[i],this.root,false))
    return()=>{
      this.rosterFx.reset();this.fx.reset();this.ragdolls.reset();this.optics.update([])
      for(const visual of visuals){visual.object.visible=false;this.visualPool[visual.unitType].push(visual)}
    }
  }
  ensureRuntimeTemplate(type) {
    if(this.runtimeTemplates[type])return this.runtimeTemplates[type]
    const runtimeTemplate=createUnitPlaceholder(engine,type);runtimeTemplate.visible=false
    this.root.add(runtimeTemplate);this.runtimeTemplates[type]=runtimeTemplate
    const far=createUnitPlaceholder(engine,type,{detail:0});far.visible=false;this.root.add(far);this.runtimeFarTemplates[type]=far
  ...
  sync(world) {
    ...
    for(const unit of world.units) {
      ...
      // A conservative whole-unit sphere prevents visible geometry from changing LOD at a screen edge.
      this.cullSphere.center.set(unit.pos.x,unit.pos.y+(unit.type==='hktank'?1.5:2),unit.pos.z)
      this.cullSphere.radius=unit.type==='hktank'?6:unit.type==='hkaerial'?4:2.5
  ...
  processEvents(world) {
    for(;this.eventIndex<world.eventLog.length;this.eventIndex++) {
      ...
      if(event.type==='shot'&&!event.unitType)this.fx.gore.hitWrecks(event,this.visuals,world.players?.get(event.playerId)||world.player)
      ...
      if(event.type==='kill') {
        ...
        this.fx.gore.event(event,visual,unit,bone,this.v1,this.v2)
      }
      if(event.type==='unit_damage') {
        if(rosterHit(visual,event))continue
        visual.rig.hit=1
        ...
        if(!target){this.fx.gore.event(event,visual,world.unitById.get(event.unitId),null,event.pos||visual.object.position,event.direction||this.v2.set(0,0,1));continue}
