...
export class UnitView {
  ...
  start(world) {
    ...
    this.visualPool={scout:[],endo:[],heavy:[]};this.completedDeaths=new Set()
  ...
  cloneTemplateFigure(unit, parent=this.root) {
    ...
    if(recycled) {
      ...
      const oldSkeleton=oldRig.mesh.skeleton
      ...
      object.name=`${unit.id} Runtime`;parent.add(object)
      const rig=bindUnitRig(object);oldSkeleton.dispose()
      rig.highGeometry=oldRig.highGeometry;rig.lowGeometry=oldRig.lowGeometry;rig.eyeMaterials=oldRig.eyeMaterials
      rig.mesh.boundingSphere=new engine.Sphere(new Vector3(0,.9,0),1.65)
      rig.mesh.frustumCulled=true;rig.eyes.visible=true
      return {object,rig,restPose}
    }
  ...
  sync(world) {
    ...
    for(const unit of world.units) {
      ...
      if(!visual) {
        ...
        if(!unit.alive && (this.completedDeaths?.has(unit.id)||world.tick-(unit.diedAtTick??world.tick)>120)) continue
    ...
    for(const unit of world.units) {
      ...
      if(!unit.alive&&visual&&!visual.ragdoll) {
        this.fx.wreck(visual)
        this.ragdolls.add(visual,unit,visual.impact,()=>{
          this.completedDeaths.add(unit.id);this.recycleVisual(unit.id,visual,unit.type)
        })
      }
    }
  ...
  recycleVisual(id,visual,type) {
    this.fx.release(visual);visual.object.visible=false;visual.object.removeFromParent();this.visuals.delete(id)
  ...
  processEvents(world) {
    for(;this.eventIndex<world.eventLog.length;this.eventIndex++) {
      ...
      if(event.type==='unit_damage') {
        ...
        if(!unit)continue
        const direction=new Vector3().subVectors(this.v1,this.v2).normalize()
        const previous=visual.impact
        if(!previous||previous.tick!==event.tick||event.headshot||previous.weapon!=='shotgun') {
          visual.impact={direction,point:this.v1.clone(),weapon:event.weapon,headshot:event.headshot,tick:event.tick}
        }
  ...
  stop() {
    ...
    this.owner?.cleanup();this.owner=null;this.root=null;this.templates={};this.runtimeTemplates={};this.runtimeFarTemplates={};this.materials=null;this.visuals.clear();this.activeIds.clear();this.eventIndex=0;this.visualPool={};this.completedDeaths?.clear()
  }
}
