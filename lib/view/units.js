...
import {InstancedUnits, INSTANCED_UNIT_TYPES} from './instanced-units.js'
...
export class UnitView {
  constructor(viewer) {
    ...
    this.eye=new Vector3();this.impactPoint=new Vector3();this.impactNormal=new Vector3();this.impactDirection=new Vector3();this.aim=new Vector3()
    this.activeIds=new Set();this.quality=getQualityPreset('high')
    this.instanced=null;this.instancedList=[];this.instancedEnabled=true
  }
  start(world, {instanced=true}={}) {
    this.stop()
    this.instancedEnabled=instanced
    ...
    // Commons render as instanced draws with skinning on the GPU. Their roots
    // live beside the units runtime root, outside modelRoot, and hold still.
    // The range and the F8 showcase keep per-unit rigs, so they skip this.
    if(this.instancedEnabled){
      this.materials ||= unitMaterials(engine)
      this.instanced={}
      for(const type of INSTANCED_UNIT_TYPES){
        const system=new InstancedUnits(this.viewer,{source:this.templates[type],type,
          capacity:(world.maxAlive||32)+8,materials:this.fx.materials,quality:this.quality})
        system.start(this.viewer.scene)
        this.instanced[type]=system
      }
      this.instancedList=Object.values(this.instanced)
    } else {this.instanced=null;this.instancedList=[]}
  ...
  sync(world) {
    ...
    for(const unit of world.units) {
      ...
      // A common that already owns an instance, or can take one, owns no scene
      // node at all. Its pose, its place and its draw all come from flat arrays.
      const system=!visual?this.instancedFor(unit):null
      if(system) {
        if(system.has(unit.id)) {
          // A dead instance keeps its slot until the death loop hands its pose
          // to a pooled ragdoll rig, which needs that pose to still be there.
          if(unit.alive)system.place(unit)
          continue
        }
        // A full slot table is not an error: the unit falls back to a rig.
        if(unit.alive&&system.acquire(unit)){system.place(unit);opticsDirty=true;continue}
      }
      if(!visual) {
    ...
    for(const unit of world.units) {
      let visual=this.visuals.get(unit.id)
      if(!unit.alive&&!visual){visual=this.instancedDeath(world,unit);if(visual)opticsDirty=true}
    ...
    let instancedLive=0
    for(const system of this.instancedList) {
      // A unit the world dropped without a kill event releases its slot here.
      for(const id of [...system.instances.keys()])if(!ids.has(id))system.release(id)
      system.root.visible=!this.showcase
      if(this.showcase){system.packLists([],[]);continue}
      system.sync(world,{player,frustum:this.frustum,cullSphere:this.cullSphere,dt,time:world.tick/60,nav:world.nav})
      instancedLive+=system.live
    }
    ...
    if(opticsDirty||instancedLive)this.optics.update(this.showcase?this.showcase.units:this.visuals.values(),
      instancedLive?optics=>{for(const system of this.instancedList)system.emitOptics(optics)}:null)
  }
  instancedFor(unit) {
    if(!this.instanced||this.showcase)return null
    return this.instanced[unit.type]||null
  }
  instancedReport(){return this.instancedList.map(system=>system.report())}
  // Death takes a common off the flat arrays and puts it on a pooled rig, so
  // the ragdoll, the wreck ember, the fade and the gore paths are unchanged.
  instancedDeath(world,unit) {
    const system=this.instanced?.[unit.type]
    if(!system?.has(unit.id))return null
    if(world.tick-(unit.diedAtTick??world.tick)>120){system.release(unit.id);return null}
    const instance=system.release(unit.id)
    const visual=this.cloneTemplateFigure(unit)
    this.visuals.set(unit.id,visual)
    system.handOff(instance,visual)
    visual.lastAnimationTick=world.tick
    return visual
  }
  ...
  processEvents(world) {
    for(;this.eventIndex<world.eventLog.length;this.eventIndex++) {
      ...
      // A common owns no rig, so its damage and kill effects are resolved
      // against its instance instead of being looked up in `visuals`.
      if((event.type==='kill'||event.type==='unit_damage')&&event.unitId&&!this.visuals.has(event.unitId)&&this.instanceOf(event.unitId)) {
        if(!this.bullets?.handles(event))this.instancedDamage(world,event)
        continue
      }
  ...
  hasPresentation(id) {
    if(this.visuals.has(id))return true
    return Boolean(this.instanceOf(id))
  }
  instanceOf(id) {
    for(const system of this.instancedList){const instance=system.get(id);if(instance)return {system,instance}}
    return null
  }
  // What a common loses: no dents, no dismemberment, no per-bone hit ray. What
  // it keeps: a per-instance flash, sparks and oil at the resolved point, the
  // headshot burst as particles, and the impact recorded for the ragdoll.
  instancedDamage(world,event,position) {
    const found=this.instanceOf(event.unitId)
    if(!found)return false
    const {system,instance}=found
    const unit=world.unitById?.get(event.unitId)
    if(!unit)return true
    const p=world.players?.get(event.playerId)||world.player
    const eye=this.eye.set(p.pos.x,p.pos.y+(p.crouch?1.12:1.65),p.pos.z)
    const point=this.impactPoint
    if(position)point.copy(position)
    else if(event.pos||event.point)point.copy(event.pos||event.point)
    else {
      const pitch=p.pitch||0,yaw=p.yaw||0
      this.aim.set(Math.sin(yaw)*Math.cos(pitch),Math.sin(pitch),Math.cos(yaw)*Math.cos(pitch))
      this.ray.set(eye,this.aim)
      // Instance bounds, not a rig: the core already resolved the hit part.
      const hit=system.hit(instance,this.ray.ray,this.hit)
      if(hit)point.copy(hit.point)
      else point.set(unit.pos.x,unit.pos.y+(event.headshot?1.05:.75),unit.pos.z)
    }
    const outward=this.impactNormal.copy(eye).sub(point)
    if(outward.lengthSq()<1e-8)outward.set(0,1,0)
    outward.normalize()
    if(event.normal)outward.set(event.normal.x,event.normal.y,event.normal.z)
    const direction=this.impactDirection.copy(outward).negate()
    if(event.direction)direction.set(event.direction.x,event.direction.y,event.direction.z)
    instance.flash=1
    instance.impact.point.copy(point);instance.impact.direction.copy(direction)
    instance.impact.weapon=event.weapon;instance.impact.headshot=Boolean(event.headshot);instance.impact.tick=event.tick
    if(event.type==='kill')this.fx.gore.burst(point,direction,unit.pos.y,event.headshot?2:1)
    else this.fx.hit(point,event.tick||0,outward,unit.pos.y)
    return true
  }
  killEffect(world,event,position) {
    if(this.instancedDamage(world,event,position))return
  ...
  damageEvent(world,event) {
    if(this.instancedDamage(world,event))return
  ...
  setQuality(quality){this.quality=quality;for(const system of this.instancedList)system.setQuality(quality)}
  toggleShowcase(force) {
    const enable=force??!this.showcase
    if(!enable) {
      if(this.showcase) {
        for(const visual of this.showcase.units) { disposeUnitRig(visual.rig,visual.object); visual.object.removeFromParent(); releaseMeshGeometry(visual.object) }
        this.showcase.stage.removeFromParent()
        if(this.showcase.hud) this.showcase.hud.style.visibility=this.showcase.hudVisibility
        this.showcase.backdrop.geometry.dispose(); this.showcase.backdrop.material.dispose(); releaseMeshGeometry(this.showcase.backdrop)
      }
      this.showcase=null
      return
    }
  ...
  stop() {
    ...
    for(const system of this.instancedList)system.dispose()
    this.instanced=null;this.instancedList=[]
    this.rosterFx?.dispose();this.rosterFx=null
    this.ragdolls?.dispose();this.ragdolls=null
    // A dropped mesh stays reachable from its geometry until the reference is
    // cleared, and the geometry then stays in the renderer's list for good.
    for(const v of this.visuals.values()){disposeUnitRig(v.rig,v.object);releaseMeshGeometry(v.object)}
    for(const pool of Object.values(this.visualPool||{}))for(const v of pool){disposeUnitRig(v.rig,v.object);releaseMeshGeometry(v.object)}
    for(const template of [...Object.values(this.runtimeTemplates||{}),...Object.values(this.runtimeFarTemplates||{})]){
      template.traverse(child=>{if(child.isSkinnedMesh)child.skeleton.dispose()});releaseMeshGeometry(template)
    }
