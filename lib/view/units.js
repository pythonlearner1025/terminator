...
export class UnitView {
  ...
  sync(world) {
    ...
    for(const unit of world.units) {
      ...
      visual.rig.mesh.geometry=distance>9?visual.rig.lowGeometry:visual.rig.highGeometry
      if(!unit.alive&&visual.rig.death===0)this.fx.wreck(visual)
      animateUnit(visual.rig,unit,dt,world.tick/60,world.nav)
  ...
  processEvents(world) {
    for(;this.eventIndex<world.eventLog.length;this.eventIndex++) {
      ...
      if(event.type==='unit_damage') {
        ...
        const outward=this.v2.clone().sub(this.v1).normalize()
        if(event.point)this.v1.set(event.point.x,event.point.y,event.point.z)
        else if(!['grenade','knife','unknown'].includes(event.weapon)) {
          this.ray.set(this.v2,new Vector3(Math.sin(yaw)*Math.cos(pitch),Math.sin(pitch),Math.cos(yaw)*Math.cos(pitch)))
        ...
        const unit=world.units.find(u=>u.id===event.unitId)
  ...
  stop() {
    ...
    this.owner?.cleanup();this.owner=null;this.root=null;this.templates={};this.runtimeTemplates={};this.runtimeFarTemplates={};this.materials=null;this.visuals.clear();this.eventIndex=0
  }
}
