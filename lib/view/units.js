...
export class UnitView {
  ...
  sync(world) {
    ...
    for (const unit of world.units) {
      ...
      if (!visual) {
        const object = this.cloneTemplateFigure(unit)
        visual = {object}
  ...
  cloneTemplateFigure(unit) {
    const source = this.templates[unit.type]
    const preview = source.children.find((child) => child.userData?.kite3dGenerated === true)
    ...
    object.position.set(unit.pos.x, unit.pos.y, unit.pos.z)
    object.traverse((child) => {
      delete child.userData?.kite3dAuthoring
      delete child.userData?.kite3dRuntime
