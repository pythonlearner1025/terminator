...
export class PartyGuest {
  ...
  receive(event) {
    ...
    if (message.type === 'relay_ready') {
      ...
      this.nextShotTick = this.inputTick
  ...
  localShot(tick, inputs) {
    if (!inputs.fire || tick < this.nextShotTick) return null
...
export function localHitscan(world, player, inputs) {
  ...
  for (const unit of world.units) {
    ...
    const hit = rayCollider(origin, direction, world.unitHitCollider(unit, spec), 100)
    if (hit && hit.distance < closest.distance) {
      closest = {kind: 'unit', unitId: unit.id, part: hit.part.part === 'head' ? 'head' : 'body', distance: hit.distance}
    }
  }
