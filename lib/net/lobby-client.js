...
export class LobbyClient {
  ...
  patchDirector() {
    ...
    this.director.beginWave = (config = null) => {
      if (this.director.phase === 'lobby') return this.originalBeginWave(config)
  ...
  useLocalFallback() {
    ...
    if (status) status.textContent = 'LOBBY SERVER OFFLINE. STARTING BUILT-IN SKYNET.'
...
`
  ...
  flushEvents() {
    ...
    for (; this.eventIndex < this.world.eventLog.length; this.eventIndex += 1) {
      ...
      if (RELAY_EVENTS.has(event.type)) events.push(event)
    }
  ...
  async beginIntermission() {
    ...
    const summary = buildWaveSummary(this.director.lastTelemetry, {wave: this.director.wave, playerDied: !this.world.player.alive})
    const budget = waveBudget(nextWave, summary).applied
    try {
      await this.post('/game/wave_summary', {
        wave: this.director.wave,
        summary,
        replay: this.director.replays?.get(this.director.wave) || [],
  ...
  applyPlan(plan) {
    ...
    for (const [type, script] of Object.entries(plan.scripts || {})) {
      if (script?.rev) this.world.skynet.revs[type] = script.rev
    }
  ...
  endMatch() {
    if (!this.baseUrl) return
    this.post('/game/match_end', {stats: {wave: this.world.wave, player_alive: this.world.player.alive, scrap: this.world.player.scrap}})
  }
...
function configuredServerUrl() {
  ...
  return value || globalThis.SKYNET_SERVER_URL || 'http://localhost:7801'
}
