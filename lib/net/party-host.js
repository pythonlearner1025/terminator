...
export class PartyHost {
  ...
  receivePeer({peerId, data}) {
    ...
    if (!message) return
    // Transport identity takes precedence over any claimed packet identity.
  ...
  processGuestShots() {
    for (const [playerId, partyPlayer] of this.players) {
      ...
      if (weapon.projectile || weapon.penetration) {
        this.world.playerFire(playerId)
        continue
      }
      ...
      if (validHit) {
        const parts = this.world.unitCatalog.types[unit.type]?.parts || {}
        const part = resolvedPart = hit.part === 'head' && Object.hasOwn(parts, 'Head') ? 'Head' : Object.hasOwn(parts, 'Chest') ? 'Chest' : 'Hull'
        result = this.world.damageUnit(unit.id, weapon.damage, {
          source: 'player', playerId, weapon: weaponId, part, headshot: part === 'Head',
        })
      }
      ...
      this.world.emit('shot', {
        by: playerId, playerId, weapon: weaponId, hit: validHit, headshot: resolvedPart === 'Head',
...
export function networkSnapshot(world) {
  ...
  try {
    return world.snapshot({includeLastInputsBundle: false, includeScriptRevisions: false})
  }
  finally {
    world.replay = replay
...
// snapshot() immediately deep-clones this projection; retain source references
// here to avoid cloning every compact field twice. Nothing escapes by reference.
function compactNetworkTelemetry(telemetry = {}) {
  ...
  return {
    ...
    scaling: telemetry.scaling || null,
