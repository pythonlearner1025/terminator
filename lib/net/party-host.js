...
const RELIABLE_EVENT_TYPES = new Set([
  ...
  'config_applied', 'script_error', 'fuelExhausted', 'fuel_exhausted',
])
...
export class PartyHost {
  ...
  receive(event) {
    ...
    else if (message.type === 'ready') this.setReady(playerId, true)
  ...
  startMatch() {
    ...
    this.send({type: 'start', snapshot: this.world.snapshot(), acks: Object.fromEntries(this.inputAcks)})
    this.broadcastState()
  }
  ...
  step(hostInputs = EMPTY_INPUTS) {
    ...
    this.flushReliableEvents()
    if (this.world.tick - this.lastSnapshotTick >= Math.ceil(TICK_RATE / PARTY_RULES.snapshotRate)) this.sendSnapshot()
    return bundle
  }
  ...
  processGuestShots() {
    for (const [playerId, partyPlayer] of this.players) {
      ...
      const latest = this.latestInputs.get(playerId)
      const player = this.world.getPlayer(playerId)
      if (!latest?.inputs.fire || !player?.alive || player.fireCooldown > 0 || player.reloadTimer > 0) continue
      ...
      const hit = this.pendingHits.get(playerId)?.get(latest.tick)
      this.pendingHits.get(playerId)?.delete(latest.tick)
  ...
  sendSnapshot() {
    ...
    this.send({
      type: 'snapshot', snapshot: this.world.snapshot(), acks: Object.fromEntries(this.inputAcks), sentAt: this.now(),
    })
  }
  ...
  async loadInviteUrls() {
    ...
    try {
      ...
      this.inviteUrls = {lan: invite.lan_url || null, tunnel: invite.tunnel_url || null}
      this.emitState()
    } catch {}
  }
