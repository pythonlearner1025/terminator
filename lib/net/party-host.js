...
export class PartyHost {
  ...
  async loadInviteUrls() {
    ...
    try {
      ...
      this.inviteUrls = {
        lan: buildInviteLink(this.code, invite.lan_url, {useRelayHost: true}),
...
function buildInviteLink(code, relay, {useRelayHost = false} = {}) {
  if (!relay) return null
  if (typeof location === 'undefined') return relay
  const game = new URL(location.href)
  const relayUrl = new URL(relay)
  if (useRelayHost) game.hostname = relayUrl.hostname
  game.search = ''
