...
chromium.launch=async options=>{
 ...
 monitor=setInterval(async()=>{const n=Number(await readFile(cg+'/memory.current','utf8'));report.peakCgroupBytes=Math.max(n,report.peakCgroupBytes);if(n>3.5*1024**3&&!report.aborted){report.aborted='cgroup >3.5GiB';console.error(report.aborted);await save();await browser?.close()}},100)
 try{
