...
// Final bounded verification permits a 3.8 GiB guard for this sequential official check only.
...
chromium.launch=async options=>{
 ...
 const report={policy:'One 480x270 browser; official sequential headless check; JS old-space768MiB; own-cgroup abort3.8GiB',peakCgroupBytes:0}
 let browser,monitor
 const save=()=>writeFile('.kite3d/check-browser-policy.json',JSON.stringify(report,null,2))
 monitor=setInterval(async()=>{const n=Number(await readFile(cg+'/memory.current','utf8'));report.peakCgroupBytes=Math.max(n,report.peakCgroupBytes);if(n>3.8*1024**3&&!report.aborted){report.aborted='cgroup >3.8GiB';console.error(report.aborted);await save();await browser?.close()}},100)
 try{
